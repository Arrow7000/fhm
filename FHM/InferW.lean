...
`TypeOfElabHM` derivation yields another. -/
...
/-- Single-variable substitution preserves typing across the whole context.
    ...
    *rewrites the term* — the substitution is pushed through the term's
    ...
    `TypeOfElabHM.onSubst_fixed` under the hypothesis that `S` fixes `e`'s
    annotation free vars. -/
theorem TypeOfElabHM.onSubstFvar {ctx : Ctx} {e : Expr} {τ : Ty} (Z : Nat) (U : Ty)
    (hU : U.IsLC) (h : TypeOfElabHM ctx e τ) :
    TypeOfElabHM (Subst.onCtx [(Z, U)] ctx) (e.substTyFvar Z U) (Subst.onTy [(Z, U)] τ) := by
  have key := TypeOfElabHM.typ_subst_preservation (ctors := ctx.ctors) (env_post := ctx.env)
...
/-- A whole substitution preserves typing, given each replacement is LC. The
    term is rewritten (`e.substTyFvars S`) — see `TypeOfElabHM.onSubstFvar`. -/
theorem TypeOfElabHM.onSubst {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
  ...
  | cons hd S' ih =>
    ...
    have step := TypeOfElabHM.onSubstFvar Z U hU h
...
/-- **D1 fixed-term corollary.** When `S` fixes every annotation free variable of
    ...
    fixed: `TypeOfElabHM (S.onCtx ctx) e (S.onTy τ)`. This is the form `Infer.sound`
    ...
    discipline. -/
theorem TypeOfElabHM.onSubst_fixed {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
    (h_lc : ∀ p ∈ S, p.2.IsLC) (h_fix : e.substTyFvars S = e) (h : TypeOfElabHM ctx e τ) :
    TypeOfElabHM (S.onCtx ctx) e (S.onTy τ) := by
  have key := TypeOfElabHM.onSubst S h_lc h
  rwa [h_fix] at key
...
theorem Expr.mem_tyFreeVars_eraseBounds (e : Expr) (x : Nat) :
  ...
  | var _i tyArgs =>
    simp only [Expr.eraseBounds, Expr.tyFreeVars, List.mem_flatMap, List.mem_map]
    constructor
    · intro ⟨t, ht, hx⟩
      obtain ⟨t0, ht0, rfl⟩ := ht
      exact ⟨t0, ht0, (Ty.mem_freeVars_eraseBounds t0 x).mp hx⟩
    · intro ⟨t, ht, hx⟩
      exact ⟨Ty.eraseBounds t, ⟨t, ht, rfl⟩, (Ty.mem_freeVars_eraseBounds t x).mpr hx⟩
  | lambda ann body ih =>
    cases ann with
    | none =>
...
/-- Path R: structural TypeOf projects to pure HM on erased ctx, term, and result.

Same judgment family; subject is `e.eraseBounds` (binder anns / tyArgs erased).
Not a pipeline step — residual bridge only. -/
theorem TypeOfElabHM.eraseBounds_of {ctx : Ctx} {e : Expr} {τ : Ty}
    (h : TypeOfElabHM ctx e τ) :
    TypeOfElabHM ctx.eraseBounds e.eraseBounds (Ty.eraseBounds τ) := by
  ...
  | @lambda paramTy ann bodyCtx ctx body bodyTy hpc hann heq hbody ihbody =>
    ...
    refine TypeOfElabHM.lambda (Ty.IsLC.eraseBounds hpc)
  ...
  | @var dbl polyTy tyArgs ty ctx hlook hlc hinst =>
    ...
    refine TypeOfElabHM.var (polyTy := PolyTy.eraseBounds polyTy) ?_
      (Ty.AreLC.eraseBounds hlc) ?_
    · -- env lookup projects through Env.eraseBounds
  ...
  | @ctor name ctorr tyArgs ty ctx hlook htyargs hinst =>
    simp only [Expr.eraseBounds, Ctx.eraseBounds]
    refine TypeOfElabHM.ctor (ctor := Ctor.eraseBounds ctorr)
  ...
  | @letIn ann ctx boundExpr bodyCtx body bodyTy M L hwf hann hcofin heq hbody ihcofin ihbody =>
    ...
    refine TypeOfElabHM.letIn (M := PolyTy.eraseBounds M) (L := L)
    ...
    · intro branch' hmem'
      ...
      · subst hpat
        have hlook' :
          ...
          have := congrArg (Option.map Ctor.eraseBounds) hspec.lookup
        ...
        have hfields' :
          ...
          simpa [Ctor.eraseBounds_contents] using
            InstantiatesBy.forall2_eraseBounds hspec.fields
        have hbody' :
            TypeOfElabHM
        ...
        refine TypeOfElabMatchBranch.mk
          ⟨hlook', hscrut',
            by simpa [Ctor.eraseBounds_paramCount, List.length_map] using hspec.arity,
            by simpa [Ctor.eraseBounds_contents, List.length_map] using hspec.bind_count,
            hfields'⟩
          rfl hbody'
      · subst hpat
...
/-- Path R transport: typing at erased term/ctx/result, then `onSubst`, then
    residual erase again. Subject is always `eraseExpr`-projected. -/
theorem TypeOfElabHM.onSubst_eraseBounds {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
    (h_lc : ∀ p ∈ S, p.2.IsLC)
    (h : TypeOfElabHM ctx.eraseBounds e.eraseBounds (Ty.eraseBounds τ)) :
    TypeOfElabHM (S.onCtx ctx.eraseBounds).eraseBounds
      ((e.eraseBounds).substTyFvars S).eraseBounds
      (Ty.eraseBounds (S.onTy τ)) := by
  -- Residual typing → structural onSubst on already-erased subject → re-erase.
  have hS := TypeOfElabHM.onSubst S h_lc h
  -- hS : TypeOfElabHM (S.onCtx ctx.eraseBounds)
  --        ((e.eraseBounds).substTyFvars S) (S.onTy (Ty.eraseBounds τ))
  have herase := TypeOfElabHM.eraseBounds_of hS
  -- herase concludes at Ty.eraseBounds (S.onTy (Ty.eraseBounds τ));
  -- collapse to Ty.eraseBounds (S.onTy τ) via the onTy/erase commute.
...
theorem TypeOfElabHM.onSubst_eraseBounds' {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
    ...
    TypeOfElabHM (S.onCtx ctx).eraseBounds
      ((e.eraseBounds).substTyFvars S).eraseBounds
      (Ty.eraseBounds (S.onTy τ)) := by
  have key := TypeOfElabHM.onSubst_eraseBounds S h_lc h
...
/-- Build the initial specs positionally from the stored annotations, starting at
    frontier `Φ`: member `j` unannotated ↦ `mono (fvar (Φ+j))` (a fresh monotype
    var for EVERY member — annotated members simply don't use theirs, keeping the
    indexing trivial); `some σ` ↦ `poly σ` (validated `σ.WF` by the caller). -/
...
/-- `init` recovers the stored annotations one-to-one (`RecSpecs.WF.anns_eq`). -/
theorem RecSpec.map_ann_init (Φ : Nat) (anns : List (Option PolyTy)) :
  ...
  | cons a as ih =>
    cases a with
    | none => simp only [RecSpec.init, List.map_cons, RecSpec.ann, ih (Φ + 1)]
    | some σ => simp only [RecSpec.init, List.map_cons, RecSpec.ann, ih (Φ + 1)]
...
theorem RecSpec.init_length (Φ : Nat) (anns : List (Option PolyTy)) :
  ...
  conv_rhs => rw [← RecSpec.map_ann_init Φ anns]
  rw [List.length_map]

/-- The `j`-th initial spec: `mono (fvar (Φ+j))` when unannotated, `poly σ` when
    annotated. -/
theorem RecSpec.init_getElem? (Φ : Nat) (anns : List (Option PolyTy)) (j : Nat) :
    (RecSpec.init Φ anns)[j]? =
      (anns[j]?).map (fun a => match a with | none => .mono (.fvar (Φ + j)) | some σ => .poly σ) := by
...
/-- Membership characterisation of the initial specs: every member is either a
    fresh monotype var in the block `[Φ, Φ + anns.length)` or a stored scheme. -/
theorem RecSpec.mem_init {s : RecSpec} :
  ...
  | cons a as ih =>
    ...
    | some σ0 =>
      rcases List.mem_cons.mp h with rfl | h
      · exact .inr ⟨σ0, List.mem_cons_self, rfl⟩
      · rcases ih h with ⟨m, h1, h2, h3⟩ | ⟨σ, h1, h2⟩
        · exact .inl ⟨m, by omega, by simp only [List.length_cons]; omega, h3⟩
        · exact .inr ⟨σ, List.mem_cons_of_mem _ h1, h2⟩
...
/-- Close scoped type variables through a recursion group's SCHEME-ANNOTATION
    BODIES: each present `σⱼ.body` is descended at `d + σⱼ.paramCount` (shielding
    `σⱼ`'s own quantified variables), the structural inverse of Core's
    `RecGroup.openAnns`. A no-op on `none` entries and self-contained schemes. This
    keeps `open`/`close` symmetric on scheme bodies, so a scheme may reference an
    enclosing scope's type variable and the close-back correctly rebinds it. -/
def RecGroup.closeAnns (d : Nat) (Xs : List Nat) (anns : List (Option PolyTy)) :
    List (Option PolyTy) :=
  anns.map (Option.map (fun σ => { σ with body := Ty.closeOverFrom (d + σ.paramCount) Xs σ.body }))

@[simp] theorem RecGroup.closeAnns_nil (d : Nat) (Xs : List Nat) :
    RecGroup.closeAnns d Xs [] = [] := rfl

theorem RecGroup.closeAnns_cons (d : Nat) (Xs : List Nat) (a : Option PolyTy)
    (as : List (Option PolyTy)) :
    RecGroup.closeAnns d Xs (a :: as)
      = a.map (fun σ => { σ with body := Ty.closeOverFrom (d + σ.paramCount) Xs σ.body })
          :: RecGroup.closeAnns d Xs as := rfl

/-- `PolyTy.substFvars` acts only on the body (the `paramCount` is preserved). -/
theorem PolyTy.substFvars_mk : ∀ (S : List (Nat × Ty)) (pc : Nat) (b : Ty),
    PolyTy.substFvars S { paramCount := pc, body := b }
      = { paramCount := pc, body := Ty.substFvars S b }
  | [], _, _ => rfl
  | (Z, U) :: rest, pc, b => by
      simp only [PolyTy.substFvars, PolyTy.substFvar, Ty.substFvars]
      exact PolyTy.substFvars_mk rest pc (Ty.substFvar Z U b)
...
/-- Close the scoped type variables `Xs` through a recursion group's bindings: each
    binding is descended at `d + RecAnn.params aⱼ` (shielding its own scheme's
    variables when annotated), the anns consumed in lockstep — the structural
    inverse of Core's `RecGroup.openTyVarsAux`. -/
private def RecGroup.closeTyVarsAux (d : Nat) (Xs : List Nat) :
    List (Option PolyTy) → List Expr → List Expr
  | _,       []        => []
  | [],      e :: rest => e.closeTyVarsAux d Xs :: RecGroup.closeTyVarsAux d Xs [] rest
  | a :: as, e :: rest =>
      e.closeTyVarsAux (d + RecAnn.params a) Xs :: RecGroup.closeTyVarsAux d Xs as rest
end
...
def Expr.annTyFreeVars : Expr → List Nat
  ...
  | .letRec anns bindings body =>
      AnnList.annTyFreeVars anns ++ RecGroup.annTyFreeVars bindings ++ body.annTyFreeVars
where
  ...
  AnnList.annTyFreeVars : List (Option PolyTy) → List Nat
  | []        => []
  | a :: rest => (a.elim [] (fun σ => σ.body.freeVars)) ++ Expr.annTyFreeVars.AnnList.annTyFreeVars rest
...
private theorem RecGroup.closeTyVarsAux_eq_zip (d : Nat) (Xs : List Nat)
    (anns : List (Option PolyTy)) (bs : List Expr) :
    RecGroup.closeTyVarsAux d Xs anns bs
      = (bs.zip (RecGroup.shieldDepths d anns bs)).map (fun p => p.1.closeTyVarsAux p.2 Xs) := by
  induction bs generalizing anns with
  | nil => cases anns <;> rfl
  | cons hd tl ih =>
    cases anns with
    | nil =>
      simp only [RecGroup.closeTyVarsAux, RecGroup.shieldDepths, List.zip_cons_cons,
        List.map_cons, ih]
    | cons a as =>
      simp only [RecGroup.closeTyVarsAux, RecGroup.shieldDepths, List.zip_cons_cons,
        List.map_cons, ih]

theorem RecGroup.closeTyVarsAux_length (d : Nat) (Xs : List Nat)
    (anns : List (Option PolyTy)) (bs : List Expr) :
    (RecGroup.closeTyVarsAux d Xs anns bs).length = bs.length := by
  rw [RecGroup.closeTyVarsAux_eq_zip, List.length_map, List.length_zip,
...
/-- `RecGroup` close-then-reopen-at-the-same-names is the identity, given the
    per-binding identity. Direct recursion on `(anns, bindings)` (no `eq_zip`,
    avoiding the `shieldDepths`-recomputation under the nested `open (close …)`). -/
private theorem RecGroup.openClose_self (Ys : List Nat) :
    ∀ (anns : List (Option PolyTy)) (bindings : List Expr) (d : Nat),
      Expr.TyBvarBounded.RecGroup d anns bindings →
      (∀ e ∈ bindings, ∀ d', e.TyBvarBounded d' →
        (e.closeTyVarsAux d' Ys).openTyVarsAux d' Ys = e) →
      RecGroup.openTyVarsAux d Ys anns (RecGroup.closeTyVarsAux d Ys anns bindings)
        = bindings := by
  intro anns bindings
  induction bindings generalizing anns with
  | nil => intro d _ _; cases anns <;> rfl
  | cons hd tl ih =>
    intro d hbb hself
    cases anns with
    | nil =>
      simp only [Expr.TyBvarBounded.RecGroup] at hbb
      simp only [RecGroup.closeTyVarsAux, RecGroup.openTyVarsAux, List.cons.injEq]
      exact ⟨hself hd List.mem_cons_self d hbb.1,
             ih [] d hbb.2 (fun e he => hself e (List.mem_cons_of_mem _ he))⟩
    | cons a as =>
      simp only [Expr.TyBvarBounded.RecGroup] at hbb
      simp only [RecGroup.closeTyVarsAux, RecGroup.openTyVarsAux, List.cons.injEq]
      exact ⟨hself hd List.mem_cons_self (d + RecAnn.params a) hbb.1,
             ih as d hbb.2 (fun e he => hself e (List.mem_cons_of_mem _ he))⟩

/-- Close-then-reopen-at-the-same-names is the identity on scheme-annotation bodies,
    given each present body's bvars are within `d + σ.paramCount` (the anns-list lift
    of `Ty.openVarsFrom_closeOverFrom_self`). Makes `open`/`close` symmetric on the
    `letRec` scheme bodies. -/
theorem RecGroup.openAnns_closeAnns_self {d : Nat} {Ys : List Nat} {anns : List (Option PolyTy)}
    (h : ∀ σ, some σ ∈ anns → ContainsBvarsUpTo (d + σ.paramCount) σ.body) :
    RecGroup.openAnns d Ys (RecGroup.closeAnns d Ys anns) = anns := by
  simp only [RecGroup.openAnns, RecGroup.closeAnns, List.map_map]
  ...
  | some σ =>
    obtain ⟨pc, sbody⟩ := σ
    simp only [Function.comp_apply, Option.map_some, id_eq, Option.some.injEq, PolyTy.mk.injEq,
      true_and]
    exact Ty.openVarsFrom_closeOverFrom_self (h ⟨pc, sbody⟩ ha)

/-- `RecGroup.openTyVarsAux` inspects the anns only through each entry's
    `RecAnn.params` (the per-binding shield depths), and `closeAnns` preserves every
    `paramCount`; so opening bindings is insensitive to whether the anns were first
    closed. Lets `openClose_self` discharge the symmetric-close bindings goal. -/
private theorem RecGroup.openTyVarsAux_closeAnns (d : Nat) (Ys : List Nat)
    (d' : Nat) (Xs : List Nat) :
    ∀ (anns : List (Option PolyTy)) (bs : List Expr),
      RecGroup.openTyVarsAux d Ys (RecGroup.closeAnns d' Xs anns) bs
        = RecGroup.openTyVarsAux d Ys anns bs := by
  intro anns bs
  induction bs generalizing anns with
  | nil => rfl
  | cons hd tl ih =>
    ...
    | cons a as =>
      cases a with
      | none =>
        simp only [RecGroup.closeAnns_cons, Option.map_none, RecGroup.openTyVarsAux,
          RecAnn.params, ih as]
      | some σ =>
        simp only [RecGroup.closeAnns_cons, Option.map_some, RecGroup.openTyVarsAux,
          RecAnn.params, ih as]
...
theorem Expr.openTyVarsAux_closeTyVarsAux_self {Ys : List Nat} :
  ...
  | letRec anns bindings body ih_bindings ih_body =>
    ...
    simp only [Expr.closeTyVarsAux, Expr.openTyVarsAux, Expr.letRec.injEq]
    refine ⟨RecGroup.openAnns_closeAnns_self hsc, ?_, ih_body d hb'⟩
    rw [RecGroup.openTyVarsAux_closeAnns d Ys d Ys anns
        (RecGroup.closeTyVarsAux d Ys anns bindings)]
    exact RecGroup.openClose_self Ys anns bindings d hbs ih_bindings
...
The fused `letRec` node subsumes the old `letRecAnn`, and `open`/`close` descend
into scheme-annotation bodies symmetrically, so the general kernel
`openTyVars_closeTyVars_rename_of_fresh` handles every case with no freshness side
condition. The `NoRecAnn` predicate and its whole preservation family are gone. -/
...
/-- The fused-group close descends into each binding (depth-shifted by its
    annotation). Since closing over `Ys` removes every `g ∈ Ys` from each binding
    (the per-binding IH), `g` survives nowhere in the closed bindings. -/
private theorem RecGroup.not_mem_closeTyVarsAux_tyFreeVars {Ys : List Nat} {g : Nat} :
    ∀ (anns : List (Option PolyTy)) (bindings : List Expr) (d : Nat),
      (∀ e ∈ bindings, ∀ d', g ∉ (e.closeTyVarsAux d' Ys).tyFreeVars) →
      g ∉ Expr.tyFreeVars.RecGroup.tyFreeVars
            (RecGroup.closeTyVarsAux d Ys anns bindings) := by
  intro anns bindings
  induction bindings generalizing anns with
  | nil =>
    intro d _
    cases anns <;>
      simp [RecGroup.closeTyVarsAux, Expr.tyFreeVars.RecGroup.tyFreeVars]
  | cons hd tl ih =>
    intro d ihB
    cases anns with
    | nil =>
      simp only [RecGroup.closeTyVarsAux, Expr.tyFreeVars.RecGroup.tyFreeVars,
    ...
    | cons a as =>
      simp only [RecGroup.closeTyVarsAux, Expr.tyFreeVars.RecGroup.tyFreeVars,
        List.mem_append, not_or]
      exact ⟨ihB hd List.mem_cons_self (d + RecAnn.params a),
        ih as d (fun e he => ihB e (List.mem_cons_of_mem _ he))⟩

/-- Closing a fused group's scheme-annotation bodies over `Ys` removes every
    `g ∈ Ys` from them (each present body is `closeOverFrom`-ed, which strips the
    closed-over names; `none` entries contribute nothing). -/
theorem RecGroup.not_mem_closeAnns_tyFreeVars {Ys : List Nat} {d g : Nat} (hg : g ∈ Ys) :
    ∀ (anns : List (Option PolyTy)),
      g ∉ Expr.tyFreeVars.AnnList.tyFreeVars (RecGroup.closeAnns d Ys anns)
  | [] => by simp [Expr.tyFreeVars.AnnList.tyFreeVars]
  | a :: as => by
      cases a with
      | none =>
        simp only [RecGroup.closeAnns_cons, Option.map_none,
          Expr.tyFreeVars.AnnList.tyFreeVars, Option.elim, List.nil_append]
        exact RecGroup.not_mem_closeAnns_tyFreeVars hg as
      | some σ =>
        simp only [RecGroup.closeAnns_cons, Option.map_some,
          Expr.tyFreeVars.AnnList.tyFreeVars, Option.elim, List.mem_append, not_or]
        exact ⟨Ty.not_mem_closeOverFrom_freeVars hg,
          RecGroup.not_mem_closeAnns_tyFreeVars hg as⟩
...
theorem Expr.not_mem_closeTyVarsAux_tyFreeVars_of_fresh {Ys : List Nat} {g : Nat}
  ...
  | letRec anns bindings body ih_bindings ih_body =>
    intro d
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, List.mem_append, not_or]
    refine ⟨⟨RecGroup.not_mem_closeAnns_tyFreeVars hg anns, ?_⟩, ih_body d⟩
    exact RecGroup.not_mem_closeTyVarsAux_tyFreeVars anns bindings d
...
/-- Λ-outside encoding of an elaborated **mixed** `letRec` (one nest level per
    member). Given member `i`'s solved `RecSpec` with shared gen-pool `G`:

    - **unannotated** (`.mono τᵢ`): bind the per-binding generalised scheme
      `Mᵢ = genGroup G τᵢ` to a fresh copy of the group's inner fused node
      `.letRec anns rawBindings' (.var i [])` (bindings shifted past the
      `i`-enclosing wrappers — there are `n - 1 - i` of them) projecting member
      `i` monomorphically, with the generalised variables `gvᵢ = genFilter G τᵢ`
      closed back over that copy (the historical `letRec` wrapper).
    - **annotated** (`.poly σᵢ`): bind the DECLARED scheme `σᵢ` to
      `.letRec anns rawBindings' (.var i (Ty.bvarRange σᵢ.paramCount))` — the
      projection instantiates member `i` at the wrapper scheme's own bound
      variables (opened by the `letIn` rule's cofinite opener); NO `closeTyVars`
      (the annotated bindings are already closed back scheme-relatively by the
      threader). Core-side rewrap ammunition: `TypeOfElabHM.rewrap_hasScheme_poly`.

    The members are processed outermost-first; calling with member `0` last
    (innermost) keeps the body's group de Bruijn indices unchanged
    (`M₀` at `0`, …, `M₍ₙ₋₁₎` at `n-1`). -/
def Expr.letRecElabNest (G : List Nat) (anns : List (Option PolyTy)) (n : Nat)
    (rawBindings : List Expr) : List (Nat × RecSpec) → Expr → Expr
  | [], body => body
  | (i, .mono τ) :: rest, body =>
      .letIn (some (PolyTy.genGroup G τ))
        (Expr.closeTyVars (Ty.genFilter G τ)
          (.letRec anns (rawBindings.map (·.shiftFrom n (n - 1 - i))) (.var i [])))
        (Expr.letRecElabNest G anns n rawBindings rest body)
  | (i, .poly σ) :: rest, body =>
      .letIn (some σ)
        (.letRec anns (rawBindings.map (·.shiftFrom n (n - 1 - i)))
          (.var i (Ty.bvarRange σ.paramCount)))
        (Expr.letRecElabNest G anns n rawBindings rest body)

/-- The full Λ-outside elaborated mixed `letRec`: stack the per-member wrappers
    with member `0` innermost (so the body sees `M₀ … M₍ₙ₋₁₎` at indices `0 … n-1`,
    matching `solvedSpecs.map (RecSpec.bodyScheme G)`). `rawBindings` is the group's
    elaborated bindings post-`S₁` (mono members: raw `eOut`; poly members: already
    closed back over their skolems by `InferRecGroup.consPoly`). -/
def Expr.letRecElab (G : List Nat) (anns : List (Option PolyTy)) (rawBindings : List Expr)
...
Mirrors Core's `eraseBounds_openTyVarsAux` for the close direction. -/

/-- `closeOverFrom` only rewrites fvars→bvars; commutes with erase. -/
private theorem Ty.eraseBounds_closeOverFrom (d : Nat) (Xs : List Nat) (τ : Ty) :
    Ty.eraseBounds (Ty.closeOverFrom d Xs τ) =
      Ty.closeOverFrom d Xs (Ty.eraseBounds τ) := by
  induction τ using Ty.rec_strong with
  | prim _ => rfl
  | arrow a b iha ihb =>
    simp only [Ty.closeOverFrom, Ty.eraseBounds_arrow, iha, ihb]
  | bvar i => rfl
  | fvar n =>
    simp only [Ty.closeOverFrom, Ty.eraseBounds_fvar]
    cases Xs.idxOf? n with
    | none => rfl
    | some _ => rfl
  | customTy nm tys ih =>
    simp only [Ty.closeOverFrom, Ty.eraseBounds_customTy, TyList.eraseBounds_eq_map,
      TyList.closeOverFrom_eq_map, List.map_map]
    congr 1
    exact List.map_congr_left fun t ht => ih t ht
  | bl lo hi e ih =>
    simp only [Ty.closeOverFrom, Ty.eraseBounds_bl, bareListTy, TyList.closeOverFrom, ih]

/-- Local copy of Core's private `RecAnn.params_eraseBounds` (paramCount fixed). -/
...
/-- Local copy of Core's private `RecGroup.shieldDepths_eraseBounds`. -/
...
/-- `closeAnns` then pointwise erase = erase anns then closeAnns. -/
private theorem RecGroup.closeAnns_eraseBounds (d : Nat) (Xs : List Nat)
    (anns : List (Option PolyTy)) :
    (RecGroup.closeAnns d Xs anns).map (Option.map PolyTy.eraseBounds) =
      RecGroup.closeAnns d Xs (anns.map (Option.map PolyTy.eraseBounds)) := by
  simp only [RecGroup.closeAnns, List.map_map]
  apply List.map_congr_left
  intro a _
  cases a with
  | none => rfl
  | some σ =>
    ...
    | mk pc b =>
      simp only [Option.map_some, Function.comp_def, PolyTy.eraseBounds,
        Ty.eraseBounds_closeOverFrom]

private theorem Expr.eraseBounds_closeTyVarsAux (Xs : List Nat) :
    ∀ (e : Expr) (d : Nat),
      (e.closeTyVarsAux d Xs).eraseBounds = e.eraseBounds.closeTyVarsAux d Xs := by
  intro e
  induction e using Expr.rec_strong with
  | primLit _ | primBinOp _ | ctor _ =>
    intro d; simp [Expr.closeTyVarsAux, Expr.eraseBounds]
  | var n tyArgs =>
    intro d
    simp only [Expr.closeTyVarsAux, Expr.eraseBounds, Expr.var.injEq, true_and, List.map_map]
    exact List.map_congr_left fun t _ => Ty.eraseBounds_closeOverFrom d Xs t
  | app _ _ ihf iharg =>
    intro d; simp [Expr.closeTyVarsAux, Expr.eraseBounds, ihf d, iharg d]
  | lambda ann body ih =>
    intro d
    simp [Expr.closeTyVarsAux, Expr.eraseBounds, ih d]
    cases ann with
    | none => rfl
    | some t =>
      simp only [Option.map_some, Option.some.injEq]
      exact Ty.eraseBounds_closeOverFrom d Xs t
  | letIn ann rhs body ihr ihb =>
    intro d
    cases ann with
    | none =>
      simp [Expr.closeTyVarsAux, Expr.eraseBounds, ihr d, ihb d]
    | some σ =>
      cases σ with
      | mk pc b =>
        simp only [Expr.closeTyVarsAux, Expr.eraseBounds, Option.map_some,
          PolyTy.eraseBounds, ihr (d + pc), ihb d, Ty.eraseBounds_closeOverFrom]
  | match_ scrut branches ihs ihbs =>
    intro d
    simp only [Expr.closeTyVarsAux, Expr.eraseBounds, ihs d,
      BranchList.closeTyVarsAux_eq_map, List.map_map]
    -- match_.injEq leaves a pointwise goal on branch bodies
    simp [Expr.match_.injEq]
    intro p b hpb
    exact ihbs p b hpb d
  | letRec anns bindings body ihbs ihb =>
    intro d
    simp only [Expr.closeTyVarsAux, Expr.eraseBounds, ihb d,
      RecGroup.closeTyVarsAux_eq_zip, RecGroup.shieldDepths_map,
      RecGroup.shieldDepths_eraseBounds', List.map_map]
    simp only [Expr.letRec.injEq]
    refine ⟨?anns, ?binds, trivial⟩
    case anns =>
      exact RecGroup.closeAnns_eraseBounds d Xs anns
    case binds =>
      -- zip (map erase bindings) depths = map (Prod.map erase id) (zip bindings depths)
...
/-- Closing scoped fvars then erase = erase then close (same name list). -/
theorem Expr.eraseBounds_closeTyVars (Xs : List Nat) (e : Expr) :
    (e.closeTyVars Xs).eraseBounds = e.eraseBounds.closeTyVars Xs :=
  Expr.eraseBounds_closeTyVarsAux Xs e 0
...
private theorem Expr.eraseBounds_shiftFrom (threshold n : Nat) :
  ...
  | primLit _ | primBinOp _ | ctor _ =>
    simp [Expr.shiftFrom, Expr.eraseBounds]
  | var i tyArgs =>
    by_cases h : i < threshold
    · simp [Expr.shiftFrom, Expr.eraseBounds, h]
    · simp [Expr.shiftFrom, Expr.eraseBounds, h]
  | app _ _ ihf iharg =>
    simp [Expr.shiftFrom, Expr.eraseBounds, ihf, iharg]
  | lambda ann body ih =>
    simp [Expr.shiftFrom, Expr.eraseBounds, ih]
  | letIn ann rhs body ihr ihb =>
    simp [Expr.shiftFrom, Expr.eraseBounds, ihr, ihb]
  | match_ scrut branches ihs ihbs =>
  ...
  | letRec anns bindings body ihbs ihb =>
    ...
    -- residual: map (erase ∘ shift thr) = map (shift thr ∘ erase) on bindings
    refine congrArg
      (fun bs =>
...
/-- `bvarRange` is pure bvars; erase is a no-op pointwise. -/
private theorem Ty.map_eraseBounds_bvarRangeFrom :
    ∀ (s k : Nat), (Ty.bvarRangeFrom s k).map Ty.eraseBounds = Ty.bvarRangeFrom s k
  | _, 0 => rfl
  | s, k + 1 => by
...
/-- Nest-level erase commute for the Λ-outside elaboratum. -/
private theorem Expr.eraseBounds_letRecElabNest (G : List Nat) (anns : List (Option PolyTy))
    (n : Nat) (rawBindings : List Expr) (members : List (Nat × RecSpec)) (body : Expr) :
    (Expr.letRecElabNest G anns n rawBindings members body).eraseBounds =
      Expr.letRecElabNest G (anns.map (Option.map PolyTy.eraseBounds)) n
        (rawBindings.map Expr.eraseBounds)
        (members.map fun p => (p.1, RecSpec.eraseBounds p.2))
        body.eraseBounds := by
  induction members with
  | nil =>
    simp only [Expr.letRecElabNest, List.map_nil]
  | cons hd rest ih =>
    obtain ⟨i, s⟩ := hd
    cases s with
    | mono τ =>
      -- Unfold both sides to a letIn of genGroup / closeTyVars-of-letRec / nest-rest.
      simp only [Expr.letRecElabNest, List.map_cons, RecSpec.eraseBounds,
        Expr.eraseBounds_letIn, Option.map_some, PolyTy.eraseBounds_genGroup, ih]
      have hgf : Ty.genFilter G (Ty.eraseBounds τ) = Ty.genFilter G τ :=
        Ty.genFilter_eraseBounds G τ
      -- Peel letIn to the bound expression (ann/body already agree via simp+ih).
      refine congrArg
        (fun r =>
          Expr.letIn (some (PolyTy.genGroup G (Ty.eraseBounds τ))) r
            (Expr.letRecElabNest G (anns.map (Option.map PolyTy.eraseBounds)) n
              (rawBindings.map Expr.eraseBounds)
              (rest.map fun p => (p.1, RecSpec.eraseBounds p.2)) body.eraseBounds)) ?_
      -- erase (closeTyVars Xs e) = closeTyVars Xs (erase e); genFilter invariant
      rw [Expr.eraseBounds_closeTyVars, hgf]
      refine congrArg (Expr.closeTyVars (Ty.genFilter G τ)) ?_
      simp only [Expr.eraseBounds, List.map_map, Expr.eraseBounds_var, List.map_nil]
      refine congrArg (fun bs =>
          Expr.letRec (anns.map (Option.map PolyTy.eraseBounds)) bs (Expr.var i [])) ?_
      apply List.map_congr_left
      intro e _
      exact Expr.eraseBounds_shiftFrom n (n - 1 - i) e
    | poly σ =>
      simp only [Expr.letRecElabNest, List.map_cons, RecSpec.eraseBounds,
        Expr.eraseBounds_letIn, Option.map_some, ih]
      -- Peel letIn to the bound expression.
      refine congrArg
        (fun r =>
          Expr.letIn (some (PolyTy.eraseBounds σ)) r
            (Expr.letRecElabNest G (anns.map (Option.map PolyTy.eraseBounds)) n
              (rawBindings.map Expr.eraseBounds)
              (rest.map fun p => (p.1, RecSpec.eraseBounds p.2)) body.eraseBounds)) ?_
      simp only [Expr.eraseBounds, List.map_map, Expr.eraseBounds_var,
        PolyTy.eraseBounds_paramCount, Ty.map_eraseBounds_bvarRange]
      refine congrArg (fun bs =>
          Expr.letRec (anns.map (Option.map PolyTy.eraseBounds)) bs
            (Expr.var i (Ty.bvarRange σ.paramCount))) ?_
      apply List.map_congr_left
      intro e _
      exact Expr.eraseBounds_shiftFrom n (n - 1 - i) e

/-- Erase of the Λ-outside letRec elaboratum is the elaboratum of erased pieces. -/
theorem Expr.eraseBounds_letRecElab (G : List Nat) (anns : List (Option PolyTy))
    (rawBindings : List Expr) (specs : List RecSpec) (body : Expr) :
    (Expr.letRecElab G anns rawBindings specs body).eraseBounds =
      Expr.letRecElab G (anns.map (Option.map PolyTy.eraseBounds))
        (rawBindings.map Expr.eraseBounds)
        (specs.map RecSpec.eraseBounds) body.eraseBounds := by
  simp only [Expr.letRecElab, List.length_map]
  -- members-map of erase = reverse of (range.zip (specs.map erase))
  have hmem :
      (((List.range rawBindings.length).zip specs).reverse).map
          (fun p => (p.1, RecSpec.eraseBounds p.2)) =
        ((List.range rawBindings.length).zip (specs.map RecSpec.eraseBounds)).reverse := by
    rw [List.map_reverse, List.reverse_inj]
    -- zip_map_right: l₁.zip (l₂.map f) = (l₁.zip l₂).map (Prod.map id f)
...
theorem Expr.shiftFrom_tyFreeVars (e : Expr) (t n : Nat) :
  ...
  | app f arg ihf iharg => simp only [Expr.shiftFrom, Expr.tyFreeVars, ihf, iharg]
  | letIn ann rhs body ihr ihb => simp only [Expr.shiftFrom, Expr.tyFreeVars, ihr, ihb]
  | match_ scrut branches ihs ihbr =>
    simp only [Expr.shiftFrom, Expr.tyFreeVars, ihs]
    congr 1
    induction branches with
    | nil => rfl
    | cons hd tl ihtl =>
      obtain ⟨p, b⟩ := hd
      change (b.shiftFrom (t + p.bindCount) n).tyFreeVars
          ++ _ = b.tyFreeVars ++ Expr.tyFreeVars.BranchList.tyFreeVars tl
      rw [ihbr p b List.mem_cons_self,
          ihtl (fun p' b' hmem => ihbr p' b' (List.mem_cons_of_mem _ hmem))]
  | letRec anns bindings body ihbs ihb =>
    simp only [Expr.shiftFrom, Expr.tyFreeVars, ihb]
    congr 1
    congr 1
    ...
    | nil => rfl
    | cons hd tl ihtl =>
      change (hd.shiftFrom thr n).tyFreeVars
          ++ _ = hd.tyFreeVars ++ Expr.tyFreeVars.RecGroup.tyFreeVars tl
...
inductive Infer : Nat → Ctx → Expr → Nat → Subst → Expr → Ty → Prop
  ...
  | primBinOpCharLt {Φ ctx trueC falseC} :
    LookupList.get? ctx.ctors ⟨"True"⟩  = some trueC → trueC.IsBoolCtor →
    LookupList.get? ctx.ctors ⟨"False"⟩ = some falseC → falseC.IsBoolCtor →
    Infer Φ ctx (.primBinOp .charLt) Φ [] (.primBinOp .charLt)
  ...
  /-- (Mutually) recursive binding group with **per-binding optional annotations**
      — the FUSION of monomorphic recursion (Damas–Milner / Pottier `LetRec`) for
      unannotated members and annotated *polymorphic* recursion (Pottier
      `LetRecPoly`) for annotated ones; mirrors the fused `TypeOfElabHM.letRec`.

      Build the initial specs positionally from `anns` at frontier `Φ`
      (`RecSpec.init Φ anns`): member `j` unannotated ↦ `mono (fvar (Φ+j))` (a
      fresh monotype var for EVERY member — annotated members simply don't use
      theirs); `some σ` ↦ `poly σ` (each annotated scheme `σ.WF`). The group RHS
      context binds every member at `RecSpec.rhsEntry [] []` (monos at their fresh
      monotype var, polys at their full declared scheme). Thread the fused
      `InferRecGroup` (monomorphic-unify for `mono`, skolem-check for `poly`).
      Afterwards, generalise the SOLVED specs (`RecSpec.onSubst S₁` — monotypes
      move under `S₁`, schemes stay RIGID) over the shared pool
      `G = genGroupVars (RecGroup.rigidVars anns bindings) … (monoTys solvedSpecs)`
      (poly members contribute nothing to the pool; the stored SCHEMES' scoped
      variables are rigid — they live in the group checking env) and type the body
      at `solvedSpecs.map (RecSpec.bodyScheme G)` (monos generalised, polys at
      their schemes). The output is the mixed `Λ`-nest `Expr.letRecElab`. -/
  | letRec {Φ ctx anns bindings body Φ₁ Φ₂ S₁ S₂ bindingsOut bodyOut τ₂} :
    (∀ σ, some σ ∈ anns → σ.WF) →
...
inductive InferRecGroup : Nat → Ctx → List Expr → List RecSpec → Nat → Subst → List Expr → Prop
  | nil {Φ ctx} : InferRecGroup Φ ctx [] [] Φ [] []
  | consMono {Φ ctx e rest τ specs Φ₁ Φ₂ S₁ S₂ S₃ eOut restOut τ'} :
    Infer Φ ctx e Φ₁ S₁ eOut τ' →
    UnifyRel τ' (S₁.onTy τ) S₂ →
    InferRecGroup Φ₁ (S₂.onCtx (S₁.onCtx ctx)) rest
      (specs.map (RecSpec.onSubst (S₁ ++ S₂))) Φ₂ S₃ restOut →
    InferRecGroup Φ ctx (e :: rest) (.mono τ :: specs) Φ₂ (S₁ ++ S₂ ++ S₃)
      (eOut :: restOut)
  | consPoly {Φ N ctx σ specs e rest Φ₁ Φ₂ S₁ Schk S₂ eOut restOut τ} :
    Φ ≤ N →
    ...
    InferRecGroup Φ₁ (Schk.onCtx (S₁.onCtx ctx)) rest
      (specs.map (RecSpec.onSubst (S₁ ++ Schk))) Φ₂ S₂ restOut →
...
theorem Expr.substTyFvars_tyBvarBounded {S : List (Nat × Ty)} (hS : ∀ p ∈ S, p.2.IsLC) :
  ...
  | var i tyArgs =>
    ...
    exact ContainsBvarsUpTo.substFvars hS (hb t0 ht0)
  | lambda ann body ih =>
    intro d hb
...
theorem InferRecGroup.frontier_le {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut) : Φ ≤ Φ' := by
  cases h with
  | nil => omega
  | consMono he _ hrest =>
...
/-- A fused `InferRecGroup` derivation preserves the group length in its output. -/
theorem InferRecGroup.bindingsOut_length {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut) :
    bindingsOut.length = bindings.length := by
  induction bindings generalizing Φ ctx specs Φ' S bindingsOut with
  | nil => cases h with | nil => rfl
  | cons e rest ih =>
    cases h with
    | consMono _ _ hrest => simp only [List.length_cons]; exact congrArg (· + 1) (ih hrest)
    | consPoly _ _ _ _ _ hrest => simp only [List.length_cons]; exact congrArg (· + 1) (ih hrest)

/-- An `InferRecGroup` derivation has matching binding/target lengths. -/
theorem InferRecGroup.length_eq {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut) : bindings.length = specs.length := by
  induction bindings generalizing Φ ctx specs Φ' S bindingsOut with
  | nil => cases h with | nil => rfl
  | cons e rest ih =>
    cases h with
...
theorem InferRecGroup.lc {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
...
/-- The principal generalization `genScheme env τ₁` is a `HasSchemeVars` witness:
    if `rhs` types as `τ₁` then it types at every fresh opening of `genScheme`.
    (The opening renames the generalization candidates `genVars` — which are not
    fixed by the env, `genVars_not_mem` — to the fresh `Xs`.) -/
theorem genScheme_hasSchemeVars {ctx : Ctx} {rhs : Expr} {τ₁ : Ty}
    (hrhs : TypeOfElabHM ctx rhs τ₁) (hlc : τ₁.IsLC) :
...
theorem HasSchemeVars.onSubst {L : List Nat} {ctx : Ctx} {rhs : Expr} {M : PolyTy}
  ...
  have hfin := TypeOfElabHM.onSubst_fixed S hS
    (Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars
      (fun p hp hc => (Expr.tyFreeVars_openTyVars hc).elim (fun h => hS_rhs p hp h)
        (fun h => hX_S p hp h)))
    hwit
  show TypeOfElabHM (S.onCtx ctx) (rhs.instTy (Xs.map Ty.fvar)) (Ty.openVars Xs (S.onTy M.body))
  rw [Expr.instTy_fvar_eq_openTyVars, ← Subst.onTy_openVars hS hX_S]
  exact hfin

/-! ### Free-var subset lemmas for the close-back operations (for `Infer.locality`)

Closing scoped variables only ever maps `fvar`s to `bvar`s, never the reverse, so
it never *introduces* a free type var. These let the locality lemma reduce the
closed-back elaborated sub-terms to their pre-close free vars. -/

/-- `Ty.closeOverFrom` introduces no new free vars. -/
theorem Ty.freeVars_closeOverFrom_subset {d : Nat} {vars : List Nat} :
    ∀ {τ : Ty} {g : Nat}, g ∈ (Ty.closeOverFrom d vars τ).freeVars → g ∈ τ.freeVars := by
  intro τ
  induction τ using Ty.rec_strong with
  | prim p => intro g h; simp [Ty.closeOverFrom, Ty.freeVars] at h
  | bvar i => intro g h; simp [Ty.closeOverFrom, Ty.freeVars] at h
  | fvar n =>
    intro g h
    simp only [Ty.closeOverFrom] at h
    cases hidx : vars.idxOf? n with
    | none => rw [hidx] at h; exact h
    | some i => rw [hidx] at h; simp [Ty.freeVars] at h
  | arrow a b iha ihb =>
    intro g h
    simp only [Ty.closeOverFrom, Ty.freeVars, List.mem_dedup, List.mem_append] at h ⊢
    rcases h with h | h
    · exact .inl (iha h)
    · exact .inr (ihb h)
  | customTy nm tys ih =>
    intro g h
    simp only [Ty.closeOverFrom, Ty.freeVars, TyList.closeOverFrom_eq_map] at h
...
private theorem RecGroup.tyFreeVars_closeTyVarsAux_subset {Xs : List Nat} {g : Nat} :
    ∀ (anns : List (Option PolyTy)) (bindings : List Expr) (d : Nat),
      (∀ e ∈ bindings, ∀ d', g ∈ (e.closeTyVarsAux d' Xs).tyFreeVars → g ∈ e.tyFreeVars) →
      g ∈ Expr.tyFreeVars.RecGroup.tyFreeVars (RecGroup.closeTyVarsAux d Xs anns bindings) →
  ...
  | cons hd tl ih =>
    ...
    | nil =>
      ...
      rcases h with h | h
      · exact .inl (ihB hd List.mem_cons_self d h)
      · exact .inr (ih [] d (fun e he => ihB e (List.mem_cons_of_mem _ he)) h)
    | cons a as =>
      simp only [RecGroup.closeTyVarsAux, Expr.tyFreeVars.RecGroup.tyFreeVars,
        List.mem_append] at h ⊢
      rcases h with h | h
      · exact .inl (ihB hd List.mem_cons_self (d + RecAnn.params a) h)
      · exact .inr (ih as d (fun e he => ihB e (List.mem_cons_of_mem _ he)) h)

/-- Closing a fused group's scheme-annotation bodies only *removes* free type vars, so a
    free var of the closed anns was already a free var of the original anns. -/
theorem RecGroup.tyFreeVars_closeAnns_subset {d : Nat} {Xs : List Nat} {g : Nat} :
    ∀ {anns : List (Option PolyTy)},
      g ∈ Expr.tyFreeVars.AnnList.tyFreeVars (RecGroup.closeAnns d Xs anns) →
      g ∈ Expr.tyFreeVars.AnnList.tyFreeVars anns
  | [], h => by simp [Expr.tyFreeVars.AnnList.tyFreeVars] at h
  | a :: as, h => by
      cases a with
      | none =>
        simp only [RecGroup.closeAnns_cons, Option.map_none,
          Expr.tyFreeVars.AnnList.tyFreeVars, Option.elim, List.nil_append] at h ⊢
        exact RecGroup.tyFreeVars_closeAnns_subset h
      | some σ =>
        simp only [RecGroup.closeAnns_cons, Option.map_some,
          Expr.tyFreeVars.AnnList.tyFreeVars, Option.elim, List.mem_append] at h ⊢
        rcases h with h | h
        · exact Or.inl (Ty.freeVars_closeOverFrom_subset h)
        · exact Or.inr (RecGroup.tyFreeVars_closeAnns_subset h)

/-- `Expr.closeTyVarsAux` introduces no new free type vars. Closing only ever *removes*
    free vars — now even through `letRecAnn` scheme bodies (descended symmetrically) — so
    the subset holds unconditionally. -/
theorem Expr.tyFreeVars_closeTyVarsAux_subset {Xs : List Nat} {g : Nat} :
    ∀ (e : Expr) (d : Nat), g ∈ (e.closeTyVarsAux d Xs).tyFreeVars → g ∈ e.tyFreeVars := by
  intro e
  induction e using Expr.rec_strong with
  ...
  | var n tyArgs =>
    intro d h
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, List.mem_flatMap] at h ⊢
    obtain ⟨t, ht, hgt⟩ := h
    obtain ⟨t0, ht0, rfl⟩ := List.mem_map.mp ht
    exact ⟨t0, ht0, Ty.freeVars_closeOverFrom_subset hgt⟩
  | lambda ann body ih =>
    intro d h
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, List.mem_append] at h ⊢
    rcases h with h | h
    · refine .inl ?_
      cases ann with
      | none => simp [Option.elim] at h
      | some t => simp only [Option.map_some, Option.elim] at h ⊢; exact Ty.freeVars_closeOverFrom_subset h
    · exact .inr (ih d h)
  | app f arg ihf iharg =>
    intro d h
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, List.mem_append] at h ⊢
  ...
  | letIn ann rhs body ihrhs ihbody =>
    ...
    | none =>
      ...
      rcases h with h | h
      · exact .inl (ihrhs d h)
      · exact .inr (ihbody d h)
    | some σ =>
      obtain ⟨pc, sbody⟩ := σ
      simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, Option.elim, List.mem_append] at h ⊢
      rcases h with (h | h) | h
      · exact .inl (.inl (Ty.freeVars_closeOverFrom_subset h))
      · exact .inl (.inr (ihrhs (d + pc) h))
      · exact .inr (ihbody d h)
  | match_ scrut branches ihscrut ihbranches =>
    intro d h
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, BranchList.closeTyVarsAux_eq_map,
      List.mem_append] at h ⊢
    rcases h with h | h
    · exact .inl (ihscrut d h)
    · refine .inr ?_
      induction branches with
      | nil => simp [Expr.tyFreeVars.BranchList.tyFreeVars] at h
      | cons hd tl ihtl =>
        obtain ⟨p, b⟩ := hd
        simp only [List.map_cons, Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append] at h ⊢
        rcases h with h | h
        · exact .inl (ihbranches p b (List.mem_cons_self ..) d h)
        · exact .inr (ihtl (fun p' b' hm => ihbranches p' b' (List.mem_cons_of_mem _ hm)) h)
  | letRec anns bindings body ih_bindings ih_body =>
    intro d h
    simp only [Expr.closeTyVarsAux, Expr.tyFreeVars, List.mem_append] at h ⊢
    rcases h with (h | h) | h
    · exact .inl (.inl (RecGroup.tyFreeVars_closeAnns_subset h))
    · exact .inl (.inr (RecGroup.tyFreeVars_closeTyVarsAux_subset anns bindings d ih_bindings h))
    · exact .inr (ih_body d h)

/-- `Expr.closeTyVars` introduces no new free type vars (top level). -/
theorem Expr.tyFreeVars_closeTyVars_subset {Xs : List Nat} {e : Expr} {g : Nat}
    (h : g ∈ (e.closeTyVars Xs).tyFreeVars) : g ∈ e.tyFreeVars :=
  Expr.tyFreeVars_closeTyVarsAux_subset e 0 h
...
mutual
theorem TypeOfElabHM.regular : {ctx : Ctx} → {e : Expr} → {τ : Ty} →
    TypeOfElabHM ctx e τ → τ.IsLC
  ...
  | _, _, _, .var _ hlc hinst => InstantiatesBy.preserves_bvars hlc.2 hinst
  ...
  | _, _, _, .letRec _ _ _ _ hbody => TypeOfElabHM.regular hbody

theorem TypeOfElabMatchBranch.regular : {ctx : Ctx} → {br : MatchPattern × Expr} →
    {scrutTy : Ty} → {rt : Ty} →
    TypeOfElabMatchBranch ctx br scrutTy rt → rt.IsLC
  | _, _, _, _, .mk _ _ hbody => TypeOfElabHM.regular hbody
...
theorem InferRecGroup.belowFvars {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
...
mutual
theorem Infer.dom_below {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
...
decreasing_by
  all_goals (try subst_vars; try simp only [Expr.size, Expr.size_openTyVars]; omega)
theorem InferBranches.dom_below {Φ ctx scrutTy ρ brs Φ' S brsOut}
    (h : InferBranches Φ ctx scrutTy ρ brs Φ' S brsOut)
    (hctx : CtxBelow Φ ctx) (hscrutTy : Ty.BelowFvars Φ scrutTy) (hρ : Ty.BelowFvars Φ ρ)
    (htfv : ∀ y ∈ Expr.tyFreeVars.BranchList.tyFreeVars brs, y < Φ) :
    (∀ p ∈ S, p.1 < Φ') := by
...
theorem InferRecGroup.dom_below {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
    (hctx : CtxBelow Φ ctx) (hspecs : ∀ s ∈ specs, s.BelowFvars Φ)
    (htfv : ∀ y ∈ Expr.tyFreeVars.RecGroup.tyFreeVars bindings, y < Φ) :
    (∀ p ∈ S, p.1 < Φ') := by
...
theorem Expr.mem_tyFreeVars_letRecElabNest {G : List Nat} {anns : List (Option PolyTy)}
    {n : Nat} {rawBindings : List Expr} :
    ∀ {members : List (Nat × RecSpec)} {body : Expr} {y : Nat},
      y ∈ (Expr.letRecElabNest G anns n rawBindings members body).tyFreeVars →
      y ∈ Expr.tyFreeVars.AnnList.tyFreeVars anns
        ∨ (∃ p ∈ members, y ∈ p.2.freeVars) ∨ (∃ e ∈ rawBindings, y ∈ e.tyFreeVars)
        ∨ y ∈ body.tyFreeVars := by
  intro members
  induction members with
  | nil => intro body y hy; exact .inr (.inr (.inr hy))
  | cons hd tl ih =>
    obtain ⟨i, s⟩ := hd
    intro body y hy
    cases s with
    | mono τ =>
      simp only [Expr.letRecElabNest, Expr.tyFreeVars, Option.elim_some, List.mem_append] at hy
      rcases hy with (hy | hy) | hy
      · refine .inr (.inl ⟨(i, .mono τ), List.mem_cons_self, ?_⟩)
        simp only [PolyTy.genGroup] at hy
        exact Ty.freeVars_closeOver_subset hy
      · have hy2 := Expr.tyFreeVars_closeTyVars_subset hy
        rw [show (Expr.letRec anns (rawBindings.map (·.shiftFrom n (n - 1 - i)))
              (Expr.var i [])).tyFreeVars
            = Expr.tyFreeVars.AnnList.tyFreeVars anns
              ++ Expr.tyFreeVars.RecGroup.tyFreeVars (rawBindings.map (·.shiftFrom n (n - 1 - i)))
              ++ (Expr.var i []).tyFreeVars from rfl, List.mem_append, List.mem_append] at hy2
        rcases hy2 with (hy2 | hy2) | hy2
        · exact .inl hy2
        · obtain ⟨e', he', hye'⟩ := Expr.mem_recGroupTyFreeVars hy2
          obtain ⟨e0, he0, rfl⟩ := List.mem_map.mp he'
          rw [Expr.shiftFrom_tyFreeVars] at hye'
          exact .inr (.inr (.inl ⟨e0, he0, hye'⟩))
        · simp [Expr.tyFreeVars] at hy2
      · rcases ih hy with h | h | h | h
        · exact .inl h
        · obtain ⟨p, hp, hyp⟩ := h
          exact .inr (.inl ⟨p, List.mem_cons_of_mem _ hp, hyp⟩)
        · exact .inr (.inr (.inl h))
        · exact .inr (.inr (.inr h))
    | poly σ =>
      simp only [Expr.letRecElabNest, Expr.tyFreeVars, Option.elim_some, List.mem_append] at hy
      rcases hy with (hy | hy) | hy
      · exact .inr (.inl ⟨(i, .poly σ), List.mem_cons_self, hy⟩)
      · rcases hy with (hy | hy) | hy
        · exact .inl hy
        · obtain ⟨e', he', hye'⟩ := Expr.mem_recGroupTyFreeVars hy
          obtain ⟨e0, he0, rfl⟩ := List.mem_map.mp he'
          rw [Expr.shiftFrom_tyFreeVars] at hye'
          exact .inr (.inr (.inl ⟨e0, he0, hye'⟩))
        · exact absurd hy Ty.not_mem_bvarRange_flatMap_freeVars
      · rcases ih hy with h | h | h | h
        · exact .inl h
        · obtain ⟨p, hp, hyp⟩ := h
          exact .inr (.inl ⟨p, List.mem_cons_of_mem _ hp, hyp⟩)
...
/-- Mixed-nest free-var locality (top level). The anns free vars fold into the
    poly members' spec free vars via `specs.map RecSpec.ann = anns`. -/
theorem Expr.mem_tyFreeVars_letRecElab {G : List Nat} {anns : List (Option PolyTy)}
    {rawBindings : List Expr} {specs : List RecSpec}
    {body : Expr} {y : Nat}
    (hanns : specs.map RecSpec.ann = anns)
    (hy : y ∈ (Expr.letRecElab G anns rawBindings specs body).tyFreeVars) :
    (∃ s ∈ specs, y ∈ s.freeVars) ∨ (∃ e ∈ rawBindings, y ∈ e.tyFreeVars) ∨ y ∈ body.tyFreeVars := by
  rcases Expr.mem_tyFreeVars_letRecElabNest hy with h | h | h | h
  ...
  · obtain ⟨p, hp, hyp⟩ := h
    exact .inl ⟨p.2, (List.of_mem_zip (List.mem_reverse.mp hp)).2, hyp⟩
  · exact .inr (.inl h)
  · exact .inr (.inr h)
...
theorem Expr.mem_tyFreeVars_substTyFvars {S : List (Nat × Ty)} {w : Nat} :
  ...
  | var i tyArgs =>
    intro h
    rw [Expr.substTyFvars_var] at h
    simp only [Expr.tyFreeVars, List.mem_flatMap] at h
    obtain ⟨t', ht', hwt'⟩ := h
    obtain ⟨t0, ht0, rfl⟩ := List.mem_map.mp ht'
    rcases Ty.mem_freeVars_substFvars hwt' with hh | hh
    · exact Or.inl (by simp only [Expr.tyFreeVars, List.mem_flatMap]; exact ⟨t0, ht0, hh⟩)
    · exact Or.inr hh
  | lambda ann body ih =>
    intro h
...
theorem Infer.eOut_avoid {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) → w ∉ e.tyFreeVars →
    (∀ p ∈ S, w ∉ p.2.freeVars) ∧ w ∉ τ.freeVars ∧ w ∉ eOut.tyFreeVars := by
  cases h with
  ...
  | primBinOpCharLt _ _ _ _ => intro w _ _ _; exact ⟨by simp, by simp [Ty.freeVars, TyList.freeVars], by simp [Expr.tyFreeVars]⟩
  | var hlook =>
    intro w hwΦ hctx _
    refine ⟨by simp, ?_, ?_⟩
    · intro hc
    ...
    · simp only [Expr.tyFreeVars, List.mem_flatMap, not_exists, not_and]
      intro t ht hwt
      obtain ⟨x, hx, rfl⟩ := List.mem_map.mp ht
  ...
  | ctor hlook =>
    intro w hwΦ _ _
    refine ⟨by simp, ?_, by simp [Expr.tyFreeVars]⟩
    intro hc
    rcases Ty.freeVars_openVars_subset w hc with h | h
  ...
  | lambda hseed hbody =>
    ...
    | none =>
      ...
      obtain ⟨hbS, hbτ, hbo⟩ := Infer.eOut_avoid hbody (w := w) (by omega)
      ...
      · simp only [Ty.freeVars, List.mem_dedup, List.mem_append, not_or]
        ...
        · exact hbS p hp hvp
      · simpa only [Expr.tyFreeVars, Option.elim_none, List.nil_append] using hbo
    | some T hT =>
      simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append, not_or] at hwe
      obtain ⟨hbS, hbτ, hbo⟩ := Infer.eOut_avoid hbody (w := w) hwΦ
        (by intro M hM; rcases List.mem_cons.mp hM with rfl | hM
  ...
  | app hf harg huni =>
    ...
    obtain ⟨hfS, hfτ, hfo⟩ := Infer.eOut_avoid hf (w := w) hwΦ hctx hwe.1
    have hfle := Infer.frontier_le hf
    have hargle := Infer.frontier_le harg
    obtain ⟨haS, haτ, hao⟩ := Infer.eOut_avoid harg (w := w) (by omega)
    ...
    · intro hc
      ...
      · simp only [Ty.freeVars, List.mem_singleton] at h; omega
      · exact hS₃ q hq hvq
    · simp only [Expr.tyFreeVars, List.mem_append, not_or]
      exact ⟨hfo, hao⟩
  | letIn hrhs hbody =>
    intro w hwΦ hctx hwe
    expose_names
    simp only [Expr.tyFreeVars, Option.elim_none, List.nil_append, List.mem_append, not_or] at hwe
    obtain ⟨hrS, hrτ, hro⟩ := Infer.eOut_avoid hrhs (w := w) hwΦ hctx hwe.1
    have hrle := Infer.frontier_le hrhs
    ...
    · simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append, not_or]
      exact ⟨⟨hMbody,
        fun hc => Expr.notMem_tyFreeVars_substTyFvars hro hrS
          (Expr.tyFreeVars_closeTyVars_subset hc)⟩, hbo⟩
  | letInAnn hσwf hΦN hrhs huni _hesc1 _hesc2 hbody =>
    intro w hwΦ hctx hwe
    expose_names
    simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append, not_or] at hwe
    have hrle := Infer.frontier_le hrhs
    obtain ⟨hrS, hrτ, hro⟩ := Infer.eOut_avoid hrhs (w := w) (by omega) hctx (by
      intro hc
    ...
    · simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append, not_or]
      exact ⟨⟨hwe.1.1,
        fun hc => Expr.notMem_tyFreeVars_substTyFvars hro
          (fun p hp => (List.mem_append.mp hp).elim (hrS p) (hSchk p))
          (Expr.tyFreeVars_closeTyVars_subset hc)⟩, hbo⟩
  | match_ hscrut hne hbr =>
    intro w hwΦ hctx hwe
    simp only [Expr.tyFreeVars, List.mem_append, not_or] at hwe
    obtain ⟨hsS, hsτ, hso⟩ := Infer.eOut_avoid hscrut (w := w) hwΦ hctx hwe.1
    have hle1 := Infer.frontier_le hscrut
    obtain ⟨hbrS, hbrρ, hbro⟩ := InferBranches.eOut_avoid hbr (w := w) (by omega)
  ...
  | letRec hwfanns hgroup hbody =>
    ...
    obtain ⟨hgS, hgo⟩ := InferRecGroup.eOut_avoid hgroup (w := w) (by omega)
      (by intro M hM
          rcases List.mem_append.mp hM with hM | hM
          · obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hM
    ...
    · intro hc
      ...
      · obtain ⟨e0, he0, rfl⟩ := List.mem_map.mp he'
        exact Expr.notMem_tyFreeVars_substTyFvars (hgo e0 he0) hgS hwe'
      · exact hbo hpbody
termination_by e.size
decreasing_by
  all_goals (try subst_vars; try simp only [Expr.size, Expr.size_openTyVars]; omega)
theorem InferBranches.eOut_avoid {Φ ctx scrutTy ρ brs Φ' S brsOut}
    (h : InferBranches Φ ctx scrutTy ρ brs Φ' S brsOut) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) → w ∉ scrutTy.freeVars →
    w ∉ ρ.freeVars → w ∉ Expr.tyFreeVars.BranchList.tyFreeVars brs →
    (∀ p ∈ S, w ∉ p.2.freeVars) ∧ w ∉ (S.onTy ρ).freeVars ∧
      w ∉ Expr.tyFreeVars.BranchList.tyFreeVars brsOut := by
  cases h with
  | nil =>
    intro w _ _ _ hρ _
    exact ⟨by simp, by simpa using hρ, by simp [Expr.tyFreeVars.BranchList.tyFreeVars]⟩
  | cons hlook hn huni0 hbody huni hrest =>
    ...
    obtain ⟨hrS, hrρ, hro⟩ := InferBranches.eOut_avoid hrest (w := w) (by omega)
    ...
    · simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append, not_or]
      exact ⟨hbo, hro⟩
  | consWild hbody huni hrest =>
    intro w hwΦ hctx hscrut hρ hbrs
    expose_names
    simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append, not_or] at hbrs
    have hle1 := Infer.frontier_le hbody
    obtain ⟨hbS, hbτ, hbo⟩ := Infer.eOut_avoid hbody (w := w) hwΦ hctx hbrs.1
    have hS₂ : ∀ p ∈ S₂, w ∉ p.2.freeVars := by
      intro p hp hwp
      rcases UnifyRel.range_mem huni p hp w hwp with h | h
      · exact hbτ h
...
theorem InferRecGroup.eOut_avoid {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) →
    (∀ s ∈ specs, w ∉ s.freeVars) → w ∉ Expr.tyFreeVars.RecGroup.tyFreeVars bindings →
    (∀ p ∈ S, w ∉ p.2.freeVars) ∧ (∀ e ∈ bindingsOut, w ∉ e.tyFreeVars) := by
  cases h with
  ...
  | consMono he huni hrest =>
    ...
    have hle1 := Infer.frontier_le he
    obtain ⟨heS, heτ, heo⟩ := Infer.eOut_avoid he (w := w) hwΦ hctx hbinds.1
    ...
    obtain ⟨hrS, hro⟩ := InferRecGroup.eOut_avoid hrest (w := w) (by omega)
  ...
  | consPoly hΦN hinfer huni hesc1 hesc2 hrest =>
    ...
    have hwopen : w ∉ (e.openTyVars (freshVars N σ.paramCount)).tyFreeVars := by
      ...
      · exact hbinds.1 h
      · have := freshVars_ge w h; omega
    obtain ⟨heS, heτ, heo⟩ := Infer.eOut_avoid hinfer (w := w) (by omega) hctx hwopen
    have hσopen : w ∉ (σ.openVars (freshVars N σ.paramCount)).freeVars := by
      intro hc
      rcases Ty.freeVars_openVars_subset w hc with h | h
      · exact hσbody h
    ...
    have hSchk : ∀ p ∈ Schk, w ∉ p.2.freeVars := by
      ...
      · exact heτ h
      · exact hσopen h
    have heo_closed : w ∉ ((eOut.substTyFvars (S₁ ++ Schk)).closeTyVars
        (freshVars N σ.paramCount)).tyFreeVars :=
      fun hc => Expr.notMem_tyFreeVars_substTyFvars heo
        (fun p hp => (List.mem_append.mp hp).elim (heS p) (hSchk p))
        (Expr.tyFreeVars_closeTyVars_subset hc)
    obtain ⟨hrS, hro⟩ := InferRecGroup.eOut_avoid hrest (w := w) (by omega)
...
mutual
theorem Infer.eliminates {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ)
  ...
  | letIn hrhs hbody =>
    ...
    intro p hp
    rcases List.mem_append.mp hp with hpS₁ | hpS₂
    · exact (Infer.eOut_avoid hbody (w := p.1) (hr_dom p hpS₁) (hbavoid p hpS₁) (hSb1 p hpS₁)).2.1
    · exact hbR p hpS₂
  | letInAnn hσwf hΦN hrhs huni hesc1 _hesc2 hbody =>
    ...
    · exact (Infer.eOut_avoid hbody (w := p.1) (hdomall p hp1Schk)
        (hbodyctx p hp1Schk) (hSbody p hp1Schk)).2.1
    · exact hbR p hpS₂
  | match_ hscrut hne hbr =>
    expose_names
    have hle1 := Infer.frontier_le hscrut
  ...
  | letRec hwfanns hgroup hbody =>
    ...
    have hcross : ∀ p ∈ S₁, ∀ q ∈ S₂, p.1 ∉ q.2.freeVars := fun p hp q hq =>
      (Infer.eOut_avoid hbody (w := p.1) (hg_dom p hp) (hbodyctx p hp) (hSbody1 p hp)).1 q hq
    refine ⟨Subst.eliminates_append hgE hbE hcross, ?_⟩
    intro p hp
    rcases List.mem_append.mp hp with hpS₁ | hpS₂
    · exact (Infer.eOut_avoid hbody (w := p.1) (hg_dom p hpS₁) (hbodyctx p hpS₁)
        (hSbody1 p hpS₁)).2.1
    · exact hbR p hpS₂
termination_by e.size
...
theorem InferBranches.eliminates {Φ ctx scrutTy ρ brs Φ' S brsOut}
  ...
  | cons hlook hn huni0 hbody huni hrest =>
    ...
    have heOutBody : ∀ p ∈ S₀, (∀ q ∈ S₁, p.1 ∉ q.2.freeVars) ∧ p.1 ∉ τb.freeVars ∧
        p.1 ∉ bodyOut.tyFreeVars := fun p hp =>
...
theorem InferRecGroup.eliminates {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
    (hctx : CtxBelow Φ ctx) (hspecs : ∀ s ∈ specs, s.BelowFvars Φ)
  ...
  | consMono he huni hrest =>
    ...
    have cross_S₁S₂_S₃ : ∀ p ∈ S₁ ++ S₂, ∀ q ∈ S₃, p.1 ∉ q.2.freeVars := by
      ...
      exact (InferRecGroup.eOut_avoid hrest (w := p.1) hpdom hpctx hptg (hSrestAll p hp)).1 q hq
  ...
  | consPoly hΦN hinfer huni hesc1 hesc2 hrest =>
    ...
    have cross2 : ∀ p ∈ S₁ ++ Schk, ∀ q ∈ S₂, p.1 ∉ q.2.freeVars := by
      ...
      exact (InferRecGroup.eOut_avoid hrest (w := p.1) (hdomall p hp) hpctx hptg hprest).1 q hq
...
theorem RecGroup.closeAnns_tyBvarBounded {Xs : List Nat} {d : Nat}
    {anns : List (Option PolyTy)}
    (h : ∀ σ, some σ ∈ anns → ContainsBvarsUpTo (d + σ.paramCount) σ.body) :
    ∀ σ, some σ ∈ RecGroup.closeAnns d Xs anns →
      ContainsBvarsUpTo (d + Xs.length + σ.paramCount) σ.body := by
  intro σ hσ
  rw [RecGroup.closeAnns] at hσ
  obtain ⟨a, ha, haeq⟩ := List.mem_map.mp hσ
  cases a with
  | none => exact absurd haeq (by simp)
  | some σ0 =>
    simp only [Option.map_some, Option.some.injEq] at haeq
    subst haeq
    have hb := Ty.closeOverFrom_preserves_bvars (d := d + σ0.paramCount) (Xs := Xs) (h σ0 ha)
    show ContainsBvarsUpTo (d + Xs.length + σ0.paramCount)
      (Ty.closeOverFrom (d + σ0.paramCount) Xs σ0.body)
    rw [show d + Xs.length + σ0.paramCount = d + σ0.paramCount + Xs.length by omega]
    exact hb

/-- The fused-group close raises each binding's shielded bvar bound by `Xs.length`
    (anns' shield depths are preserved by `closeAnns`). -/
private theorem RecGroup.closeTyVarsAux_tyBvarBounded_aux {Xs : List Nat} :
    ∀ (anns : List (Option PolyTy)) (bs : List Expr) (d : Nat),
      (∀ e ∈ bs, ∀ d', e.TyBvarBounded d' →
        (e.closeTyVarsAux d' Xs).TyBvarBounded (d' + Xs.length)) →
      Expr.TyBvarBounded.RecGroup d anns bs →
      Expr.TyBvarBounded.RecGroup (d + Xs.length) (RecGroup.closeAnns d Xs anns)
        (RecGroup.closeTyVarsAux d Xs anns bs) := by
  intro anns bs
  induction bs generalizing anns with
  | nil => intro d _ _; cases anns <;> exact trivial
  | cons hd tl ih =>
    ...
    | cons a as =>
      refine ⟨?_, ih as d (fun e he => ihB e (List.mem_cons_of_mem _ he)) hbb.2⟩
      cases a with
      | none => exact ihB hd List.mem_cons_self d hbb.1
      | some σ0 =>
        have hh := ihB hd List.mem_cons_self (d + σ0.paramCount) hbb.1
        show (hd.closeTyVarsAux (d + σ0.paramCount) Xs).TyBvarBounded
          (d + Xs.length + σ0.paramCount)
        rw [show d + Xs.length + σ0.paramCount = d + σ0.paramCount + Xs.length by omega]
        exact hh

/-- `closeTyVarsAux` raises the bvar bound by `Xs.length` (each leaf type is closed
    via `Ty.closeOverFrom`, binders advancing `d` in lockstep; the fused `letRec`'s
    scheme bodies and shielded bindings raise uniformly). -/
theorem Expr.closeTyVarsAux_tyBvarBounded {Xs : List Nat} :
  ...
  | match_ scrut branches ihs ihbr =>
    ...
    simp only [Prod.mk.injEq] at heq
    obtain ⟨_, rfl⟩ := heq
    exact ihbr p' b' hmem d (hb.2 p' b' hmem)
  | letRec anns bindings body ih_bindings ih_body =>
    intro d hb
    obtain ⟨hsch, hrg, hbody⟩ := hb
    simp only [Expr.closeTyVarsAux]
    exact ⟨RecGroup.closeAnns_tyBvarBounded hsch,
      RecGroup.closeTyVarsAux_tyBvarBounded_aux anns bindings d
        (fun e he d' => ih_bindings e he d') hrg,
      ih_body d hbody⟩
...
/-- Mapping a function over the right list of a `zip` distributes over the zip. -/
theorem List.zip_map_right_eq' {α β γ : Type _} (g : β → γ) :
    ∀ (l : List α) (r : List β), l.zip (r.map g) = (l.zip r).map (fun ab => (ab.1, g ab.2)) := by
  intro l
  induction l with
  | nil => intro r; simp
  | cons hd tl ih =>
...
/-- Shifting the group's term de Bruijn indices preserves the shielded per-binding
    bound (shield depths are insensitive to structural maps of the bindings). -/
theorem Expr.TyBvarBounded.RecGroup_map_shiftFrom {d t k : Nat}
    {anns : List (Option PolyTy)} {bs : List Expr}
    (h : Expr.TyBvarBounded.RecGroup d anns bs) :
    Expr.TyBvarBounded.RecGroup d anns (bs.map (·.shiftFrom t k)) := by
  rw [Expr.TyBvarBounded.RecGroup_iff] at h ⊢
  intro p hp
  rw [RecGroup.shieldDepths_map, List.zip_map_left_eq'] at hp
  obtain ⟨⟨b, d'⟩, hmem, rfl⟩ := List.mem_map.mp hp
  exact Expr.shiftFrom_tyBvarBounded k t d' (h (b, d') hmem)

/-- An LC type substitution over the group's bindings preserves the shielded
    per-binding bound (with the anns view unchanged). -/
theorem Expr.TyBvarBounded.RecGroup_map_substTyFvars {d : Nat} {S : List (Nat × Ty)}
    (hS : ∀ p ∈ S, p.2.IsLC) {anns : List (Option PolyTy)} {bs : List Expr}
    (h : Expr.TyBvarBounded.RecGroup d anns bs) :
    Expr.TyBvarBounded.RecGroup d anns (bs.map (·.substTyFvars S)) := by
  rw [Expr.TyBvarBounded.RecGroup_iff] at h ⊢
  intro p hp
  rw [RecGroup.shieldDepths_map, List.zip_map_left_eq'] at hp
  obtain ⟨⟨b, d'⟩, hmem, rfl⟩ := List.mem_map.mp hp
  exact Expr.substTyFvars_tyBvarBounded hS (h (b, d') hmem)

/-- The shielded group bound is monotone in the ambient depth. -/
theorem Expr.TyBvarBounded.RecGroup.monotone {n m : Nat} (hnm : n ≤ m) :
    ∀ {anns : List (Option PolyTy)} {bs : List Expr},
      Expr.TyBvarBounded.RecGroup n anns bs → Expr.TyBvarBounded.RecGroup m anns bs := by
  intro anns bs
  induction bs generalizing anns with
  | nil => intro h; cases anns <;> exact trivial
  | cons hd tl ih =>
    intro h
    cases anns with
    | nil => exact ⟨Expr.TyBvarBounded.mono h.1 hnm, ih h.2⟩
    | cons a as => exact ⟨Expr.TyBvarBounded.mono h.1 (by omega), ih h.2⟩

/-- Every member of a `bvarRange` block is bounded by the block width. -/
private theorem ContainsBvarsUpTo.bvarRangeFrom' :
    ∀ (k s : Nat), ∀ t ∈ Ty.bvarRangeFrom s k, ContainsBvarsUpTo (s + k) t
  | 0, s => by intro t ht; simp [Ty.bvarRangeFrom] at ht
  | k + 1, s => by
      intro t ht
      rcases List.mem_cons.mp ht with rfl | ht
      · exact .bvar (by omega)
      · exact (ContainsBvarsUpTo.bvarRangeFrom' k (s + 1) t ht).mono (by omega)

theorem ContainsBvarsUpTo.bvarRange {k : Nat} :
    ∀ t ∈ Ty.bvarRange k, ContainsBvarsUpTo k t := fun t ht => by
  have := ContainsBvarsUpTo.bvarRangeFrom' k 0 t ht
  rwa [Nat.zero_add] at this

/-- The mixed Λ-outside nest has no dangling type `bvar`s. Each mono wrapper's
    annotated `letIn (some Mⱼ)` absorbs the `closeTyVars`-introduced bvars; each
    poly wrapper's `letIn (some σⱼ)` carries a `WF` scheme, its inner projection's
    `Ty.bvarRange σⱼ.paramCount` tyArgs are shielded by the wrapper's own depth,
    and the inner `.letRec anns …` node is `TyBvarBounded` from the (shifted,
    ann-shielded) raw bindings. -/
theorem Expr.letRecElabNest_tyBvarBounded {G : List Nat} {anns : List (Option PolyTy)}
    {n : Nat} {rawBindings : List Expr}
    (hrawb : Expr.TyBvarBounded.RecGroup 0 anns rawBindings)
    (hanns : ∀ σ, some σ ∈ anns → σ.WF) :
    ∀ (members : List (Nat × RecSpec)) {body : Expr}, (∀ p ∈ members, p.2.LC) →
      body.TyBvarBounded 0 →
      (Expr.letRecElabNest G anns n rawBindings members body).TyBvarBounded 0 := by
  intro members
  induction members with
  ...
  | cons hd tl ih =>
    ...
    cases s with
    | mono τ =>
      have hτlc : τ.IsLC := hlc (i, .mono τ) List.mem_cons_self
      simp only [Expr.letRecElabNest, Expr.TyBvarBounded, PolyTy.genGroup, Nat.zero_add]
      refine ⟨Ty.closeOver_preserves_bvars hτlc, ?_,
        ih (fun p hp => hlc p (List.mem_cons_of_mem _ hp)) hbody⟩
      have hinner : (Expr.letRec anns (rawBindings.map (·.shiftFrom n (n - 1 - i)))
          (Expr.var i [])).TyBvarBounded 0 := by
        refine ⟨fun σ' hσ' => ?_, Expr.TyBvarBounded.RecGroup_map_shiftFrom hrawb,
          by intro t ht; simp at ht⟩
        rw [Nat.zero_add]
        exact hanns σ' hσ'
      exact Expr.closeTyVars_tyBvarBounded hinner
    | poly σ =>
      have hσwf : σ.WF := hlc (i, .poly σ) List.mem_cons_self
      simp only [Expr.letRecElabNest, Expr.TyBvarBounded, Nat.zero_add]
      refine ⟨hσwf, ?_, ih (fun p hp => hlc p (List.mem_cons_of_mem _ hp)) hbody⟩
      refine ⟨fun σ' hσ' => (hanns σ' hσ').mono (by omega),
        Expr.TyBvarBounded.RecGroup.monotone (Nat.zero_le _)
          (Expr.TyBvarBounded.RecGroup_map_shiftFrom hrawb),
        ?_⟩
      intro t ht
      exact ContainsBvarsUpTo.bvarRange t ht

/-- Top-level mixed-nest type-bvar boundedness (`letRecElab` at `TyBvarBounded 0`). -/
theorem Expr.letRecElab_tyBvarBounded {G : List Nat} {anns : List (Option PolyTy)}
    {rawBindings : List Expr} {specs : List RecSpec}
    {body : Expr} (hrawb : Expr.TyBvarBounded.RecGroup 0 anns rawBindings)
    (hanns : ∀ σ, some σ ∈ anns → σ.WF)
    (hspecs : ∀ s ∈ specs, s.LC) (hbody : body.TyBvarBounded 0) :
    (Expr.letRecElab G anns rawBindings specs body).TyBvarBounded 0 :=
...
theorem Infer.eOut_tyBvarBounded {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ)
  ...
  | primBinOpCharLt _ _ _ _ => exact trivial
  | var hlook =>
  ...
  | letIn hrhs hbody =>
    ...
    refine ⟨Ty.closeOver_preserves_bvars hτ₁lc,
      Expr.closeTyVars_tyBvarBounded
        (Expr.substTyFvars_tyBvarBounded hrhs_s (Infer.eOut_tyBvarBounded hrhs hctx)),
  ...
  | letInAnn hσwf hΦN hrhs huni _ _ hbody =>
    ...
    · have hh := Expr.closeTyVars_tyBvarBounded (Xs := freshVars N σ.paramCount)
        (Expr.substTyFvars_tyBvarBounded
          ...
          (Infer.eOut_tyBvarBounded hrhs hctx))
...
termination_by Expr.sizeBranches brs
decreasing_by all_goals (try subst_vars; try simp only [Expr.sizeBranches]; omega)
/-- Fused `InferRecGroup` outputs are type-bvar-bounded *at their annotation's
    shield depth* (the `TyBvarBounded.RecGroup` zip form): mono members are raw
    `eOut` (bounded at 0, matching their `none` ann); poly members are closed back
    over their skolems and the close introduces exactly `σ.paramCount` bvars,
    absorbed by their `some σ` ann's shield. (Statement fused from the old split
    `∀ e ∈ bindingsOut, TyBvarBounded 0` (mono) + `RecGroupAnn 0 schemes` (ann).) -/
theorem InferRecGroup.eOut_tyBvarBounded {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
    (hctx : CtxWF ctx) (hspecs : ∀ s ∈ specs, s.LC) :
    Expr.TyBvarBounded.RecGroup 0 (specs.map RecSpec.ann) bindingsOut := by
  cases h with
  | nil => exact trivial
  | consMono he huni hrest =>
    expose_names
    ...
    have hτ : τ.IsLC := hspecs (.mono τ) List.mem_cons_self
    have hS₂ := UnifyRel.lc huni hτ' (Subst.onTy_lc hS₁ hτ)
    have htail := InferRecGroup.eOut_tyBvarBounded hrest
      (Subst.onCtx_wf hS₂ (Subst.onCtx_wf hS₁ hctx))
      (fun s' hs' => by
        obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hs'
        exact RecSpec.LC.onSubst (fun p hp => (List.mem_append.mp hp).elim (hS₁ p) (hS₂ p))
          (hspecs s (List.mem_cons_of_mem _ hs)))
    refine ⟨Infer.eOut_tyBvarBounded he hctx, ?_⟩
    rw [← RecSpec.map_ann_onSubst (S₁ ++ S₂) specs]
    exact htail
  | consPoly hΦN hinfer huni hesc1 hesc2 hrest =>
    expose_names
    obtain ⟨hτlc, hS₁⟩ := Infer.lc hinfer hctx
    have hσwf : σ.WF := hspecs (.poly σ) List.mem_cons_self
    have hSchk := UnifyRel.lc huni hτlc (PolyTy.openVars_isLC hσwf (by simp))
    have htail := InferRecGroup.eOut_tyBvarBounded hrest
      (Subst.onCtx_wf hSchk (Subst.onCtx_wf hS₁ hctx))
      (fun s' hs' => by
        obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hs'
        exact RecSpec.LC.onSubst (fun p hp => (List.mem_append.mp hp).elim (hS₁ p) (hSchk p))
          (hspecs s (List.mem_cons_of_mem _ hs)))
    refine ⟨?_, ?_⟩
    · have hbb : eOut.TyBvarBounded 0 := Infer.eOut_tyBvarBounded hinfer hctx
      have hsub := Expr.substTyFvars_tyBvarBounded
        (fun p hp => (List.mem_append.mp hp).elim (hS₁ p) (hSchk p)) hbb
      have hcl := Expr.closeTyVars_tyBvarBounded (Xs := freshVars N σ.paramCount) hsub
      rw [freshVars_length] at hcl
      rw [Nat.zero_add]
      exact hcl
    · rw [← RecSpec.map_ann_onSubst (S₁ ++ Schk) specs]
      exact htail
termination_by Expr.sizeRecGroup bindings
...
private theorem RecGroup.substFvars_closeAnns {S : Subst} {Xs : List Nat}
    (hS_Xs : ∀ p ∈ S, p.1 ∉ Xs) (hS_ran : ∀ p ∈ S, ∀ x ∈ p.2.freeVars, x ∉ Xs)
    (d : Nat) (anns : List (Option PolyTy)) :
    (RecGroup.closeAnns d Xs anns).map (RecAnn.substFvars S)
      = RecGroup.closeAnns d Xs (anns.map (RecAnn.substFvars S)) := by
  simp only [RecGroup.closeAnns, List.map_map]
  ...
  | some σ =>
    simp only [Function.comp_apply, Option.map_some, RecAnn.substFvars_some,
      PolyTy.substFvars_eq, Option.some.injEq]
    rw [Ty.substFvars_closeOverFrom hS_Xs hS_ran]

/-- Group-level `substTyFvars`/`closeTyVarsAux` commute, given the pointwise
    commutation for each binding (shield depths preserved by `RecAnn.substFvars`). -/
private theorem RecGroup.substTyFvars_closeTyVarsAux_aux {S : Subst} {Xs : List Nat} :
    ∀ (anns : List (Option PolyTy)) (bs : List Expr) (d : Nat),
      (∀ e ∈ bs, ∀ (d' : Nat),
        (e.closeTyVarsAux d' Xs).substTyFvars S = (e.substTyFvars S).closeTyVarsAux d' Xs) →
      (RecGroup.closeTyVarsAux d Xs anns bs).map (·.substTyFvars S)
        = RecGroup.closeTyVarsAux d Xs (anns.map (RecAnn.substFvars S))
            (bs.map (·.substTyFvars S)) := by
  intro anns bs
  induction bs generalizing anns with
  | nil => intro d _; cases anns <;> rfl
  | cons hd tl ih =>
    intro d hcomm
    cases anns with
    | nil =>
      have h1 := hcomm hd List.mem_cons_self d
      have h2 := ih [] d (fun e he d' => hcomm e (List.mem_cons_of_mem _ he) d')
      show (hd.closeTyVarsAux d Xs).substTyFvars S
            :: (RecGroup.closeTyVarsAux d Xs [] tl).map (·.substTyFvars S)
          = (hd.substTyFvars S).closeTyVarsAux d Xs
            :: RecGroup.closeTyVarsAux d Xs [] (tl.map (·.substTyFvars S))
      rw [h1, h2, List.map_nil]
    | cons a as =>
      have h1 := hcomm hd List.mem_cons_self (d + RecAnn.params a)
      have h2 := ih as d (fun e he d' => hcomm e (List.mem_cons_of_mem _ he) d')
      show (hd.closeTyVarsAux (d + RecAnn.params a) Xs).substTyFvars S
            :: (RecGroup.closeTyVarsAux d Xs as tl).map (·.substTyFvars S)
          = (hd.substTyFvars S).closeTyVarsAux (d + RecAnn.params (RecAnn.substFvars S a)) Xs
            :: RecGroup.closeTyVarsAux d Xs (as.map (RecAnn.substFvars S))
                (tl.map (·.substTyFvars S))
      rw [h1, h2, RecAnn.params_substFvars]

/-- Iterated `substTyFvars` commutes with `closeTyVarsAux` when the substitution
    `S` avoids the closing names `Xs` in both domain and range. (Fused `letRec`:
    the scheme-annotation bodies commute by `RecGroup.substFvars_closeAnns` and
    the bindings pointwise at their preserved shield depths.) -/
theorem Expr.substTyFvars_closeTyVarsAux {S : Subst} {Xs : List Nat}
  ...
  | letIn ann be body ih_be ih_body =>
    ...
    | some σ =>
      ...
      rw [ih_be (d + σ.paramCount), ih_body d,
  ...
  | match_ scrut branches ih_scrut ih_branches =>
    ...
    exact ih_branches p b hpb d
  | letRec anns bs body ih_bs ih_body =>
    intro d
    simp only [Expr.closeTyVarsAux, Expr.substTyFvars_letRec, Expr.letRec.injEq]
    exact ⟨RecGroup.substFvars_closeAnns hS_Xs hS_ran d anns,
      RecGroup.substTyFvars_closeTyVarsAux_aux anns bs d (fun e he d' => ih_bs e he d'),
      ih_body d⟩
...
/-- **Step A — the missing close/subst commutation.** Pushing a substitution `S`
    ...
    (Re-based off `NoRecAnn`; commutes through `letRecAnn` too.) This is the
...
theorem Expr.substTyFvars_closeTyVars {S : Subst} {Xs : List Nat} {e : Expr}
  ...
  Expr.substTyFvars_closeTyVarsAux hS_Xs hS_ran e 0
...
theorem Infer.sound_letIn {ctx : Ctx} {rhsElab bodyElab : Expr}
  ...
  refine TypeOfElabHM.letIn (M := ⟨genV.length, Ty.closeOver genV τ₁⟩) (L := genV)
    (Ty.closeOver_preserves_bvars hrhs_lc)
    (fun σ' h => by cases h; rfl) ?_ rfl hbody_ty
  intro Xs hXfresh
  obtain ⟨hXlen, hXnodup, hXavoid⟩ := hXfresh
...
theorem Infer.sound_letInAnn {ctx : Ctx} {σ : PolyTy} {rhsElab bodyElab : Expr}
  ...
  refine TypeOfElabHM.letIn (M := σ) (L := Ys) hσwf
    (fun σ' h => by cases h; rfl) ?_ rfl hbody_ty
  intro Xs hXfresh
  obtain ⟨hXlen, hXnodup, hXavoid⟩ := hXfresh
...
/-- Path R residual packing for unannotated `letIn`: pure TypeOf on erased term. -/
theorem Infer.sound_letIn_erase {ctx : Ctx} {rhsElab bodyElab : Expr}
    {τ₁ τ₂ : Ty} {genV : List Nat}
    (hrhs_ty : TypeOfElabHM ctx.eraseBounds rhsElab.eraseBounds (Ty.eraseBounds τ₁))
    (hrhs_lc : τ₁.IsLC)
    (hgenV_nodup : genV.Nodup) (hgenV_env : ∀ g ∈ genV, g ∉ ctx.env.freeVars)
    (hbody_ty : TypeOfElabHM
      { ctx.eraseBounds with
          env := PolyTy.eraseBounds ⟨genV.length, Ty.closeOver genV τ₁⟩
            :: ctx.env.eraseBounds }
        bodyElab.eraseBounds (Ty.eraseBounds τ₂)) :
    TypeOfElabHM ctx.eraseBounds
      (Expr.eraseBounds
        (.letIn (some ⟨genV.length, Ty.closeOver genV τ₁⟩)
          (rhsElab.closeTyVars genV) bodyElab))
      (Ty.eraseBounds τ₂) := by
  rw [Expr.eraseBounds_letIn, Option.map_some, Expr.eraseBounds_closeTyVars]
  have hscheme :
      PolyTy.eraseBounds ⟨genV.length, Ty.closeOver genV τ₁⟩ =
        ⟨genV.length, Ty.closeOver genV (Ty.eraseBounds τ₁)⟩ := by
    simp only [PolyTy.eraseBounds, Ty.eraseBounds_closeOver]
  rw [hscheme]
  refine Infer.sound_letIn (ctx := ctx.eraseBounds) (rhsElab := rhsElab.eraseBounds)
    (bodyElab := bodyElab.eraseBounds) (genV := genV) hrhs_ty
    (Ty.IsLC.eraseBounds hrhs_lc) hgenV_nodup ?_ ?_
  · intro g hg hc
    exact hgenV_env g hg
      ((Env.mem_freeVars_eraseBounds ctx.env g).mp (by simpa [Ctx.eraseBounds] using hc))
  · simpa [Ctx.eraseBounds, hscheme] using hbody_ty

/-- Path R residual packing for annotated `letIn`: pure TypeOf on erased term. -/
theorem Infer.sound_letInAnn_erase {ctx : Ctx} {σ : PolyTy} {rhsElab bodyElab : Expr}
    {Ys : List Nat} {τ₂ : Ty}
    (hrhs_at_Ys : TypeOfElabHM ctx.eraseBounds rhsElab.eraseBounds
      (Ty.eraseBounds (σ.openVars Ys)))
    (hσwf : σ.WF) (hYnodup : Ys.Nodup) (hYlen : Ys.length = σ.paramCount)
    (hYs_env : ∀ y ∈ Ys, y ∉ ctx.env.freeVars)
    (hYs_σ : ∀ y ∈ Ys, y ∉ σ.body.freeVars)
    (hbody_ty : TypeOfElabHM
      { ctx.eraseBounds with env := PolyTy.eraseBounds σ :: ctx.env.eraseBounds }
        bodyElab.eraseBounds (Ty.eraseBounds τ₂)) :
    TypeOfElabHM ctx.eraseBounds
      (Expr.eraseBounds (.letIn (some σ) (rhsElab.closeTyVars Ys) bodyElab))
      (Ty.eraseBounds τ₂) := by
  rw [Expr.eraseBounds_letIn, Option.map_some, Expr.eraseBounds_closeTyVars]
  refine Infer.sound_letInAnn (ctx := ctx.eraseBounds) (σ := PolyTy.eraseBounds σ)
    (rhsElab := rhsElab.eraseBounds) (bodyElab := bodyElab.eraseBounds) (Ys := Ys)
    (τ₂ := Ty.eraseBounds τ₂)
    ?_ (PolyTy.WF.eraseBounds hσwf) hYnodup ?_ ?_ ?_ ?_
  · rwa [← PolyTy.eraseBounds_openVars]
  · simpa [PolyTy.eraseBounds_paramCount] using hYlen
  · intro y hy hc
    exact hYs_env y hy
      ((Env.mem_freeVars_eraseBounds ctx.env y).mp (by simpa [Ctx.eraseBounds] using hc))
  · intro y hy hc
    exact hYs_σ y hy ((Ty.mem_freeVars_eraseBounds σ.body y).mp hc)
  · simpa [Ctx.eraseBounds] using hbody_ty
...
/-- The shared-monotype `letRec` cofinite premise, built from the solved RHS
    typings by a single uniform substitution `S₂ ++ (G ↦ Xs)`: `S₂` lands the body
    context and `G ↦ Xs` is the shared opening of the gen-var pool (the gen-vars
    are renamed throughout; `S₂` avoids the bindings' annotation vars and `G` is
    disjoint from both the bindings' and the (substituted) env's free vars). -/
theorem TypeOfElabHM.letRec_cofinite_of_rhs
    {ctors : CtorEnv} {envCtx : Env} {bindings : List Expr} {τs : List Ty}
    {G Xs : List Nat} {S₂ : Subst}
    (hRHS : ∀ p ∈ bindings.zip τs,
      TypeOfElabHM ⟨τs.map PolyTy.mkTrivial ++ envCtx, ctors⟩ p.1 p.2)
    (hS₂lc : ∀ p ∈ S₂, p.2.IsLC)
    (hS₂_bind : ∀ e ∈ bindings, ∀ p ∈ S₂, p.1 ∉ e.tyFreeVars)
    (hG_bind : ∀ e ∈ bindings, ∀ g ∈ G, g ∉ e.tyFreeVars)
    (hG_env : ∀ g ∈ G, g ∉ (S₂.onEnv envCtx).freeVars) :
    ∀ p ∈ bindings.zip ((τs.map S₂.onTy).map (Ty.renameG G Xs)),
      TypeOfElabHM ⟨((τs.map S₂.onTy).map (Ty.renameG G Xs)).map PolyTy.mkTrivial
                  ++ S₂.onEnv envCtx, ctors⟩ p.1 p.2 := by
  intro p hp
  rw [List.map_map] at hp
  obtain ⟨e, τ, hmem, rfl⟩ := List.mem_zip_map_snd hp
  have he : e ∈ bindings := (List.of_mem_zip hmem).1
  have hLC : ∀ q ∈ S₂ ++ G.zip (Xs.map (Ty.fvar ·)), q.2.IsLC := by
    intro q hq
    rcases List.mem_append.mp hq with hq | hq
    · exact hS₂lc q hq
    · obtain ⟨x, _, hx⟩ := List.mem_map.mp (List.of_mem_zip hq).2
      rw [← hx]; exact ContainsBvarsUpTo.fvar
  have hsub := TypeOfElabHM.onSubst (S₂ ++ G.zip (Xs.map (Ty.fvar ·))) hLC (hRHS (e, τ) hmem)
  have hfix : e.substTyFvars (S₂ ++ G.zip (Xs.map (Ty.fvar ·))) = e := by
    refine Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars (fun q hq hc => ?_)
    rcases List.mem_append.mp hq with hq | hq
    · exact hS₂_bind e he q hq hc
    · exact hG_bind e he q.1 (List.of_mem_zip hq).1 hc
  rw [hfix] at hsub
  have htype : (S₂ ++ G.zip (Xs.map (Ty.fvar ·))).onTy τ = Ty.renameG G Xs (S₂.onTy τ) := by
    rw [Subst.onTy_append, Subst.onTy_zip_fvar_eq_renameG]
  have henv : (S₂ ++ G.zip (Xs.map (Ty.fvar ·))).onCtx ⟨τs.map PolyTy.mkTrivial ++ envCtx, ctors⟩
      = ⟨((τs.map S₂.onTy).map (Ty.renameG G Xs)).map PolyTy.mkTrivial ++ S₂.onEnv envCtx, ctors⟩ := by
    simp only [Subst.onCtx]
    refine congrArg (fun E => (⟨E, ctors⟩ : Ctx)) ?_
    show (τs.map PolyTy.mkTrivial ++ envCtx).map (S₂ ++ G.zip (Xs.map (Ty.fvar ·))).onPolyTy
        = ((τs.map S₂.onTy).map (Ty.renameG G Xs)).map PolyTy.mkTrivial ++ S₂.onEnv envCtx
    rw [List.map_append]
    congr 1
    · simp only [List.map_map]
      apply List.map_congr_left
      intro t _
      simp only [Function.comp_apply, Subst.onPolyTy, PolyTy.mkTrivial, Subst.onTy_append,
        Subst.onTy_zip_fvar_eq_renameG]
    · rw [show envCtx.map (S₂ ++ G.zip (Xs.map (Ty.fvar ·))).onPolyTy
            = (S₂ ++ G.zip (Xs.map (Ty.fvar ·))).onEnv envCtx from rfl, Subst.onEnv_append]
      exact Subst.onEnv_eq_self_of_fresh (fun q hq => hG_env q.1 (List.of_mem_zip hq).1)
...
/-- `((range l.length).zip l).map (f ∘ snd) = l.map f` (the zip's second
    components are exactly `l`). -/
private theorem map_snd_range_zip {α β : Type _} (f : α → β) (l : List α) :
    (((List.range l.length).zip l).map (fun p => f p.2)) = l.map f := by
  apply List.ext_getElem
  · simp [List.length_zip, List.length_range]
  · intro k h1 h2
    simp only [List.getElem_map, List.getElem_zip]

/-- The length of a `bvarRange` block. -/
private theorem Ty.bvarRangeFrom_length : ∀ (n s : Nat), (Ty.bvarRangeFrom s n).length = n
  | 0, _ => rfl
  | n + 1, s => by
      simp only [Ty.bvarRangeFrom, List.length_cons, Ty.bvarRangeFrom_length n (s + 1)]

/-- Opening a full `bvarRange` block at names `Xs` (of matching length) yields
    exactly the names-as-fvars block. -/
private theorem Ty.bvarRange_openVarsFrom_eq_fvars {m : Nat} {Xs : List Nat}
    (hXlen : Xs.length = m) :
    (Ty.bvarRange m).map (Ty.openVarsFrom 0 Xs) = Xs.map Ty.fvar := by
  apply List.ext_getElem
  · simp only [List.length_map, Ty.bvarRange, Ty.bvarRangeFrom_length, hXlen]
  · intro k h1 h2
    have hkm : k < m := by
      simpa only [List.length_map, Ty.bvarRange, Ty.bvarRangeFrom_length] using h1
    have hklen : k < (Ty.bvarRange m).length := by
      simp only [Ty.bvarRange, Ty.bvarRangeFrom_length]; exact hkm
    have hbv : (Ty.bvarRange m)[k] = Ty.bvar k := by
      have h := Ty.bvarRange_getElem? (n := m) (k := k) hkm
      rw [List.getElem?_eq_getElem hklen] at h
      exact Option.some.inj h
    have hkX : k < Xs.length := by omega
    simp only [List.getElem_map, hbv, Ty.openVarsFrom, Ty.instantiate, Nat.sub_zero,
      List.getElem?_eq_getElem hkX, Option.elim_some, if_neg (Nat.not_lt_zero k)]

/-- Opening is a no-op on a shielded-bounded recursion group's bindings. -/
private theorem RecGroup.openTyVarsAux_eq_self {Xs : List Nat} :
    ∀ {anns : List (Option PolyTy)} {bs : List Expr} {d : Nat},
      Expr.TyBvarBounded.RecGroup d anns bs →
      RecGroup.openTyVarsAux d Xs anns bs = bs := by
  intro anns bs
  induction bs generalizing anns with
  | nil => intro d _; cases anns <;> rfl
  | cons hd tl ih =>
    ...
    cases anns with
    | nil =>
      show hd.openTyVarsAux d Xs :: RecGroup.openTyVarsAux d Xs [] tl = hd :: tl
      rw [Expr.openTyVarsAux_eq_self_of_tyBvarBounded Xs hd d h.1, ih h.2]
    | cons a as =>
      show hd.openTyVarsAux (d + RecAnn.params a) Xs :: RecGroup.openTyVarsAux d Xs as tl
          = hd :: tl
      rw [Expr.openTyVarsAux_eq_self_of_tyBvarBounded Xs hd _ h.1, ih h.2]

/-- Opening the inner mixed group node at `Xs` only opens the projection's
    `bvarRange` tyArgs (anns and bindings are shielded-closed). -/
private theorem Expr.letRec_proj_openTyVars {anns : List (Option PolyTy)} {bs' : List Expr}
    {i m : Nat} {Xs : List Nat}
    (hanns : ∀ σ, some σ ∈ anns → σ.WF)
    (hbs : Expr.TyBvarBounded.RecGroup 0 anns bs')
    (hXlen : Xs.length = m) :
    (Expr.letRec anns bs' (.var i (Ty.bvarRange m))).openTyVars Xs
      = Expr.letRec anns bs' (.var i (Xs.map Ty.fvar)) := by
  show Expr.letRec (RecGroup.openAnns 0 Xs anns) (RecGroup.openTyVarsAux 0 Xs anns bs')
      (Expr.openTyVarsAux 0 Xs (.var i (Ty.bvarRange m)))
    = Expr.letRec anns bs' (.var i (Xs.map Ty.fvar))
  rw [RecGroup.openAnns_eq_self_of_bvars (fun σ hσ => by rw [Nat.zero_add]; exact hanns σ hσ),
      RecGroup.openTyVarsAux_eq_self hbs]
  show Expr.letRec anns bs' (.var i ((Ty.bvarRange m).map (Ty.openVarsFrom 0 Xs))) = _
  rw [Ty.bvarRange_openVarsFrom_eq_fvars hXlen]

/-- The inner mixed group node of the Λ-outside nest: re-derive the fused
    declarative rule at the EMPTY pool (the solved-form hypotheses ARE its
    cofinite premises — opening names are immaterial at pool `[]`), absorbing the
    term-var `shiftFrom` into the inserted wrapper env `accEnv` via `weaken_env`. -/
private theorem innerFusedLetRec_sound {ctors : CtorEnv} {ctxEnv accEnv : Env}
    {Lp : List Nat} {anns : List (Option PolyTy)} {bs : List Expr} {specs : List RecSpec}
    {n i : Nat}
    (hwf_anns : specs.map RecSpec.ann = anns)
    (hwf_len : bs.length = specs.length)
    (hwf_mono : ∀ τ, RecSpec.mono τ ∈ specs → τ.IsLC)
    (hwf_poly : ∀ σ, RecSpec.poly σ ∈ specs → σ.WF)
    (hn : n = bs.length) (hshift : n - 1 - i = accEnv.length)
    (hmono : ∀ p ∈ bs.zip specs, ∀ τ, p.2 = RecSpec.mono τ →
      TypeOfElabHM ⟨specs.map (RecSpec.rhsEntry [] []) ++ ctxEnv, ctors⟩ p.1 τ)
    (hpoly : ∀ p ∈ bs.zip specs, ∀ σ, p.2 = RecSpec.poly σ →
      ∀ Ys, FreshNames Lp σ.paramCount Ys →
        TypeOfElabHM ⟨specs.map (RecSpec.rhsEntry [] []) ++ ctxEnv, ctors⟩
          (p.1.openTyVars Ys) (σ.openVars Ys))
    {e : Expr} {t : Ty}
    (hbody : TypeOfElabHM ⟨specs.map (RecSpec.rhsEntry [] []) ++ (accEnv ++ ctxEnv), ctors⟩
      e t) :
    TypeOfElabHM ⟨accEnv ++ ctxEnv, ctors⟩
      (.letRec anns (bs.map (·.shiftFrom n (n - 1 - i))) e) t := by
  have hmap_len : (specs.map (RecSpec.rhsEntry [] [])).length = n := by
    rw [List.length_map, ← hwf_len, ← hn]
  refine TypeOfElabHM.letRec (specs := specs) (G := []) (L := Lp)
    (bodyCtx := ⟨specs.map (RecSpec.rhsEntry [] []) ++ (accEnv ++ ctxEnv), ctors⟩)
    ⟨hwf_anns, by rw [List.length_map]; exact hwf_len, List.nodup_nil, hwf_mono, hwf_poly⟩
    ?_ ?_ ?_ hbody
  · -- mono cofinite premise at the empty pool: identity opening + `weaken_env`
    intro Zs _hZs p hp τ hτ
    rw [List.zip_map_left_eq'] at hp
    obtain ⟨⟨b, s⟩, hq, rfl⟩ := List.mem_map.mp hp
    have hs : s = RecSpec.mono τ := hτ
    subst hs
    simp only [RecSpecs.rhsCtx]
    rw [show Ty.renameG [] Zs τ = τ from Ty.renameG_nil_pool,
        List.map_congr_left (fun s' _ => RecSpec.rhsEntry_nil_any Zs s')]
    have hw := TypeOfElabHM.weaken_env (env_pre := specs.map (RecSpec.rhsEntry [] []))
      (env_extra := accEnv) (env := ctxEnv) (hmono (b, .mono τ) hq τ rfl)
    rw [hmap_len, ← hshift, List.append_assoc] at hw
    exact hw
  · -- poly cofinite premise at the empty pool: skolems transfer + `weaken_env`
    intro Zs _hZs p hp σ hσ Ys hYs
    rw [List.zip_map_left_eq'] at hp
    obtain ⟨⟨b, s⟩, hq, rfl⟩ := List.mem_map.mp hp
    have hs : s = RecSpec.poly σ := hσ
    subst hs
    simp only [RecSpecs.rhsCtx]
    rw [List.map_congr_left (fun s' _ => RecSpec.rhsEntry_nil_any Zs s')]
    have hYs' : FreshNames Lp σ.paramCount Ys :=
      ⟨hYs.length, hYs.nodup, fun y hy hc => hYs.avoid y hy (List.mem_append_left _ hc)⟩
    have hw := TypeOfElabHM.weaken_env (env_pre := specs.map (RecSpec.rhsEntry [] []))
      (env_extra := accEnv) (env := ctxEnv) (hpoly (b, .poly σ) hq σ rfl Ys hYs')
    rw [hmap_len, ← hshift, List.append_assoc, Expr.shiftFrom_openTyVars] at hw
    exact hw
  · -- the empty-pool body env IS the RHS env (the mono-group trick)
    simp only [RecSpecs.bodyCtx]
    rw [List.map_congr_left (fun s' _ => RecSpec.bodyScheme_nil s')]

/-- The mixed Λ-outside nest types: each MONO wrapper generalises `genFilter G τᵢ`
    over the inner group projection via the committed close-back
    (`Infer.sound_letInAnn`); each POLY wrapper binds its declared scheme `σᵢ`
    directly, its cofinite `letIn` premise re-deriving the inner rule at the
    opened projection. Induction over the member stack `ms`, accumulating the
    outer wrapper schemes `acc` (whose length equals the shift depth `n-1-i`). -/
private theorem letRecElabNest_sound {ctx : Ctx} {Lp G : List Nat}
    {anns : List (Option PolyTy)} {bs : List Expr} {specs : List RecSpec}
    {body : Expr} {ρ : Ty} {n : Nat}
    (hn : n = bs.length)
    (hwf : RecSpecs.WF anns bs specs G)
    (hbnd : Expr.TyBvarBounded.RecGroup 0 anns bs)
    ...
    (hpoly : ∀ p ∈ bs.zip specs, ∀ σ, p.2 = RecSpec.poly σ →
      ∀ Ys, FreshNames Lp σ.paramCount Ys →
        TypeOfElabHM ⟨specs.map (RecSpec.rhsEntry [] []) ++ ctx.env, ctx.ctors⟩
          (p.1.openTyVars Ys) (σ.openVars Ys)) :
    ∀ (ms : List (Nat × RecSpec)) (acc : List PolyTy),
      (∀ M ∈ acc, ∀ g ∈ G, g ∉ M.body.freeVars) →
      acc.length + ms.length = n →
      (∀ k (hk : k < ms.length), (ms[k]'hk).1 = n - 1 - (acc.length + k) ∧
        specs[(ms[k]'hk).1]? = some (ms[k]'hk).2) →
      TypeOfElabHM ⟨(ms.map (fun p => RecSpec.bodyScheme G p.2)).reverse ++ acc ++ ctx.env,
          ctx.ctors⟩ body ρ →
      TypeOfElabHM ⟨acc ++ ctx.env, ctx.ctors⟩
        (Expr.letRecElabNest G anns n bs ms body) ρ := by
  intro ms
  induction ms with
  | nil =>
  ...
  | cons hd rest ih =>
    ...
    obtain ⟨i, s⟩ := hd
    have hcard1 : acc.length + (rest.length + 1) = n := by
      simpa only [List.length_cons] using hcard
    obtain ⟨hi_eq, hs_eq⟩ := hms 0 (by simp)
    simp only [List.getElem_cons_zero, Nat.add_zero] at hi_eq hs_eq
    have hs_mem : s ∈ specs := List.mem_of_getElem? hs_eq
    have hshift : n - 1 - i = acc.length := by omega
    have hms' : ∀ (M : PolyTy) (k : Nat) (hk : k < rest.length),
        (rest[k]'hk).1 = n - 1 - ((M :: acc).length + k) ∧
        specs[(rest[k]'hk).1]? = some (rest[k]'hk).2 := by
      intro M k hk
      have h := hms (k + 1) (by simp only [List.length_cons]; omega)
      simp only [List.getElem_cons_succ, List.length_cons] at h ⊢
      refine ⟨?_, h.2⟩
      rw [h.1]; congr 1; omega
    have hcard' : ∀ (M : PolyTy), (M :: acc).length + rest.length = n := by
      intro M; simp only [List.length_cons]; omega
    cases s with
    | mono τ =>
      have hτlc : τ.IsLC := hwf.mono_lc τ hs_mem
      simp only [Expr.letRecElabNest]
      refine Infer.sound_letInAnn
        (σ := PolyTy.genGroup G τ) (Ys := Ty.genFilter G τ)
        (rhsElab := .letRec anns (bs.map (·.shiftFrom n (n - 1 - i))) (.var i []))
        (bodyElab := Expr.letRecElabNest G anns n bs rest body)
        ?hrhs (PolyTy.genGroup_wf hτlc) (hwf.nodup.filter _) rfl ?hYsenv ?hYsσ ?hbodyM
      case hrhs =>
        have hopen : PolyTy.openVars (Ty.genFilter G τ) (PolyTy.genGroup G τ) = τ := by
          simp only [PolyTy.openVars, PolyTy.genGroup]
          exact Ty.openVars_closeOver_self hτlc
        rw [hopen]
        refine innerFusedLetRec_sound hwf.anns_eq hwf.length hwf.mono_lc hwf.poly_wf
          hn hshift hmono hpoly ?_
      ...
      case hYsenv =>
        ...
        · exact hacc_genG pt hpt y (Ty.mem_of_mem_genFilter hy) hypt
        · exact hG_env y (Ty.mem_of_mem_genFilter hy) (Env.mem_freeVars_iff.mpr ⟨pt, hpt, hypt⟩)
      case hYsσ =>
        intro y hy
    ...
    | poly σ =>
      have hσwf : σ.WF := hwf.poly_wf σ hs_mem
      have hanns_wf : ∀ σ', some σ' ∈ anns → σ'.WF := by
        intro σ' hσ'
        rw [← hwf.anns_eq] at hσ'
        obtain ⟨s', hs', hsa⟩ := List.mem_map.mp hσ'
        cases RecSpec.ann_eq_some hsa
        exact hwf.poly_wf σ' hs'
      have hbnd' := Expr.TyBvarBounded.RecGroup_map_shiftFrom
        (t := n) (k := n - 1 - i) hbnd
      have hi : i < specs.length := by
        by_contra hc
        rw [List.getElem?_eq_none (by omega)] at hs_eq
      ...
      refine TypeOfElabHM.letIn (M := σ) (L := Lp) hσwf
        (fun a h => by cases h; rfl) ?gen rfl ?hbodyP
      case gen =>
        intro Xs hXs
        simp only [Expr.openBoundTyVars]
        rw [Expr.letRec_proj_openTyVars hanns_wf hbnd' hXs.length]
        refine innerFusedLetRec_sound hwf.anns_eq hwf.length hwf.mono_lc hwf.poly_wf
          hn hshift hmono hpoly ?_
        refine TypeOfElabHM.var (polyTy := σ) ?_
          ⟨by rw [List.length_map, hXs.length], fun V hV => by
            obtain ⟨x, _, rfl⟩ := List.mem_map.mp hV; exact ContainsBvarsUpTo.fvar⟩
          (InstantiatesBy.openVars hσwf (Nat.le_of_eq hXs.length.symm))
        rw [List.getElem?_append_left (by rw [List.length_map]; exact hi),
            List.getElem?_map, hs_eq]
        rfl
      case hbodyP =>
        have hacc'_genG : ∀ M ∈ σ :: acc, ∀ g ∈ G, g ∉ M.body.freeVars := by
          intro M hM g hg
          rcases List.mem_cons.mp hM with rfl | hM
          · exact hG_specs g hg M hs_mem
          · exact hacc_genG M hM g hg
        have hbody' : TypeOfElabHM
            ⟨(rest.map (fun p => RecSpec.bodyScheme G p.2)).reverse
...
/-- **Λ-outside mixed-`letRec` elaboration soundness.** The fully-elaborated mixed
    group `Expr.letRecElab G anns bs specs body` types at `ρ` given the SOLVED-form
    RHS derivations — mono members at their solved monotypes and poly members
    scheme-relatively (skolems fresh for `Lp`), both in the identity RHS env
    `specs.map (rhsEntry [] [])` — and the body typing under the per-binding body
    schemes. ADJUSTED (Phase B, resolving the flagged packaging point): hypotheses
    are the solved/identity form over `bs` directly (what `InferRecGroup.sound`
    produces), not Core's pool-opened `RecSpecs.Mono/PolyTyped G`; the pool `G`
    premise re-emerges inside via the empty-pool inner rule. A new premise
    `hG_specs` (the pool avoids stored scheme bodies) is required by the poly
    wrappers — supplied by `RecGroup.rigidVars` at the call site. -/
theorem Expr.letRecElab_sound {ctx : Ctx} {Lp G : List Nat} {anns : List (Option PolyTy)}
    ...
    TypeOfElabHM ctx (Expr.letRecElab G anns bs specs body) ρ := by
  have hbnd_zip : ∀ p ∈ bs.zip specs, p.1.TyBvarBounded (RecAnn.params p.2.ann) := by
    ...
    | poly σ =>
      obtain ⟨Ys, hYlen, hYnodup, hYavoid⟩ := exists_fresh_names Lp σ.paramCount
      have hc := hpoly p hp σ hspec Ys ⟨hYlen, hYnodup, hYavoid⟩
      have hb := Expr.tyBvarBounded_of_openTyVarsAux Ys p.1 0
        (by simpa only [Expr.openTyVars] using TypeOfElabHM.tyBvarBounded hc)
      simpa only [RecSpec.ann, RecAnn.params, Nat.zero_add, hYlen] using hb
  ...
  rw [Expr.letRecElab]
  have hlen : bs.length = specs.length := hwf.length
  have hcard : ([] : List PolyTy).length + (((List.range bs.length).zip specs).reverse).length
      = bs.length := by
  ...
  have hbodyctx : TypeOfElabHM
      ⟨((((List.range bs.length).zip specs).reverse).map
            (fun p => RecSpec.bodyScheme G p.2)).reverse
          ++ ([] : List PolyTy) ++ ctx.env, ctx.ctors⟩ body ρ := by
    rw [List.map_reverse, List.reverse_reverse, List.append_nil]
    rw [show bs.length = specs.length from hlen, map_snd_range_zip]
    simpa only [RecSpecs.bodyCtx] using hbody
  have hvalid : ∀ k (hk : k < (((List.range bs.length).zip specs).reverse).length),
      ((((List.range bs.length).zip specs).reverse)[k]'hk).1
          = bs.length - 1 - (([] : List PolyTy).length + k) ∧
      specs[((((List.range bs.length).zip specs).reverse)[k]'hk).1]?
          = some ((((List.range bs.length).zip specs).reverse)[k]'hk).2 := by
    intro k hk
    have hlen_ms : (((List.range bs.length).zip specs).reverse).length = bs.length := by
      rw [List.length_reverse, List.length_zip, List.length_range, hlen, Nat.min_self]
    have hkn : k < bs.length := by rw [← hlen_ms]; exact hk
    have h1 : ((List.range bs.length).zip specs).length = bs.length := by
      rw [List.length_zip, List.length_range, hlen, Nat.min_self]
    have hidx : (((List.range bs.length).zip specs).reverse)[k]'hk
        = (bs.length - 1 - k, specs[bs.length - 1 - k]'(by omega)) := by
      simp only [List.getElem_reverse, List.getElem_zip, List.getElem_range, h1]
    rw [hidx]
    refine ⟨by simp only [List.length_nil, Nat.zero_add], ?_⟩
    simp only [List.getElem?_eq_getElem (by omega : bs.length - 1 - k < specs.length)]
  have key := letRecElabNest_sound (ctx := ctx) (Lp := Lp) (G := G) (n := bs.length)
    rfl hwf hbnd hG_env hG_specs hmono hpoly
...
/-- **Substitution removes its own domain from a term's free type vars.** If `w`
    avoids `S.onTy x` for every `x` (i.e. `w` is idempotently eliminated by `S`),
    then `w` is not free in `e.substTyFvars S` (for `letRecAnn`-free `e`). Direct
    structural twin of `Expr.mem_tyFreeVars_substTyFvars` that keeps the
    `(S.onTy t)` form for annotations instead of weakening to a disjunction — this
    is what makes `S₁` idempotent on the close-after-`S₁` `letIn` rhs. -/
theorem Expr.dom_notMem_substTyFvars {S : Subst} {w : Nat}
    (hidem : ∀ x : Ty, w ∉ (Ty.substFvars S x).freeVars) :
    ∀ {e : Expr}, w ∉ (e.substTyFvars S).tyFreeVars := by
  intro e
  induction e using Expr.rec_strong with
  ...
  | lambda ann body ih =>
    rw [Expr.substTyFvars_lambda]
    cases ann with
    | none => simpa only [Expr.tyFreeVars, Option.map_none, Option.elim_none, List.nil_append] using ih
    | some t =>
      simp only [Expr.tyFreeVars, Option.map_some, Option.elim_some, List.mem_append, not_or]
      exact ⟨hidem t, ih⟩
  | app f arg ihf iharg =>
    rw [Expr.substTyFvars_app]
    simp only [Expr.tyFreeVars, List.mem_append, not_or]
  ...
  | letIn ann rhs body ihr ihb =>
    ...
    | some σ =>
      simp only [Expr.tyFreeVars, Option.map_some, Option.elim_some, List.mem_append, not_or]
      exact ⟨⟨hidem σ.body, ihr⟩, ihb⟩
  | match_ scrut branches ihs ihbr =>
    rw [Expr.substTyFvars_match]
    simp only [Expr.tyFreeVars, List.mem_append, not_or]
    refine ⟨ihs, ?_⟩
    intro hc
    obtain ⟨pb, hpb, hwpb⟩ := Expr.mem_branchListTyFreeVars hc
    obtain ⟨pb0, hpb0, rfl⟩ := List.mem_map.mp hpb
    obtain ⟨p0, b0⟩ := pb0
    exact ihbr p0 b0 hpb0 hwpb
  | letRec anns bindings body ihbs ihb =>
    rw [Expr.substTyFvars_letRec]
    simp only [Expr.tyFreeVars, List.mem_append, not_or]
    refine ⟨⟨?_, ?_⟩, ihb⟩
    · induction anns with
      ...
      | cons a as iha =>
        cases a with
        | none =>
          simpa only [List.map_cons, RecAnn.substFvars_none, Expr.tyFreeVars.AnnList.tyFreeVars,
            Option.elim, List.nil_append] using iha
        | some σ =>
          simp only [List.map_cons, RecAnn.substFvars_some, Expr.tyFreeVars.AnnList.tyFreeVars,
            Option.elim, List.mem_append, not_or, PolyTy.body_substFvars]
          exact ⟨hidem σ.body, iha⟩
    · intro hc
      obtain ⟨e', he', hwe'⟩ := Expr.mem_recGroupTyFreeVars hc
      obtain ⟨e0, he0, rfl⟩ := List.mem_map.mp he'
      exact ihbs e0 he0 hwe'
...
`range_mem`. -/
mutual
theorem Infer.dom_avoid {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) → w ∉ e.tyFreeVars →
    w ∉ S.map Prod.fst := by
  cases h with
  ...
  | letRec hwfanns hgroup hbody =>
    ...
    have hctxg : ∀ M ∈ ((RecSpec.init Φ anns).map (RecSpec.rhsEntry [] []) ++ ctx.env),
      ...
      · obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hM
        ...
        exact hspecsA s hs
      · exact hctx M hM
    obtain ⟨hgS, hgo⟩ := InferRecGroup.eOut_avoid hgroup (w := w) (by omega) hctxg
      hspecsA hwe.1.2
...
decreasing_by
  all_goals (try subst_vars; try simp only [Expr.size, Expr.size_openTyVars]; omega)
theorem InferBranches.dom_avoid {Φ ctx scrutTy ρ brs Φ' S brsOut}
    (h : InferBranches Φ ctx scrutTy ρ brs Φ' S brsOut) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) → w ∉ scrutTy.freeVars →
    w ∉ ρ.freeVars → w ∉ Expr.tyFreeVars.BranchList.tyFreeVars brs →
    w ∉ S.map Prod.fst := by
...
theorem InferRecGroup.dom_avoid {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut) :
    ∀ {w : Nat}, w < Φ → (∀ M ∈ ctx.env, w ∉ M.body.freeVars) →
    (∀ s ∈ specs, w ∉ s.freeVars) → w ∉ Expr.tyFreeVars.RecGroup.tyFreeVars bindings →
    w ∉ S.map Prod.fst := by
  ...
  | consMono he huni hrest =>
    ...
    have hle1 := Infer.frontier_le he
    obtain ⟨heS, heτ, heo⟩ := Infer.eOut_avoid he (w := w) hwΦ hctx hbinds.1
    have hedom := Infer.dom_avoid he hwΦ hctx hbinds.1
    have hτA : w ∉ τ.freeVars := hspecs (.mono τ) List.mem_cons_self
    have hS₂ : ∀ p ∈ S₂, w ∉ p.2.freeVars := by
      intro p hp hwp
      rcases UnifyRel.range_mem huni p hp w hwp with h | h
    ...
    have hrdom := InferRecGroup.dom_avoid hrest (w := w) (by omega)
      (Subst.onCtx_avoid (Subst.onCtx_avoid hctx heS) hS₂)
      (by intro s' hs'
          obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hs'
          exact RecSpec.notMem_freeVars_onSubst
  ...
  | consPoly hΦN hinfer huni hesc1 hesc2 hrest =>
    ...
    have hwopen : w ∉ (e.openTyVars (freshVars N σ.paramCount)).tyFreeVars := by
      ...
      · have := freshVars_ge w h; omega
    obtain ⟨heS, heτ, heo⟩ := Infer.eOut_avoid hinfer (w := w) (by omega) hctx hwopen
    have hedom := Infer.dom_avoid hinfer (w := w) (by omega) hctx hwopen
    have hσopen : w ∉ (σ.openVars (freshVars N σ.paramCount)).freeVars := by
      intro hc
      rcases Ty.freeVars_openVars_subset w hc with h | h
      · exact hσbody h
  ...
  | letRec anns bs body ihbs ihb =>
    ...
    | cons hd tl ihtl =>
      ...
      rw [ihbs hd List.mem_cons_self thr n,
          ihtl (fun e' hm t' n' => ihbs e' (List.mem_cons_of_mem _ hm) t' n')]
...
/-- Distribution of a `G`-avoiding substitution over one mixed-nest level. The
    inner `.letRec anns …` node's anns thread by `RecAnn.substFvars`; a mono
    wrapper's generalised scheme commutes via `Subst.onPolyTy_genGroup` /
    `Ty.genFilter_onTy`; a poly wrapper's declared scheme is RIGID under
    `RecSpec.onSubst`, so the extra premise requires `S` to fix each stored
    poly-scheme body (the caller has this from the skolem-escape checks —
    ADJUSTED (Phase B): premise added, resolving the flagged design point). -/
theorem Expr.substTyFvars_letRecElabNest {S : Subst} {G : List Nat}
    {anns : List (Option PolyTy)} {n : Nat} {bs : List Expr}
    (hSG : ∀ p ∈ S, p.1 ∉ G) (hSran : ∀ p ∈ S, ∀ u ∈ p.2.freeVars, u ∉ G) :
    ∀ (L : List (Nat × RecSpec)) (body : Expr),
      (∀ p ∈ L, ∀ σ, p.2 = RecSpec.poly σ → ∀ q ∈ S, q.1 ∉ σ.body.freeVars) →
      (Expr.letRecElabNest G anns n bs L body).substTyFvars S
        = Expr.letRecElabNest G (anns.map (RecAnn.substFvars S)) n (bs.map (·.substTyFvars S))
            (L.map (fun p => (p.1, p.2.onSubst S))) (body.substTyFvars S) := by
  intro L
  induction L with
  | nil => intro body _; rfl
  | cons hd tl ih =>
    obtain ⟨i, s⟩ := hd
    intro body hpoly
    have hshift : (bs.map (·.shiftFrom n (n - 1 - i))).map (·.substTyFvars S)
        = (bs.map (·.substTyFvars S)).map (·.shiftFrom n (n - 1 - i)) := by
      simp only [List.map_map]
      exact List.map_congr_left (fun e _ => Expr.substTyFvars_shiftFrom e n (n - 1 - i))
    cases s with
    | mono τ =>
      have hGmem : ∀ x ∈ Ty.genFilter G τ, x ∈ G := fun x hx => by
        simp only [Ty.genFilter] at hx; exact List.mem_of_mem_filter hx
      have hSXs : ∀ q ∈ S, q.1 ∉ Ty.genFilter G τ := fun q hq hc => hSG q hq (hGmem _ hc)
      have hSranXs : ∀ q ∈ S, ∀ u ∈ q.2.freeVars, u ∉ Ty.genFilter G τ :=
        fun q hq u hu hc => hSran q hq u hu (hGmem _ hc)
      have hrhs : (Expr.closeTyVars (Ty.genFilter G τ)
            (.letRec anns (bs.map (·.shiftFrom n (n - 1 - i))) (.var i []))).substTyFvars S
          = Expr.closeTyVars (Ty.genFilter G (Subst.onTy S τ))
              (.letRec (anns.map (RecAnn.substFvars S))
                ((bs.map (·.substTyFvars S)).map (·.shiftFrom n (n - 1 - i))) (.var i [])) := by
        rw [Ty.genFilter_onTy hSG hSran, Expr.substTyFvars_closeTyVars hSXs hSranXs,
            Expr.substTyFvars_letRec, hshift, Expr.substTyFvars_var, List.map_nil]
      simp only [Expr.letRecElabNest, Expr.substTyFvars_letIn, Option.map_some, List.map_cons,
        RecSpec.onSubst]
      rw [ih body (fun p hp => hpoly p (List.mem_cons_of_mem _ hp)), hrhs,
          show (⟨(PolyTy.genGroup G τ).paramCount,
                Ty.substFvars S (PolyTy.genGroup G τ).body⟩ : PolyTy)
              = PolyTy.genGroup G (Subst.onTy S τ) from Subst.onPolyTy_genGroup hSG hSran]
      exact rfl
    | poly σ =>
      have hbfix : Ty.substFvars S σ.body = σ.body :=
        Ty.substFvars_eq_self_of_no_key
          (fun q hq hc => hpoly (i, .poly σ) List.mem_cons_self σ rfl q hq hc)
      have hta : (Ty.bvarRange σ.paramCount).map (Ty.substFvars S)
          = Ty.bvarRange σ.paramCount := by
        conv_rhs => rw [← List.map_id (Ty.bvarRange σ.paramCount)]
        refine List.map_congr_left (fun t ht => ?_)
        refine Ty.substFvars_eq_self_of_no_key (fun q hq hc => ?_)
        rw [Ty.bvarRangeFrom_freeVars_nil σ.paramCount 0 t ht] at hc
        exact absurd hc List.not_mem_nil
      have hrhs : (Expr.letRec anns (bs.map (·.shiftFrom n (n - 1 - i)))
            (.var i (Ty.bvarRange σ.paramCount))).substTyFvars S
          = Expr.letRec (anns.map (RecAnn.substFvars S))
              ((bs.map (·.substTyFvars S)).map (·.shiftFrom n (n - 1 - i)))
              (.var i (Ty.bvarRange σ.paramCount)) := by
        rw [Expr.substTyFvars_letRec, hshift, Expr.substTyFvars_var, hta]
      simp only [Expr.letRecElabNest, Expr.substTyFvars_letIn, Option.map_some, List.map_cons,
        RecSpec.onSubst]
      rw [ih body (fun p hp => hpoly p (List.mem_cons_of_mem _ hp)), hrhs,
          show (⟨σ.paramCount, Ty.substFvars S σ.body⟩ : PolyTy) = σ by
            rw [hbfix]]
      exact rfl

/-- Distribution of a `G`-avoiding substitution over the whole mixed `Λ`-nest
    (see `substTyFvars_letRecElabNest`; same added poly-scheme fixedness premise,
    now keyed by the specs list). -/
theorem Expr.substTyFvars_letRecElab {S : Subst} {G : List Nat} {anns : List (Option PolyTy)}
    {bs : List Expr} {specs : List RecSpec} {body : Expr}
    (hSG : ∀ p ∈ S, p.1 ∉ G) (hSran : ∀ p ∈ S, ∀ u ∈ p.2.freeVars, u ∉ G)
    (hSpoly : ∀ σ, RecSpec.poly σ ∈ specs → ∀ q ∈ S, q.1 ∉ σ.body.freeVars) :
    (Expr.letRecElab G anns bs specs body).substTyFvars S
      = Expr.letRecElab G (anns.map (RecAnn.substFvars S)) (bs.map (·.substTyFvars S))
          (specs.map (RecSpec.onSubst S)) (body.substTyFvars S) := by
  have hzip : ∀ {α β γ : Type} (f : β → γ) (l : List α) (r : List β),
      l.zip (r.map f) = (l.zip r).map (fun ab => (ab.1, f ab.2)) := by
    intro α β γ f l
    induction l with
    | nil => intro r; simp
    | cons hd tl ihl =>
      intro r; cases r with
      | nil => simp
      | cons rhd rtl => simp only [List.map_cons, List.zip_cons_cons, ihl]
  simp only [Expr.letRecElab, List.length_map]
  rw [Expr.substTyFvars_letRecElabNest hSG hSran _ body
      (fun p hp σ hpσ => hSpoly σ (by
        have := (List.of_mem_zip (List.mem_reverse.mp hp)).2
        exact hpσ ▸ this))]
  congr 1
...
Bridge: Infer elaboratum + monotype, after subst, projected by `eraseBounds` on
ctx schemes, term annotations (`Expr.eraseBounds`), and result monotype, is a
pure structural `TypeOfElabHM` derivation. Real dual-stack elaboratum keeps BL;
only the theorem erases.
-/

/-- Residual subject commute: erasing before/after a substitution agrees after a
    final erase (BL in the subst range is re-erased either way). -/
theorem Expr.eraseBounds_substTyFvars_comm (S : Subst) (e : Expr) :
    ((e.eraseBounds).substTyFvars S).eraseBounds = (e.substTyFvars S).eraseBounds := by
...
/-- Residual transport concluding at the honest elaboratum subject
    `(e.substTyFvars S).eraseBounds` (not `e.eraseBounds.substTyFvars S`). -/
theorem TypeOfElabHM.onSubst_eraseBounds_elab {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
    (h_lc : ∀ p ∈ S, p.2.IsLC)
    (h : TypeOfElabHM ctx.eraseBounds e.eraseBounds (Ty.eraseBounds τ)) :
    TypeOfElabHM (S.onCtx ctx).eraseBounds
      ((e.substTyFvars S).eraseBounds)
      (Ty.eraseBounds (S.onTy τ)) := by
  have key := TypeOfElabHM.onSubst_eraseBounds' S h_lc h
  rwa [Expr.eraseBounds_substTyFvars_comm] at key

/-- Residual fixed-term transport (annotations rigid under `S`). -/
theorem TypeOfElabHM.onSubst_eraseBounds_fixed {ctx : Ctx} {e : Expr} {τ : Ty} (S : Subst)
    (h_lc : ∀ p ∈ S, p.2.IsLC) (h_fix : e.substTyFvars S = e)
    (h : TypeOfElabHM ctx.eraseBounds e.eraseBounds (Ty.eraseBounds τ)) :
    TypeOfElabHM (S.onCtx ctx).eraseBounds e.eraseBounds (Ty.eraseBounds (S.onTy τ)) := by
  have key := TypeOfElabHM.onSubst_eraseBounds_elab S h_lc h
  rwa [h_fix] at key
...
theorem TypeOfElabHM.onSubst_eraseBounds_elab_append {ctx : Ctx} {e : Expr} {τ : Ty}
    ...
    TypeOfElabHM ((S₁ ++ S₂).onCtx ctx).eraseBounds
      ((e.substTyFvars (S₁ ++ S₂)).eraseBounds) (Ty.eraseBounds (S₂.onTy τ)) := by
  have key := TypeOfElabHM.onSubst_eraseBounds_elab (ctx := S₁.onCtx ctx)
    (e := e.substTyFvars S₁) (τ := τ) S₂ h₂ h
  simpa only [Subst.onCtx_append, Expr.substTyFvars_append] using key

private theorem Ctor.IsBoolCtor.typeOfElabHM_erase {ctx : Ctx} {name : CtorName} {c : Ctor}
    (hlook : LookupList.get? ctx.ctors name = some c) (hb : c.IsBoolCtor) :
    TypeOfElabHM ctx.eraseBounds (.ctor name) (.customTy ⟨"Bool"⟩ []) := by
  ...
  have hb' : (Ctor.eraseBounds c).IsBoolCtor := by
    -- Bool ctors are nullary with empty contents; erase is identity on that shape.
...
theorem Infer.sound {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
    ...
    TypeOfElabHM (S.onCtx ctx).eraseBounds
      ((eOut.substTyFvars S).eraseBounds) (Ty.eraseBounds τ) := by
  cases h with
  | primLitUnit =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds, Ty.eraseBounds_prim]
    exact .primLitUnit
  | primLitInt =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds, Ty.eraseBounds_prim]
  ...
  | primBinOpIntAdd =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds,
      Ty.eraseBounds_arrow, Ty.eraseBounds_prim]
    exact .primBinOpIntAdd
  | primBinOpIntSub =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds,
  ...
  | primBinOpIntLt hlookT hbT hlookF hbF =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds,
      Ty.eraseBounds_arrow, Ty.eraseBounds_prim, Ty.eraseBounds_customTy, TyList.eraseBounds_nil]
    exact .primBinOpIntLt (Ctor.IsBoolCtor.typeOfElabHM_erase hlookT hbT)
      (Ctor.IsBoolCtor.typeOfElabHM_erase hlookF hbF)
  | primBinOpCharLt hlookT hbT hlookF hbF =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds,
      Ty.eraseBounds_arrow, Ty.eraseBounds_prim, Ty.eraseBounds_customTy, TyList.eraseBounds_nil]
    exact .primBinOpCharLt (Ctor.IsBoolCtor.typeOfElabHM_erase hlookT hbT)
      (Ctor.IsBoolCtor.typeOfElabHM_erase hlookF hbF)
  | lambda hseed hbody =>
    intro hctx hbelow K hKΦ hKe hSK
    rw [Expr.substTyFvars_lambda, Expr.eraseBounds_lambda]
    cases hseed
    case none =>
  ...
  | app hf harg huni =>
    ...
    have hs3 := huni.lc (Subst.onTy_lc harg_s hf_lc) (.arrow harg_lc ContainsBvarsUpTo.fvar)
    have helim := Infer.eliminates hf hbelow (fun y hy => hKΦ y (hKe y (.inl hy)))
      (fun p hp hc => hSK p (List.mem_append_left _ (List.mem_append_left _ hp)) (hKe p.1 (.inl hc)))
    have hargfix : argOut.substTyFvars S₁ = argOut := Infer.eOut_substTyFvars_eq harg
      (fun p hp => Infer.dom_below hf hbelow (fun y hy => hKΦ y (hKe y (.inl hy))) p hp)
      (fun p hp M hM => by
    ...
    have hf2 := TypeOfElabHM.onSubst_eraseBounds_elab_append S₁ S₂ hf_s harg_s hf_sound
    have hf3 := TypeOfElabHM.onSubst_eraseBounds_elab (ctx := (S₁ ++ S₂).onCtx ctx)
      (e := fOut.substTyFvars (S₁ ++ S₂)) (τ := S₂.onTy τf) S₃ hs3
    ...
    have hf_arr : TypeOfElabHM ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds
        ((fOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds)
      ...
      have hterm_eq :
          ((fOut.substTyFvars (S₁ ++ S₂)).substTyFvars S₃).eraseBounds =
            (fOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds := by
        rw [← Expr.substTyFvars_append]
      rwa [hctx_eq, hterm_eq] at key
    have harg3 := TypeOfElabHM.onSubst_eraseBounds_elab
      (ctx := S₂.onCtx (S₁.onCtx ctx)) (e := argOut.substTyFvars S₂) (τ := τa) S₃ hs3 harg_sound
    have hargterm : argOut.substTyFvars (S₁ ++ S₂ ++ S₃) =
        (argOut.substTyFvars S₂).substTyFvars S₃ := by
      rw [show S₁ ++ S₂ ++ S₃ = S₁ ++ (S₂ ++ S₃) from List.append_assoc ..,
        Expr.substTyFvars_append, hargfix, Expr.substTyFvars_append]
    have harg_arr : TypeOfElabHM ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds
        ((argOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds)
      ...
      have hterm_eq :
          ((argOut.substTyFvars S₂).substTyFvars S₃).eraseBounds =
            (argOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds := by
        rw [← hargterm]
      rwa [hctx_eq, hterm_eq] at harg3
    rw [Expr.substTyFvars_app, Expr.eraseBounds_app]
    change TypeOfElabHM ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds
      (.app ((fOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds)
        ((argOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds))
      (Ty.eraseBounds (S₃.onTy (.fvar Φ₂)))
    exact TypeOfElabHM.app hf_arr harg_arr
  | @var Φ ctx i tyArgsIn polyTy hlook =>
    intro hctx _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds]
    refine TypeOfElabHM.var (polyTy := PolyTy.eraseBounds polyTy) ?_
      ⟨by simp [freshVars_length, PolyTy.eraseBounds_paramCount], fun tyArg ht => ?_⟩ ?_
    · have hlook' := congrArg (Option.map PolyTy.eraseBounds) hlook
  ...
  | @ctor Φ ctx name ctor hlook =>
    intro _ _ _ _ _ _
    simp only [Subst.onCtx_nil, Expr.substTyFvars, Expr.eraseBounds]
    refine TypeOfElabHM.ctor (ctor := Ctor.eraseBounds ctor)
  ...
  | letIn hrhs hbody =>
    ...
    have hgenV_ge : ∀ g ∈ genV, Φ ≤ g := by
      ...
      exact (Infer.eOut_avoid hrhs (w := g) hlt hg_ctxenv hg_rigid).2.1 (hgenV_τ₁ g hg)
    ...
    have hbodyfix : bodyOut.substTyFvars S₁ = bodyOut :=
      Infer.eOut_substTyFvars_eq hbody
        (fun p hp => Infer.dom_below hrhs hbelow (fun y hy => hKΦ y (hKe y (.inl hy))) p hp)
        (fun p hp M hM => by
          rcases List.mem_cons.mp hM with rfl | hM
          · exact fun hc2 => helimR.2 p hp (Ty.freeVars_closeOver_subset hc2)
          · obtain ⟨M₀, _, rfl⟩ := List.mem_map.mp hM; exact helimR.1 p hp M₀.body)
        (fun p hp hc => hSK p (List.mem_append_left _ hp) (hKe p.1 (.inr hc)))
    have hS₁fixE : ∀ p ∈ S₁, p.1 ∉ (rhsOut.substTyFvars S₁).tyFreeVars :=
      fun p hp => Expr.dom_notMem_substTyFvars (fun x => helimR.1 p hp x)
    have hbound :
        ((rhsOut.substTyFvars S₁).closeTyVars genV).substTyFvars (S₁ ++ S₂)
          = (rhsOut.substTyFvars (S₁ ++ S₂)).closeTyVars genV := by
      rw [Expr.substTyFvars_append,
          Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars
            (fun p hp hc => hS₁fixE p hp (Expr.tyFreeVars_closeTyVars_subset hc)),
          Expr.substTyFvars_closeTyVars hS₂genV hS₂genVran,
    ...
    have hschemebody : S₂.onTy (Ty.closeOver genV τ₁) =
      ...
      rw [h1, ← hSτ₁]
    have hann : (S₁ ++ S₂).onTy (Ty.closeOver genV τ₁) =
        Ty.closeOver genV ((S₁ ++ S₂).onTy τ₁) := by
      rw [Subst.onTy_append,
        show S₁.onTy (Ty.closeOver genV τ₁) = Ty.closeOver genV τ₁ from
          Ty.substFvars_eq_self_of_no_key
            (fun p hp hc => helimR.2 p hp (Ty.freeVars_closeOver_subset hc)),
        hschemebody]
    ...
    have hr2' : TypeOfElabHM ((S₁ ++ S₂).onCtx ctx).eraseBounds
        ((rhsOut.substTyFvars (S₁ ++ S₂)).eraseBounds)
    ...
    have hbody_pack : TypeOfElabHM
        ...
        ((bodyOut.substTyFvars (S₁ ++ S₂)).eraseBounds) (Ty.eraseBounds τ) := by
      ...
      rw [hctx_eq, hschemeeq, ← hbodyterm] at key
      exact key
    have hgoal_e :
        ((Expr.letIn (some (genScheme rhs.tyFreeVars (S₁.onCtx ctx).env τ₁))
            ((rhsOut.substTyFvars S₁).closeTyVars genV) bodyOut).substTyFvars (S₁ ++ S₂))
        = Expr.letIn (some ⟨genV.length, Ty.closeOver genV ((S₁ ++ S₂).onTy τ₁)⟩)
    ...
    rw [hgoal_e]
    exact Infer.sound_letIn_erase (ctx := (S₁ ++ S₂).onCtx ctx)
      (rhsElab := rhsOut.substTyFvars (S₁ ++ S₂))
      (bodyElab := bodyOut.substTyFvars (S₁ ++ S₂))
      (τ₁ := (S₁ ++ S₂).onTy τ₁) (τ₂ := τ) (genV := genV)
      hr2'
      (Subst.onTy_lc (fun p hp => by
        rcases List.mem_append.mp hp with h | h
        · exact hrhs_s p h
        · exact hbody_s p h) hrhs_lc)
      (by simp only [genV]; exact genVars_nodup)
      hgenV_env' hbody_pack
  | letInAnn hσwf hΦN hrhs huni hesc1 hesc2 hbody =>
    intro hctx hbelow K hKΦ hKe hSK
    expose_names
    simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append] at hKe
    ...
    obtain ⟨hrhs_lc, hrhs_s⟩ := Infer.lc hrhs hctx
    have hΦ_rhs : ∀ y ∈ (rhs.openTyVars (freshVars N σ.paramCount)).tyFreeVars,
        y < N + σ.paramCount := by
      intro y hy
      rcases Expr.tyFreeVars_openTyVars hy with h | h
      · have := hKΦ y (hKe y (.inl (.inr h))); have := hΦN; omega
      · have := freshVars_lt y h; omega
    have hSe_rhs : ∀ p ∈ S₁, p.1 ∉ (rhs.openTyVars (freshVars N σ.paramCount)).tyFreeVars := by
      intro p hp hc
      rcases Expr.tyFreeVars_openTyVars hc with h | h
    ...
    have hbody_s := (Infer.lc hbody hbodyWF).2
    have hSlc : ∀ p ∈ S₁ ++ Schk ++ S₂, p.2.IsLC := by
      intro p hp
      ...
      · rcases List.mem_append.mp hp with hp | hp
        ...
        · exact hSchk_lc p hp
      · exact hbody_s p hp
    have helimR := Infer.eliminates hrhs hctx_pc hΦ_rhs hSe_rhs
    have hcross : ∀ p ∈ S₁, ∀ q ∈ Schk, p.1 ∉ q.2.freeVars := by
      intro p hp q hq hwq
      rcases UnifyRel.range_mem huni q hq p.1 hwq with h | h
      · exact helimR.2 p hp h
      · rcases Ty.freeVars_openVars_subset p.1 h with h | h
        · exact hSK p (List.mem_append_left _ (List.mem_append_left _ hp))
            (hKe p.1 (.inl (.inl h)))
        · exact hesc1 p.1 h (List.mem_map.mpr ⟨p, List.mem_append_left _ hp, rfl⟩)
    have hcombelim := Subst.eliminates_append helimR.1 (UnifyRel.eliminates huni) hcross
    ...
    have hSYs : ∀ p ∈ S₁ ++ Schk ++ S₂, p.1 ∉ Ys := by
      ...
      · exact hS₂Ys p hp
    have hRfixE : ∀ p ∈ S₁ ++ Schk, p.1 ∉ (rhsOut.substTyFvars (S₁ ++ Schk)).tyFreeVars :=
      fun p hp => Expr.dom_notMem_substTyFvars (fun x => hcombelim p hp x)
    ...
    have hbound :
        ((rhsOut.substTyFvars (S₁ ++ Schk)).closeTyVars Ys).substTyFvars (S₁ ++ Schk ++ S₂)
          = (rhsOut.substTyFvars (S₁ ++ Schk ++ S₂)).closeTyVars Ys := by
      rw [show (S₁ ++ Schk ++ S₂) = (S₁ ++ Schk) ++ S₂ from rfl, Expr.substTyFvars_append,
          Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars
    ...
    have hrhs_sound := Infer.sound hrhs hctx hctx_pc (K ++ Ys)
      ...
      (fun p hp hc => by
        ...
        · simp only [Ys] at h
          exact hesc1 p.1 h (List.mem_map.mpr ⟨p, List.mem_append_left _ hp, rfl⟩))
    have hr1 := TypeOfElabHM.onSubst_eraseBounds_elab_append S₁ Schk hrhs_s hSchk_lc hrhs_sound
    -- residual unifier: erase (Schk · τ₁) = erase (Schk · σ.openVars Ys) = erase (σ.openVars Ys)
    have hu : Ty.eraseBounds (Schk.onTy τ₁) = Ty.eraseBounds (Schk.onTy (σ.openVars Ys)) :=
      huni.unifies
    have hr1' : TypeOfElabHM ((S₁ ++ Schk).onCtx ctx).eraseBounds
        ((rhsOut.substTyFvars (S₁ ++ Schk)).eraseBounds)
        (Ty.eraseBounds (σ.openVars Ys)) := by
      have key := hr1
      rwa [hu, hSchk_fix] at key
    have hr2 := TypeOfElabHM.onSubst_eraseBounds_elab
      (ctx := (S₁ ++ Schk).onCtx ctx) (e := rhsOut.substTyFvars (S₁ ++ Schk))
      (τ := σ.openVars Ys) S₂ hbody_s hr1'
    have hr2' : TypeOfElabHM ((S₁ ++ Schk ++ S₂).onCtx ctx).eraseBounds
        ((rhsOut.substTyFvars (S₁ ++ Schk ++ S₂)).eraseBounds)
        (Ty.eraseBounds (σ.openVars Ys)) := by
      have key := hr2
      rw [hS₂fix] at key
      convert key using 1
    ...
    have hbodyfix : bodyOut.substTyFvars (S₁ ++ Schk) = bodyOut :=
      Infer.eOut_substTyFvars_eq hbody
        (fun p hp => by
          rcases List.mem_append.mp hp with hp | hp
          · exact Infer.dom_below hrhs hctx_pc hΦ_rhs p hp
          · rcases UnifyRel.dom_mem huni p hp with h | h
            · exact hr_τ.mem_lt p.1 h
            · exact hσopen.mem_lt p.1 h)
        (fun p hp M hM => by
          rcases List.mem_cons.mp hM with rfl | hM
          · exact fun hc => hSK p (List.mem_append_left _ hp) (hKe p.1 (.inl (.inl hc)))
          · rw [show (Schk.onCtx (S₁.onCtx ctx)) = (S₁ ++ Schk).onCtx ctx
                  from (Subst.onCtx_append S₁ Schk ctx).symm] at hM
            exact Subst.eliminates_onCtx (hcombelim p hp) M hM)
        (fun p hp hc => hSK p (List.mem_append_left _ hp) (hKe p.1 (.inr hc)))
    have hbodyterm : bodyOut.substTyFvars (S₁ ++ Schk ++ S₂) = bodyOut.substTyFvars S₂ := by
      rw [show (S₁ ++ Schk ++ S₂) = (S₁ ++ Schk) ++ S₂ from rfl, Expr.substTyFvars_append,
        hbodyfix]
    have hS₁Schk_σbody : (S₁ ++ Schk).onTy σ.body = σ.body :=
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => hSK p (List.mem_append_left _ hp)
        (hKe p.1 (.inl (.inl hc))))
    have hSσ : Subst.onPolyTy (S₁ ++ Schk) σ = σ := by
      simp only [Subst.onPolyTy, hS₁Schk_σbody]
    have hschemeeq : Subst.onPolyTy S₂ σ = Subst.onPolyTy (S₁ ++ Schk ++ S₂) σ := by
      rw [show (S₁ ++ Schk ++ S₂) = (S₁ ++ Schk) ++ S₂ from rfl, Subst.onPolyTy_append, hSσ]
    have heqbodyctx : Subst.onCtx S₂ { (Schk.onCtx (S₁.onCtx ctx)) with
          env := σ :: (Schk.onCtx (S₁.onCtx ctx)).env }
        = { (S₁ ++ Schk ++ S₂).onCtx ctx with
          env := Subst.onPolyTy S₂ σ :: ((S₁ ++ Schk ++ S₂).onCtx ctx).env } := by
    ...
    have hgoal_e :
        ((Expr.letIn (some σ)
            ((rhsOut.substTyFvars (S₁ ++ Schk)).closeTyVars Ys) bodyOut).substTyFvars
          (S₁ ++ Schk ++ S₂))
        = Expr.letIn (some σ)
            ((rhsOut.substTyFvars (S₁ ++ Schk ++ S₂)).closeTyVars Ys)
            (bodyOut.substTyFvars (S₁ ++ Schk ++ S₂)) := by
      rw [Expr.substTyFvars_letIn]
      simp only [Option.map_some]
      refine congrArg₂ (fun a b => Expr.letIn a b
        (bodyOut.substTyFvars (S₁ ++ Schk ++ S₂))) ?_ hbound
      exact congrArg some hSσ_full
    rw [hgoal_e]
    refine Infer.sound_letInAnn_erase (ctx := (S₁ ++ Schk ++ S₂).onCtx ctx)
      (σ := σ) (rhsElab := rhsOut.substTyFvars (S₁ ++ Schk ++ S₂))
      (bodyElab := bodyOut.substTyFvars (S₁ ++ Schk ++ S₂))
      (Ys := Ys) (τ₂ := τ)
      hr2' hσwf (by simp only [Ys]; exact freshVars_nodup)
      (by simp only [Ys]; rw [freshVars_length])
      hYs_env hYs_σ hbody_pack
  | match_ hscrut hne hbr =>
    intro hctx hbelow K hKΦ hKe hSK
    expose_names
    simp only [Expr.tyFreeVars, List.mem_append] at hKe
    ...
    have hbrne : branchesOut ≠ [] := by
      ...
      | consWild => simp
    rw [Expr.substTyFvars_match]
    simp only [Expr.eraseBounds, List.map_map, Function.comp_def]
    -- Goal result is erase(S₂.onTy (.fvar Φ₁)); scrut residual is erase(S₂.onTy τs).
    -- InferBranches.sound yields the same residual scrut/ρ shapes under S₂.
    ...
    · intro br hbr_mem
      ...
      -- br = (pb.1, (pb.2.substTyFvars (S₁++S₂)).eraseBounds)
      ...
      have hctx_eq : (S₂.onCtx (S₁.onCtx ctx)).eraseBounds =
        ...
        rw [← Subst.onCtx_append]
      -- only ctx/term need packing; residual types already match match_ premises
      rw [← hctx_eq, hterm]
      exact hbrp
  | letRec hwfanns hgroup hbody =>
    intro hctx hbelow K hKΦ hKe hSK
    ...
    obtain ⟨hmonoS, L₀, hpolyS⟩ := InferRecGroup.sound hgroup hctxgWF hctxgBelow
    ...
    obtain ⟨G, hG⟩ : ∃ g, g = genGroupVars (RecGroup.rigidVars anns bindings) (S₁.onCtx ctx).env
        (RecSpecs.monoTys ((RecSpec.init Φ anns).map (RecSpec.onSubst S₁))) := ⟨_, rfl⟩
    rw [← hG] at hbody ⊢
    have hsolvedLC : ∀ s ∈ (RecSpec.init Φ anns).map (RecSpec.onSubst S₁), s.LC := by
      intro s' hs'
      obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hs'
    ...
    have hG_ge : ∀ g ∈ G, Φ ≤ g := by
      ...
      have hgrange := (InferRecGroup.eOut_avoid hgroup (w := g) (by omega)
        (by intro M hM
            rcases List.mem_append.mp hM with hM | hM
            · obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hM
              rw [RecSpec.rhsEntry_nil_body_freeVars]
    ...
    have hS₂Gran : ∀ p ∈ S₂, ∀ u ∈ p.2.freeVars, u ∉ G := fun p hp u hu hc =>
      (Infer.eOut_avoid hbody (w := u) (hG_lt u hc) (hG_bodyCtx u hc) (hG_body u hc)).1 p hp hu
    ...
    have hEoutS₁ : (Expr.letRecElab G anns (bindingsOut.map (·.substTyFvars S₁))
      ...
      · obtain ⟨s, hs, rfl⟩ := List.mem_map.mp hs'
        ...
        | mono τ0 =>
          rcases RecSpec.mem_init hs with ⟨m, _, _, heq⟩ | ⟨σ0, _, heq⟩
          · cases heq
            exact helimG p hp (Ty.fvar m) hws'
          · exact absurd heq (by simp)
        | poly σ0 =>
          exact hS₁K p hp (hKsch σ0 (RecSpec.poly_mem_init hs) p.1 hws')
      · obtain ⟨e0, he0, rfl⟩ := List.mem_map.mp he'
        exact Expr.dom_notMem_substTyFvars (fun u => helimG p hp u) hwe'
      · exact (Infer.eOut_avoid hbody (w := p.1) (hS₁dom_below p hp) (hS₁bc p hp)
          (fun hcb => hS₁K p hp (hKe p.1 (.inr hcb)))).2.2 hpbody
    -- distribute `S₂` over the elaborated output
    have hannsfix : anns.map (RecAnn.substFvars S₂) = anns := by
      conv_rhs => rw [← List.map_id anns]
      refine List.map_congr_left (fun a ha => ?_)
      cases a with
      | none => exact RecAnn.substFvars_none S₂
      | some σ0 =>
        rw [RecAnn.substFvars_some]
        refine congrArg some ?_
        rw [PolyTy.substFvars_eq,
            Ty.substFvars_eq_self_of_no_key (fun p hp hc => hS₂K p hp (hKsch σ0 ha p.1 hc))]
    have hbmap : (bindingsOut.map (·.substTyFvars S₁)).map (·.substTyFvars S₂)
    ...
    set bsS : List Expr := bindingsOut.map (·.substTyFvars (S₁ ++ S₂)) with hbsS
    ...
    have hwfS : RecSpecs.WF anns bsS specsS G := by
      refine ⟨?_, ?_, ?_, ?_, ?_⟩
      ...
      · -- length
        simp only [bsS, specsS, specsS₁, init0, List.length_map]
        exact (InferRecGroup.bindingsOut_length hgroup).trans
          (InferRecGroup.length_eq hgroup)
      · -- nodup of gen-pool
    ...
    have hwfR := RecSpecs.WF.eraseBounds hwfS
    -- RHS ctx at S₁ (pre-erase form used by hmonoS / hpolyS)
    set rhsS₁ : Ctx :=
      { (S₁.onCtx ctx) with
          env := specsS₁.map (RecSpec.rhsEntry [] []) ++ (S₁.onCtx ctx).env } with hrhsS₁
    -- residual RHS after S₂
    set rhsS : Ctx :=
      { ((S₁ ++ S₂).onCtx ctx) with
          env := specsS.map (RecSpec.rhsEntry [] []) ++ ((S₁ ++ S₂).onCtx ctx).env } with hrhsS
    ...
    refine Expr.letRecElab_sound (ctx := ((S₁ ++ S₂).onCtx ctx).eraseBounds)
      ...
      (specs := specsS.map RecSpec.eraseBounds)
      (body := (bodyOut.substTyFvars S₂).eraseBounds)
      (ρ := Ty.eraseBounds τ)
      hwfR ?hG_env ?hG_specs ?hmono ?hpoly ?hbody
    · -- hG_env: G avoids residual env free vars
      intro g hg hc
      have hc' : g ∈ ((S₁ ++ S₂).onCtx ctx).env.freeVars :=
        (Env.mem_freeVars_eraseBounds _ g).mp (by simpa [Ctx.eraseBounds] using hc)
      rw [Subst.onCtx_append, Env.mem_freeVars_iff] at hc'
      obtain ⟨M, hM, hgM⟩ := hc'
      rw [show (S₂.onCtx (S₁.onCtx ctx)).env = (S₁.onCtx ctx).env.map (Subst.onPolyTy S₂)
            from rfl, List.mem_map] at hM
      obtain ⟨M₀, hM₀, rfl⟩ := hM
      exact Subst.notMemOnTy (fun p hp hgp => hS₂Gran p hp g hgp hg)
        (fun hc2 => hG_envS₁ g hg (Env.mem_freeVars_iff.mpr ⟨M₀, hM₀, hc2⟩)) hgM
    · -- hG_specs: G avoids residual poly scheme bodies
      intro g hg σ hσ
      obtain ⟨s, hs, heq⟩ := List.mem_map.mp hσ
      cases s with
      | mono _ => exact RecSpec.noConfusion heq
      | poly σ0 =>
        injection heq with hσσ
        have hσs : RecSpec.poly σ0 ∈ specsS₁ :=
          RecSpec.poly_mem_map_onSubst.mp (by simpa only [specsS] using hs)
        intro hc
        exact hG_anns g hg σ0 (hpoly_mem_anns σ0 hσs)
          ((Ty.mem_freeVars_eraseBounds σ0.body g).mp (by simpa [← hσσ, PolyTy.eraseBounds] using hc))
    · -- residual mono: peel erase/zip, transport hmonoS under S₂, align ctx
      intro p hp τm hτm
      obtain ⟨e₀, s₀, hmem₀, hpEq⟩ := List.mem_zip_map_eraseBounds hp
      subst hpEq
      cases s₀ with
      | mono τ₀ =>
        injection hτm with hττ
        subst hττ
        change TypeOfElabHM _ e₀.eraseBounds (Ty.eraseBounds τ₀)
        have hzip : bsS.zip specsS =
            (bindingsOut.zip specsS₁).map (fun ab =>
              (ab.1.substTyFvars (S₁ ++ S₂), RecSpec.onSubst S₂ ab.2)) := by
          simp only [bsS, specsS]
          rw [List.zip_map_left_eq', List.zip_map_right_eq', List.map_map]
          rfl
        rw [hzip] at hmem₀
        obtain ⟨pr, hmem_raw, heq⟩ := List.mem_map.mp hmem₀
        obtain ⟨he_eq, hs_eq⟩ := Prod.mk.inj heq
        -- heq : (subst pr.1, onSubst pr.2) = (e₀, mono τ₀)
        have he₀ : e₀ = pr.1.substTyFvars (S₁ ++ S₂) := he_eq.symm
        have hs : RecSpec.onSubst S₂ pr.2 = RecSpec.mono τ₀ := hs_eq
        rcases pr with ⟨e_raw, s_raw⟩
        dsimp only at he₀ hs hmem_raw
        rw [he₀]
        cases s_raw with
        | mono τS₁ =>
          have hτ0eq : τ₀ = S₂.onTy τS₁ := by
            simp only [RecSpec.onSubst] at hs
            injection hs with h; exact h.symm
          subst hτ0eq
          have h0 := hmonoS (e_raw, RecSpec.mono τS₁) hmem_raw τS₁ rfl
          have h1 := TypeOfElabHM.onSubst_eraseBounds_elab (ctx := rhsS₁)
            (e := e_raw.substTyFvars S₁) (τ := τS₁) S₂ hS₂lc h0
          have hctx_eq : (S₂.onCtx rhsS₁).eraseBounds =
              { env := (specsS.map RecSpec.eraseBounds).map (RecSpec.rhsEntry [] [])
                    ++ ((S₁ ++ S₂).onCtx ctx).eraseBounds.env
                , ctors := ((S₁ ++ S₂).onCtx ctx).eraseBounds.ctors } := by
            rw [hbridge2']
            simp only [rhsS, Ctx.eraseBounds, Env.eraseBounds, List.map_append, List.map_map]
            refine congrArg₂ Ctx.mk ?_ rfl
            refine congrArg₂ List.append ?_ rfl
            exact List.map_congr_left fun s _ => RecSpec.eraseBounds_rhsEntry [] [] s
          have hterm_eq :
              ((e_raw.substTyFvars S₁).substTyFvars S₂).eraseBounds =
                (e_raw.substTyFvars (S₁ ++ S₂)).eraseBounds := by
            rw [← Expr.substTyFvars_append]
          rw [← hctx_eq, ← hterm_eq]
          exact h1
        | poly _ => exact absurd hs (by simp [RecSpec.onSubst])
      | poly σp =>
        exact RecSpec.noConfusion (by
          change RecSpec.poly (PolyTy.eraseBounds σp) = RecSpec.mono τm
          exact hτm)
    · -- residual poly: peel, hpolyS at L₀, transport S₂ with Lp-fresh Ys
      ...
      | poly σ₀ =>
        injection hσ with hσσ
        subst hσσ
        change TypeOfElabHM _
          (e₀.eraseBounds.openTyVars Ys) ((PolyTy.eraseBounds σ₀).openVars Ys)
        have hzip : bsS.zip specsS =
            (bindingsOut.zip specsS₁).map (fun ab =>
              (ab.1.substTyFvars (S₁ ++ S₂), RecSpec.onSubst S₂ ab.2)) := by
          simp only [bsS, specsS]
        ...
        obtain ⟨he_eq, hs_eq⟩ := Prod.mk.inj heq
        have he₀ : e₀ = pr.1.substTyFvars (S₁ ++ S₂) := he_eq.symm
        have hs : RecSpec.onSubst S₂ pr.2 = RecSpec.poly σ₀ := hs_eq
        rcases pr with ⟨e_raw, s_raw⟩
        dsimp only at he₀ hs hmem_raw
        rw [he₀]
        cases s_raw with
        | mono _ => exact absurd hs (by simp [RecSpec.onSubst])
        | poly σ1 =>
          -- hs : poly σ1 = poly σ₀ after dsimp of onSubst
          injection hs with hσeq
          subst hσeq
          have hYs0 : FreshNames L₀ σ1.paramCount Ys :=
            ⟨by simpa [PolyTy.eraseBounds_paramCount] using hYs.length,
              hYs.nodup, fun x hx hc =>
              hYs.avoid x hx (List.mem_append_left _ hc)⟩
          have hS₂Ys : ∀ q ∈ S₂, q.1 ∉ Ys := fun q hq hc =>
            hYs.avoid q.1 hc (List.mem_append_right _
              (List.mem_map.mpr ⟨q, hq, rfl⟩))
          have hσ_mem_anns : some σ1 ∈ anns :=
            hpoly_mem_anns σ1 (List.of_mem_zip hmem_raw).2
          have h0 := hpolyS (e_raw, RecSpec.poly σ1) hmem_raw σ1 rfl Ys hYs0
          have h1 := TypeOfElabHM.onSubst_eraseBounds_elab (ctx := rhsS₁)
            (e := (e_raw.substTyFvars S₁).openTyVars Ys)
            (τ := σ1.openVars Ys) S₂ hS₂lc h0
          have hσopen_fix : S₂.onTy (σ1.openVars Ys) = σ1.openVars Ys :=
            Ty.substFvars_eq_self_of_no_key (fun q hq hc => by
              rcases Ty.freeVars_openVars_subset q.1 hc with hbody | hY
              · exact hS₂K q hq (hKsch σ1 hσ_mem_anns q.1 hbody)
              · exact hS₂Ys q hq hY)
          have hctx_eq : (S₂.onCtx rhsS₁).eraseBounds =
              { env := (specsS.map RecSpec.eraseBounds).map (RecSpec.rhsEntry [] [])
                    ++ ((S₁ ++ S₂).onCtx ctx).eraseBounds.env
            ...
            exact List.map_congr_left fun s _ => RecSpec.eraseBounds_rhsEntry [] [] s
          have hterm_eq :
              (((e_raw.substTyFvars S₁).openTyVars Ys).substTyFvars S₂).eraseBounds =
                ((e_raw.substTyFvars (S₁ ++ S₂)).openTyVars Ys).eraseBounds := by
            rw [← Expr.substTyFvars_openTyVars hS₂lc hS₂Ys,
                ← Expr.substTyFvars_append]
          rw [← Expr.eraseBounds_openTyVars, ← PolyTy.eraseBounds_openVars,
              ← hterm_eq, ← hσopen_fix, ← hctx_eq]
          exact h1
    · -- residual body
      rw [← RecSpecs.bodyCtx_eraseBounds]
      convert hbodysound using 1
      · exact congrArg Ctx.eraseBounds hbridge_body.symm
...
theorem InferBranches.sound {Φ ctx scrutTy ρ brs Φ' S brsOut}
    ...
    (hSK : ∀ p ∈ S, p.1 ∉ K) :
    ∀ p ∈ brsOut,
      TypeOfElabMatchBranch (S.onCtx ctx).eraseBounds
        (p.1, (p.2.substTyFvars S).eraseBounds)
  ...
  | cons hlook hn huni0 hbody huni hrest =>
    ...
    -- belowFvars + eliminates scaffolding (mirrors InferBranches.eliminates)
    have hS₀bel : ∀ p ∈ S₀, Ty.BelowFvars (Φ + ctor.paramCount) p.2 :=
    ...
    have hS₀dom : ∀ p ∈ S₀, p.1 < Φ + ctor.paramCount := by
      intro p hp; rcases UnifyRel.dom_mem huni0 p hp with h | h
      · exact (hscrutB.mono (show Φ ≤ Φ + ctor.paramCount by omega)).mem_lt p.1 h
      · simp only [Ty.freeVars] at h; rw [mem_TyList_freeVars] at h
        obtain ⟨t, ht, hgt⟩ := h; obtain ⟨x, hx, rfl⟩ := List.mem_map.mp ht
        simp only [Ty.freeVars, List.mem_singleton] at hgt
        have := freshVars_lt x hx; omega
    ...
    have hSbody : ∀ p ∈ S₁, p.1 ∉ body.tyFreeVars := fun p hp hc =>
      hSe p (List.mem_append_left _ (List.mem_append_left _ (List.mem_append_right _ hp)))
        (by simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append]; exact Or.inl hc)
    obtain ⟨hbE, hbR⟩ := Infer.eliminates hbody hbodyBelow
      (fun y hy => by have := hΦhead y hy; omega) hSbody
    obtain ⟨hb_τbel, hb_sbel⟩ := Infer.belowFvars hbody hbodyBelow
      (fun y hy => by have := hΦhead y hy; omega)
    have hb_dom := Infer.dom_below hbody hbodyBelow
      (fun y hy => by have := hΦhead y hy; omega)
    have hbranchAvoid : ∀ p ∈ S₀, ∀ M ∈ ({ (S₀.onCtx ctx) with
        env := (ctor.contents.map (Ty.openWith
            (((freshVars Φ ctor.paramCount).map (Ty.fvar ·)).map S₀.onTy))).map PolyTy.mkTrivial
      ...
      intro p hp M hM; rcases List.mem_append.mp hM with hM | hM
      · obtain ⟨t, ht, rfl⟩ := List.mem_map.mp hM
        obtain ⟨c0, hc0, rfl⟩ := List.mem_map.mp ht
        simp only [PolyTy.mkTrivial]
        refine Ty.not_mem_freeVars_openWith ?_ ((ctor.closed c0 hc0).not_mem_freeVars p.1)
        intro v hv
        obtain ⟨v0, hv0, rfl⟩ := List.mem_map.mp hv
        obtain ⟨x, hx, rfl⟩ := List.mem_map.mp hv0
        exact UnifyRel.eliminates huni0 p hp (Ty.fvar x)
    ...
    have heOutBody : ∀ p ∈ S₀, (∀ q ∈ S₁, p.1 ∉ q.2.freeVars) ∧ p.1 ∉ τb.freeVars ∧
        p.1 ∉ bodyOut.tyFreeVars := fun p hp =>
      Infer.eOut_avoid hbody (w := p.1) (hS₀dom p hp) (hbranchAvoid p hp)
        (fun hc => hSe p (List.mem_append_left _ (List.mem_append_left _
            (List.mem_append_left _ hp)))
          (by simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append]; exact Or.inl hc))
    have cross01 : ∀ p ∈ S₀, ∀ q ∈ S₁, p.1 ∉ q.2.freeVars :=
      fun p hp q hq => (heOutBody p hp).1 q hq
    have hE01 := Subst.eliminates_append (UnifyRel.eliminates huni0) hbE cross01
    have hS₀ρbel : Ty.BelowFvars Φ₁ (S₀.onTy ρ) :=
    ...
    -- prefix-fixes
    have hbodyfix : bodyOut.substTyFvars S₀ = bodyOut :=
      Infer.eOut_substTyFvars_eq hbody hS₀dom hbranchAvoid
        (fun p hp hc => hSe p (List.mem_append_left _ (List.mem_append_left _
            (List.mem_append_left _ hp)))
          (by simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append]; exact Or.inl hc))
    have hrestfix : ∀ q ∈ restOut, q.2.substTyFvars ((S₀ ++ S₁) ++ S₂) = q.2 := by
      intro q hq
      refine Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars (fun p hp hc => ?_)
      refine (InferBranches.eOut_avoid hrest (w := p.1)
        (by rcases List.mem_append.mp hp with hp01 | hp2
            · rcases List.mem_append.mp hp01 with hp0 | hp1
              · have := hS₀dom p hp0; omega
              · exact hb_dom p hp1
            · exact hS₂dom p hp2)
        (honCtx3 ▸ Subst.eliminates_onCtx (hE012 p hp))
        (happ3 scrutTy ▸ hE012 p hp scrutTy)
        (happ3 ρ ▸ hE012 p hp ρ)
    ...
    · -- head named branch: residual body + residual BranchCtorSpec
      ...
      have h1 := TypeOfElabHM.onSubst_eraseBounds_elab
        (ctx := S₁.onCtx bodyCtx) (e := bodyOut.substTyFvars S₁) (τ := τb)
        S₂ hS₂lc h0
      have huni_eq := huni.unifies
      simp only [Unifies, AgreesHM] at huni_eq
      have h1ρ : TypeOfElabHM (S₂.onCtx (S₁.onCtx bodyCtx)).eraseBounds
          (((bodyOut.substTyFvars S₁).substTyFvars S₂).eraseBounds)
          (Ty.eraseBounds (S₂.onTy (S₁.onTy (S₀.onTy ρ)))) := by
        rwa [huni_eq] at h1
      have h2 := TypeOfElabHM.onSubst_eraseBounds_elab
        (ctx := S₂.onCtx (S₁.onCtx bodyCtx))
        (e := (bodyOut.substTyFvars S₁).substTyFvars S₂)
        (τ := S₂.onTy (S₁.onTy (S₀.onTy ρ))) S₃ hS₃lc h1ρ
      set taFull : List Ty :=
      ...
      have hbb123 :
        ...
        -- openWith args fuse with taFull/instContents definitionally
        rw [step1, hb2, hb3]
      have hctx_pack :
        ...
        simp only [← Subst.onCtx_append, List.append_assoc]
      have hterm_pack :
          (((bodyOut.substTyFvars S₁).substTyFvars S₂).substTyFvars S₃).eraseBounds =
            (bodyOut.substTyFvars (S₀ ++ S₁ ++ S₂ ++ S₃)).eraseBounds := by
        have lhs : ((bodyOut.substTyFvars S₁).substTyFvars S₂).substTyFvars S₃ =
            bodyOut.substTyFvars (S₁ ++ S₂ ++ S₃) := by
          simp only [← Expr.substTyFvars_append, List.append_assoc]
        have rhs : bodyOut.substTyFvars (S₀ ++ S₁ ++ S₂ ++ S₃) =
            bodyOut.substTyFvars (S₁ ++ S₂ ++ S₃) := by
          rw [show S₀ ++ S₁ ++ S₂ ++ S₃ = S₀ ++ (S₁ ++ S₂ ++ S₃) from by
                simp only [List.append_assoc],
              Expr.substTyFvars_append, hbodyfix]
        rw [lhs, rhs]
      ...
      have hbody_final : TypeOfElabHM
          ...
          ((bodyOut.substTyFvars (S₀ ++ S₁ ++ S₂ ++ S₃)).eraseBounds)
          (Ty.eraseBounds ((S₀ ++ S₁ ++ S₂ ++ S₃).onTy ρ)) := by
        have key := h2
        rw [hbb123] at key
        ...
        rw [henv, hctors, hterm_pack, hτ_pack] at key
        exact key
      have hlook' :
      ...
      have hscrut' :
        ...
        have hr : Ty.eraseBounds ((S₁ ++ S₂ ++ S₃).onTy
          ...
          -- x : Nat from freshVars after unfolding ta0
      ...
      refine TypeOfElabMatchBranch.mk
        ...
        rfl hbody_final
    · -- rest branches
      have hterm : (p.2.substTyFvars (S₀ ++ S₁ ++ S₂ ++ S₃)).eraseBounds =
          (p.2.substTyFvars S₃).eraseBounds := by
        rw [show S₀ ++ S₁ ++ S₂ ++ S₃ = ((S₀ ++ S₁) ++ S₂) ++ S₃ from by
              simp only [List.append_assoc],
          Expr.substTyFvars_append, hrestfix p hp_rest]
      ...
      rw [← hctx_eq, ← hscrut_eq, ← hρ_eq, hterm]
      exact key
  | consWild hbody huni hrest =>
    expose_names
    have hΦ : ∀ y ∈ Expr.tyFreeVars.BranchList.tyFreeVars ((MatchPattern.wildcard, body) :: rest),
        y < Φ :=
      fun y hy => hKΦ y (hKbr y hy)
    have hSe : ∀ p ∈ S₁ ++ S₂ ++ S₃,
    ...
    have hSbody : ∀ p ∈ S₁, p.1 ∉ body.tyFreeVars := fun p hp hc =>
      hSe p (List.mem_append_left _ (List.mem_append_left _ hp))
        (by simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append]; exact Or.inl hc)
    obtain ⟨hbE, hbR⟩ := Infer.eliminates hbody hbelow hΦhead hSbody
    obtain ⟨hb_τbel, hb_sbel⟩ := Infer.belowFvars hbody hbelow hΦhead
    have hb_dom := Infer.dom_below hbody hbelow hΦhead
    ...
    have hρ'bel : Ty.BelowFvars Φ₁ (S₂.onTy (S₁.onTy ρ)) :=
      Subst.onTy_belowFvars hS₂bel hS₁ρbel
    have hrestfix : ∀ q ∈ restOut, q.2.substTyFvars (S₁ ++ S₂) = q.2 := by
      intro q hq
      refine Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars (fun p hp hc => ?_)
      refine (InferBranches.eOut_avoid hrest (w := p.1)
    ...
    · -- head wildcard: residual body typing, transport S₂ then S₃, unify ρ
      have h0 := Infer.sound hbody hctx hbelow K hKΦ
        (fun y hy => hKbr y (by
          simp only [Expr.tyFreeVars.BranchList.tyFreeVars, List.mem_append]; exact Or.inl hy))
        (fun p hp => hSK p (List.mem_append_left _ (List.mem_append_left _ hp)))
      have h1 := TypeOfElabHM.onSubst_eraseBounds_elab_append S₁ S₂ hS₁lc hS₂lc h0
      have huni_eq := huni.unifies
      simp only [Unifies, AgreesHM] at huni_eq
      have h1ρ : TypeOfElabHM ((S₁ ++ S₂).onCtx ctx).eraseBounds
          ((bodyOut.substTyFvars (S₁ ++ S₂)).eraseBounds)
          (Ty.eraseBounds (S₂.onTy (S₁.onTy ρ))) := by
        rwa [huni_eq] at h1
      have h2 := TypeOfElabHM.onSubst_eraseBounds_elab
        (ctx := (S₁ ++ S₂).onCtx ctx) (e := bodyOut.substTyFvars (S₁ ++ S₂))
        (τ := S₂.onTy (S₁.onTy ρ)) S₃ hS₃lc h1ρ
      have hctx_eq : (S₃.onCtx ((S₁ ++ S₂).onCtx ctx)).eraseBounds =
          ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds := by
        rw [← Subst.onCtx_append]
      have hterm_eq :
          ((bodyOut.substTyFvars (S₁ ++ S₂)).substTyFvars S₃).eraseBounds =
            (bodyOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds := by
        rw [← Expr.substTyFvars_append]
      have hτ_eq : Ty.eraseBounds (S₃.onTy (S₂.onTy (S₁.onTy ρ))) =
          Ty.eraseBounds ((S₁ ++ S₂ ++ S₃).onTy ρ) := by
        rw [Subst.onTy_append, Subst.onTy_append]
      have hfinal : TypeOfElabHM ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds
          ((bodyOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds)
          (Ty.eraseBounds ((S₁ ++ S₂ ++ S₃).onTy ρ)) := by
        rwa [hctx_eq, hterm_eq, hτ_eq] at h2
      exact TypeOfElabMatchBranch.wildcard hfinal
    · -- rest branches: S₁++S₂ fix + residual rest IH
      have hterm : (p.2.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds =
          (p.2.substTyFvars S₃).eraseBounds := by
        rw [Expr.substTyFvars_append, hrestfix p hp_rest]
      have hctx_eq : (S₃.onCtx (S₂.onCtx (S₁.onCtx ctx))).eraseBounds =
          ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds := by
        simp only [← Subst.onCtx_append, List.append_assoc]
...
theorem InferRecGroup.sound {Φ ctx bindings specs Φ' S bindingsOut}
    ...
    (hSK : ∀ p ∈ S, p.1 ∉ K) :
    (∀ p ∈ bindingsOut.zip (specs.map (RecSpec.onSubst S)), ∀ τ, p.2 = RecSpec.mono τ →
        TypeOfElabHM (S.onCtx ctx).eraseBounds
          ((p.1.substTyFvars S).eraseBounds) (Ty.eraseBounds τ))
      ∧ (∃ L : List Nat, ∀ p ∈ bindingsOut.zip (specs.map (RecSpec.onSubst S)),
          ∀ σ, p.2 = RecSpec.poly σ → ∀ Xs, FreshNames L σ.paramCount Xs →
            TypeOfElabHM (S.onCtx ctx).eraseBounds
              (((p.1.substTyFvars S).openTyVars Xs).eraseBounds)
  ...
  | consMono he huni hrest =>
    ...
    have hfix : ∀ b ∈ restOut, b.substTyFvars (S₁ ++ S₂) = b := by
      ...
      · intro s' hs' hcc
        have h1 := hspecs_env' s' hs' p.1 hcc
        obtain ⟨M, hM, hyM⟩ := Env.mem_freeVars_iff.mp h1
        rw [show (S₂.onCtx (S₁.onCtx ctx)) = (S₁ ++ S₂).onCtx ctx
              from (Subst.onCtx_append S₁ S₂ ctx).symm] at hM
        exact Subst.eliminates_onCtx (hS₁₂elim p hp) M hM hyM
      · rcases List.mem_append.mp hp with hp | hp
    ...
    · intro p hp τ0 hτ0
      ...
      · have hred : RecSpec.onSubst (S₁ ++ S₂ ++ S₃) (RecSpec.mono τ)
        ...
        have huni_eq := huni.unifies
        simp only [Unifies, AgreesHM] at huni_eq
        have h1' : TypeOfElabHM ((S₁ ++ S₂).onCtx ctx).eraseBounds
            ((eOut.substTyFvars (S₁ ++ S₂)).eraseBounds)
            (Ty.eraseBounds (S₂.onTy (S₁.onTy τ))) := by
          rwa [huni_eq] at h1
        have h2 := TypeOfElabHM.onSubst_eraseBounds_elab
          (ctx := (S₁ ++ S₂).onCtx ctx) (e := eOut.substTyFvars (S₁ ++ S₂))
          (τ := S₂.onTy (S₁.onTy τ)) S₃ hS₃lc h1'
        have hctx_eq : (S₃.onCtx ((S₁ ++ S₂).onCtx ctx)).eraseBounds =
            ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds := by
          rw [← Subst.onCtx_append]
        have hterm_eq :
            ((eOut.substTyFvars (S₁ ++ S₂)).substTyFvars S₃).eraseBounds =
              (eOut.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds := by
          rw [← Expr.substTyFvars_append]
        have hty_eq : Ty.eraseBounds (S₃.onTy (S₂.onTy (S₁.onTy τ))) =
            Ty.eraseBounds ((S₁ ++ S₂ ++ S₃).onTy τ) := by
          simp only [Subst.onTy_append]
        rwa [hctx_eq, hterm_eq, hty_eq] at h2
      · rw [hspecmap] at hp_rest
        have hb_in : p.1 ∈ restOut := (List.of_mem_zip hp_rest).1
        have hctx_eq : (S₃.onCtx (S₂.onCtx (S₁.onCtx ctx))).eraseBounds =
            ((S₁ ++ S₂ ++ S₃).onCtx ctx).eraseBounds := by
          simp only [← Subst.onCtx_append, List.append_assoc]
        have hterm_eq : (p.1.substTyFvars S₃).eraseBounds =
            (p.1.substTyFvars (S₁ ++ S₂ ++ S₃)).eraseBounds := by
          rw [show (S₁ ++ S₂ ++ S₃ : Subst) = (S₁ ++ S₂) ++ S₃ from rfl,
              Expr.substTyFvars_append, hfix p.1 hb_in]
    ...
    · intro p hp σ0 hσ0 Xs hXs
      ...
      · rw [hspecmap] at hp_rest
        ...
        have hterm_eq :
            ((p.1.substTyFvars S₃).openTyVars Xs).eraseBounds =
              ((p.1.substTyFvars (S₁ ++ S₂ ++ S₃)).openTyVars Xs).eraseBounds := by
          rw [show (S₁ ++ S₂ ++ S₃ : Subst) = (S₁ ++ S₂) ++ S₃ from rfl,
              Expr.substTyFvars_append, hfix p.1 hb_in]
  ...
  | consPoly hN he huni hesc1 hesc2 hrest =>
    ...
    have hΦ_rhs : ∀ y ∈ (Expr.openTyVars (freshVars N σ.paramCount) e).tyFreeVars,
        y < N + σ.paramCount := by
      intro y hy
      rcases Expr.tyFreeVars_openTyVars hy with h | h
    ...
    have hSe_rhs : ∀ p ∈ S₁, p.1 ∉ (Expr.openTyVars (freshVars N σ.paramCount) e).tyFreeVars := by
      intro p hp hc
      rcases Expr.tyFreeVars_openTyVars hc with h | h
    ...
    have helimR := Infer.eliminates he hctx_pc hΦ_rhs hSe_rhs
    have hcross : ∀ p ∈ S₁, ∀ q ∈ Schk, p.1 ∉ q.2.freeVars := by
      intro p hp q hq hwq
      rcases UnifyRel.range_mem huni q hq p.1 hwq with h | h
      · exact helimR.2 p hp h
      · rcases Ty.freeVars_openVars_subset p.1 h with h | h
        · exact hK1 p hp (hKsch σ List.mem_cons_self p.1 h)
        · exact hesc1 p.1 h (List.mem_map.mpr ⟨p, List.mem_append_left _ hp, rfl⟩)
    have hcombelim := Subst.eliminates_append helimR.1 (UnifyRel.eliminates huni) hcross
    set Ys := freshVars N σ.paramCount with hYs_def
    have hYs_lt : ∀ y ∈ Ys, y < Φ₁ :=
    ...
    have hYs_rest : ∀ y ∈ Ys,
        y ∉ Expr.tyFreeVars.RecGroup.tyFreeVars rest :=
      fun y hy hc => by
    ...
    have hS₂fix : S₂.onTy (σ.openVars Ys) = σ.openVars Ys :=
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
        ...
        · exact hS₂Ys p hp h)
    have hr1' : TypeOfElabHM ((S₁ ++ Schk).onCtx ctx).eraseBounds
        ((eOut.substTyFvars (S₁ ++ Schk)).eraseBounds)
        (Ty.eraseBounds (σ.openVars Ys)) := by
      have key := hr1
      rwa [hu, hSchk_fix] at key
    have hr2 := TypeOfElabHM.onSubst_eraseBounds_elab
      (ctx := (S₁ ++ Schk).onCtx ctx) (e := eOut.substTyFvars (S₁ ++ Schk))
      (τ := σ.openVars Ys) S₂ hS₂lc hr1'
    have hr2' : TypeOfElabHM ((S₁ ++ Schk ++ S₂).onCtx ctx).eraseBounds
        ((eOut.substTyFvars (S₁ ++ Schk ++ S₂)).eraseBounds)
        (Ty.eraseBounds (σ.openVars Ys)) := by
      have key := hr2
      rw [hS₂fix] at key
      convert key using 1
      · rw [← Subst.onCtx_append]
      · rw [← Expr.substTyFvars_append]
    have hRfixE : ∀ p ∈ S₁ ++ Schk, p.1 ∉ (eOut.substTyFvars (S₁ ++ Schk)).tyFreeVars :=
      fun p hp => Expr.dom_notMem_substTyFvars (fun x => hcombelim p hp x)
    have hbound : ((eOut.substTyFvars (S₁ ++ Schk)).closeTyVars Ys).substTyFvars
          (S₁ ++ Schk ++ S₂)
        = (eOut.substTyFvars (S₁ ++ Schk ++ S₂)).closeTyVars Ys := by
    ...
    have hYs_σ : ∀ y ∈ Ys, y ∉ σ.body.freeVars :=
      fun y hy hc => by
        ...
        simp only [Ys] at hy; have := freshVars_ge y hy; omega
    -- Residual closeBack: structural closeBack on erased subjects.
    have hr2_erased : TypeOfElabHM ((S₁ ++ Schk ++ S₂).onCtx ctx).eraseBounds
        ((eOut.substTyFvars (S₁ ++ Schk ++ S₂)).eraseBounds)
        ((PolyTy.eraseBounds σ).openVars Ys) := by
      have key := hr2'
      rwa [PolyTy.eraseBounds_openVars] at key
    have head_cof := TypeOfElabHM.closeBack_cofinite
      (ctx := ((S₁ ++ Schk ++ S₂).onCtx ctx).eraseBounds)
      (σ := PolyTy.eraseBounds σ)
      (rhsElab := (eOut.substTyFvars (S₁ ++ Schk ++ S₂)).eraseBounds)
      (Ys := Ys) hr2_erased
      (by simp only [Ys]; exact freshVars_nodup)
      (by simp only [Ys, PolyTy.eraseBounds_paramCount]; rw [freshVars_length])
    ...
    have hfix : ∀ b ∈ restOut, b.substTyFvars (S₁ ++ Schk ++ S₂) = b.substTyFvars S₂ := by
      ...
      refine (InferRecGroup.eOut_avoid hrest (w := p.1) ?_ ?_ ?_ ?_).2 b hb hc
      · rcases List.mem_append.mp hp with hp | hp
        · exact Infer.dom_below he hctx_pc hΦ_rhs p hp
        · rcases UnifyRel.dom_mem huni p hp with h | h
          · exact hr_τ.mem_lt p.1 h
          · exact hσopen.mem_lt p.1 h
      · intro M hM
      ...
      · intro s' hs' hcc
        ...
        rw [show (Schk.onCtx (S₁.onCtx ctx)) = (S₁ ++ Schk).onCtx ctx
              from (Subst.onCtx_append S₁ Schk ctx).symm] at hM
        exact Subst.eliminates_onCtx (hcombelim p hp) M hM hyM
      · exact fun hcc => (List.mem_append.mp hp).elim
          (fun h1 => hK1 p h1 (hKbr' p.1 hcc))
          (fun h2 => hKchk p h2 (hKbr' p.1 hcc))
    ...
    · intro p hp τ0 hτ0
      ...
      · rw [hspecmap] at hp_rest
        ...
        have hctx_eq : (S₂.onCtx (Schk.onCtx (S₁.onCtx ctx))).eraseBounds =
          ...
          rw [hctxbridge]
        have hterm_eq : (p.1.substTyFvars S₂).eraseBounds =
            (p.1.substTyFvars (S₁ ++ Schk ++ S₂)).eraseBounds := by
          rw [hfix p.1 hb_in]
        have htail := hmono_tail p hp_rest τ0 hτ0
        rwa [hctx_eq, hterm_eq] at htail
    · intro p hp σ0 hσ0 Xs hXs
      simp only [List.map_cons, List.zip_cons_cons, List.mem_cons] at hp
      rcases hp with rfl | hp_rest
      · -- head poly member: residual closeBack on erased open/close
        -- After rfl: p = (close Ys (subst (S₁++Schk) eOut), poly σ)
        have hσeq : σ0 = σ := by
          simp only [RecSpec.onSubst] at hσ0
          injection hσ0 with h
          exact h.symm
        have hfresh : FreshNames Ys (PolyTy.eraseBounds σ).paramCount Xs := by
          refine ⟨?_, hXs.nodup, fun x hx hc => hXs.avoid x hx (List.mem_append_left _ hc)⟩
          -- hXs.length : Xs.length = σ0.paramCount; rewrite via σ0 = σ
          simpa [PolyTy.eraseBounds_paramCount, hσeq] using hXs.length
        -- subject: subst of closed head = close of full subst (hbound), then open/close/erase
        change TypeOfElabHM _ ((((eOut.substTyFvars (S₁ ++ Schk)).closeTyVars Ys).substTyFvars
            (S₁ ++ Schk ++ S₂)).openTyVars Xs).eraseBounds (Ty.eraseBounds (σ0.openVars Xs))
        rw [hσeq, hbound, Expr.eraseBounds_openTyVars, Expr.eraseBounds_closeTyVars]
        -- residual type: erase(open σ) = open(erase σ)
        convert head_cof Xs hfresh using 1
        exact PolyTy.eraseBounds_openVars Xs σ
      · rw [hspecmap] at hp_rest
        have hb_in : p.1 ∈ restOut := (List.of_mem_zip hp_rest).1
        ...
        have hterm_eq :
            ((p.1.substTyFvars S₂).openTyVars Xs).eraseBounds =
              ((p.1.substTyFvars (S₁ ++ Schk ++ S₂)).openTyVars Xs).eraseBounds := by
          rw [hfix p.1 hb_in]
        rwa [hctx_eq, hterm_eq] at htail
termination_by Expr.sizeRecGroup bindings
decreasing_by
...
theorem Infer.sound_closed {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ)
...
residual TypeOf. Pipeline keeps anns intact for the bounds layer.

`Infer.lambda` / `letInAnn` copy the source annotation option **exactly** onto
`eOut` (before any later `substTyFvars S` on the whole elaboratum). Unannotated
`letIn none` is filled with a genScheme (not a user BL). This relation is the
structural zip of those facts — not residual erase.
-/

mutual
/-- Source and elaboratum agree on **user-facing binder ascriptions** (same
    `Option` payloads at λ / annotated let / letRec; recursive under structure).
    Unannotated `let` may gain a synthesised scheme on the elaboratum. -/
inductive Expr.UserAnnsCopied : Expr → Expr → Prop
  | primLit {p} :
    Expr.UserAnnsCopied (.primLit p) (.primLit p)
  | primBinOp {op} :
    Expr.UserAnnsCopied (.primBinOp op) (.primBinOp op)
  | ctor {c} :
    Expr.UserAnnsCopied (.ctor c) (.ctor c)
  | var {i tyArgsIn tyArgsOut} :
    Expr.UserAnnsCopied (.var i tyArgsIn) (.var i tyArgsOut)
  | lambda {ann body bodyOut} :
    Expr.UserAnnsCopied body bodyOut →
    Expr.UserAnnsCopied (.lambda ann body) (.lambda ann bodyOut)
  | app {f fOut a aOut} :
    Expr.UserAnnsCopied f fOut →
    Expr.UserAnnsCopied a aOut →
    Expr.UserAnnsCopied (.app f a) (.app fOut aOut)
  /-- Unannotated let: elaboratum installs a synthesised scheme. RHS may be
      `closeTyVars ∘ substTyFvars` of the inferred elaboratum (not a pure
      same-shape walk of `rhs`); only the body is zipped. -/
  | letIn_none {rhs body rhsOut bodyOut σ} :
    Expr.UserAnnsCopied body bodyOut →
    Expr.UserAnnsCopied (.letIn none rhs body) (.letIn (some σ) rhsOut bodyOut)
  /-- Annotated let: scheme ascription copied exactly; body zipped; RHS may be closed. -/
  | letIn_some {σ rhs body rhsOut bodyOut} :
    Expr.UserAnnsCopied body bodyOut →
    Expr.UserAnnsCopied (.letIn (some σ) rhs body) (.letIn (some σ) rhsOut bodyOut)
  | match_ {scrut scrutOut brs brsOut} :
...
inductive Expr.UserAnnsCopiedBranches :
    List (MatchPattern × Expr) → List (MatchPattern × Expr) → Prop
  | nil : Expr.UserAnnsCopiedBranches [] []
  | cons {p e eOut rest restOut} :
    Expr.UserAnnsCopied e eOut →
    Expr.UserAnnsCopiedBranches rest restOut →
    Expr.UserAnnsCopiedBranches ((p, e) :: rest) ((p, eOut) :: restOut)

/-- Binding list zip (raw `InferRecGroup` elaborata; pre outer `map substTyFvars`). -/
inductive Expr.UserAnnsCopiedList : List Expr → List Expr → Prop
  | nil : Expr.UserAnnsCopiedList [] []
  | cons {e eOut rest restOut} :
    Expr.UserAnnsCopied e eOut →
    Expr.UserAnnsCopiedList rest restOut →
    Expr.UserAnnsCopiedList (e :: rest) (eOut :: restOut)
end

/-! ### Ann preservation (`Infer.preservesAnns`)

Mutual with `InferBranches`. `letRec` elaboratum is `letRecElab` (not bare
`.letRec`); group-level anns + body are zipped by `UserAnnsCopied.letRec`.
Bindings of a rec group are not fully zipped at the `letRecElab` layer (they
may be `substTyFvars` / `closeTyVars` of raw group elaborata — same discipline
as let RHS). -/

mutual
/-- Infer's elaboratum copies user binder ascriptions (pre-outer-`substTyFvars`). -/
theorem Infer.preservesAnns {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
    Expr.UserAnnsCopied e eOut := by
  cases h with
  | primLitUnit => exact .primLit
  | primLitInt => exact .primLit
  | primLitNat => exact .primLit
  | primLitChar => exact .primLit
  | primBinOpIntAdd => exact .primBinOp
  | primBinOpIntSub => exact .primBinOp
  | primBinOpIntLt _ _ _ _ => exact .primBinOp
  | primBinOpCharLt _ _ _ _ => exact .primBinOp
  | lambda _ hbody => exact .lambda (Infer.preservesAnns hbody)
  | app hf harg _ => exact .app (Infer.preservesAnns hf) (Infer.preservesAnns harg)
  | var => exact .var
  | ctor => exact .ctor
  | letIn _ hbody => exact .letIn_none (Infer.preservesAnns hbody)
  ...
  | letRec _ _ hbody =>
    exact .letRec (Infer.preservesAnns hbody)
termination_by e.size
decreasing_by
  all_goals (try subst_vars; try simp only [Expr.size]; omega)

theorem InferBranches.preservesAnns {Φ ctx scrutTy ρ brs Φ' S brsOut}
    (h : InferBranches Φ ctx scrutTy ρ brs Φ' S brsOut) :
    Expr.UserAnnsCopiedBranches brs brsOut := by
  cases h with
  | nil => exact .nil
  | cons _ _ _ hbody _ hrest =>
    exact .cons (Infer.preservesAnns hbody) (InferBranches.preservesAnns hrest)
  | consWild hbody _ hrest =>
    exact .cons (Infer.preservesAnns hbody) (InferBranches.preservesAnns hrest)
termination_by Expr.sizeBranches brs
decreasing_by
...
theorem TypeOfHM.eraseBounds_of {ctx : Ctx} {e : Expr} {τ : Ty}
  ...
  | @lambda paramTy ann bodyCtx ctx body bodyTy hpc hann heq hbody ihbody =>
    ...
    simpa only [Ctx.eraseBounds, Env.eraseBounds_cons, PolyTy.eraseBounds_mkTrivial]
      using ihbody
  | @var dbl polyTy instArgs tyArgs ty ctx hlook htyargs hinst =>
    -- decoration-blind: stored tyArgs erased, instantiation via erased instArgs
    simp only [Expr.eraseBounds, Ctx.eraseBounds]
...
theorem Infer.sourceSound {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
  ...
  | letRec hwfanns hgroup hbody =>
    ...
    have hwfSrc : RecSpecs.WF anns bindings specsS G := by
      ...
      · -- poly_wf: schemes rigid, come from user anns
        ...
        have hσs : RecSpec.poly σ ∈ specsS₁ :=
          RecSpec.poly_mem_map_onSubst.mp (by simpa only [specsS] using hσ)
        exact hwfanns σ (hpoly_mem_anns σ hσs)
    have hwfR := RecSpecs.WF.eraseBounds hwfSrc
    -- RHS ctx at S₁ (pre-erase form used by hmonoS / hpolyS)
    set rhsS₁ : Ctx :=
      { (S₁.onCtx ctx) with
          env := specsS₁.map (RecSpec.rhsEntry [] []) ++ (S₁.onCtx ctx).env } with hrhsS₁
    ...
    -- Lp for poly: L₀ plus S₂ domain so FreshNames avoids S₂ keys (open/subst commute)
    set Lp : List Nat := L₀ ++ S₂.map (·.1) with hLp
    refine TypeOfHM.letRec_of_emptyPool (ctx := ((S₁ ++ S₂).onCtx ctx).eraseBounds)
      (Lp := Lp) (G := G)
      (anns := anns.map (Option.map PolyTy.eraseBounds))
      (bs := bindings.map Expr.eraseBounds)
      (specs := specsS.map RecSpec.eraseBounds)
      (body := body.eraseBounds)
      (ρ := Ty.eraseBounds τ)
      hwfR ?hG_env ?hG_specs ?hG_bs ?hmono ?hpoly ?hbody
    · -- hG_env: G avoids residual env free vars
      intro g hg hc
      have hc' : g ∈ ((S₁ ++ S₂).onCtx ctx).env.freeVars :=
        (Env.mem_freeVars_eraseBounds _ g).mp (by simpa [Ctx.eraseBounds] using hc)
    ...
    · -- hG_specs: G avoids residual poly scheme bodies
      ...
      | poly σ0 =>
        ...
        exact hG_anns g hg σ0 (hpoly_mem_anns σ0 hσs)
          ((Ty.mem_freeVars_eraseBounds σ0.body g).mp (by simpa [← hσσ, PolyTy.eraseBounds] using hc))
    · -- hG_bs: G avoids residual binding tyFreeVars (erased)
      intro g hg e he hc
      obtain ⟨e₀, he₀, rfl⟩ := List.mem_map.mp he
      exact hG_bind g hg (Expr.mem_recGroup_tyFreeVars.mpr
        (List.mem_flatMap.mpr ⟨e₀, he₀, (Expr.mem_tyFreeVars_eraseBounds e₀ g).mp hc⟩))
    · -- residual mono: peel erase/zip, transport hmonoS under S₂, align ctx
      intro p hp τm hτm
      obtain ⟨e₀, s₀, hmem₀, hpEq⟩ := List.mem_zip_map_eraseBounds hp
      subst hpEq
      ...
      | mono τ₀ =>
        injection hτm with hττ
        subst hττ
        change TypeOfHM _ e₀.eraseBounds (Ty.eraseBounds τ₀)
        have hzip : bindings.zip specsS =
            (bindings.zip specsS₁).map (fun ab => (ab.1, RecSpec.onSubst S₂ ab.2)) := by
          simp only [specsS]
          rw [List.zip_map_right_eq']
        rw [hzip] at hmem₀
        obtain ⟨pr, hmem_raw, heq⟩ := List.mem_map.mp hmem₀
        obtain ⟨he_eq, hs_eq⟩ := Prod.mk.inj heq
        have he₀ : e₀ = pr.1 := he_eq.symm
        have hs : RecSpec.onSubst S₂ pr.2 = RecSpec.mono τ₀ := hs_eq
        rcases pr with ⟨e_raw, s_raw⟩
        dsimp only at he₀ hs hmem_raw
        rw [he₀]
        ...
        | mono τS₁ =>
          ...
          subst hτ0eq
          have h0 := hmonoS (e_raw, RecSpec.mono τS₁) hmem_raw τS₁ rfl
          have hfix : e_raw.substTyFvars S₂ = e_raw :=
            Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars (fun p hp hc => by
              exact hS₂K p hp (hKbind p.1 (Expr.mem_recGroup_tyFreeVars.mpr
                (List.mem_flatMap.mpr ⟨e_raw, (List.of_mem_zip hmem_raw).1, hc⟩))))
          have h1 := TypeOfHM.onSubst_eraseBounds_fixed (ctx := rhsS₁)
            (e := e_raw) (τ := τS₁) S₂ hS₂lc hfix h0
          have hctx_eq : (S₂.onCtx rhsS₁).eraseBounds =
              { env := (specsS.map RecSpec.eraseBounds).map (RecSpec.rhsEntry [] [])
                    ++ ((S₁ ++ S₂).onCtx ctx).eraseBounds.env
                , ctors := ((S₁ ++ S₂).onCtx ctx).eraseBounds.ctors } := by
      ...
      | poly σ₀ =>
        ...
        | poly σ1 =>
          ...
          have hσ_mem_anns : some σ1 ∈ anns :=
            hpoly_mem_anns σ1 (List.of_mem_zip hmem_raw).2
          have h0 := hpolyS (e_raw, RecSpec.poly σ1) hmem_raw σ1 rfl Ys hYs0
          have hfix : (e_raw.openTyVars Ys).substTyFvars S₂ = e_raw.openTyVars Ys :=
            Expr.substTyFvars_eq_self_of_not_mem_tyFreeVars (fun q hq hc => by
              rcases Expr.tyFreeVars_openTyVars hc with h | h
              · exact hS₂K q hq (hKbind q.1 (Expr.mem_recGroup_tyFreeVars.mpr
                  (List.mem_flatMap.mpr ⟨e_raw, (List.of_mem_zip hmem_raw).1, h⟩)))
              · exact hS₂Ys q hq h)
          have h1 := TypeOfHM.onSubst_eraseBounds_fixed (ctx := rhsS₁)
            (e := e_raw.openTyVars Ys) (τ := σ1.openVars Ys) S₂ hS₂lc hfix h0
          have hσopen_fix : S₂.onTy (σ1.openVars Ys) = σ1.openVars Ys :=
            Ty.substFvars_eq_self_of_no_key (fun q hq hc => by
              rcases Ty.freeVars_openVars_subset q.1 hc with hbody | hY
              · exact hS₂K q hq (hKsch σ1 hσ_mem_anns q.1 hbody)
...
theorem InferRecGroup.sourceSound {Φ ctx bindings specs Φ' S bindingsOut}
    (h : InferRecGroup Φ ctx bindings specs Φ' S bindingsOut)
    (hctx : CtxWF ctx) (hbelow : CtxBelow Φ ctx)
    (hspecs : ∀ s ∈ specs, s.LC) (hspecsB : ∀ s ∈ specs, s.BelowFvars Φ)
    (hspecs_env : ∀ s ∈ specs, ∀ y ∈ s.freeVars, y ∈ ctx.env.freeVars)
...
/-- Path R principality at `e`: any **erase-normal** declarative typing of
    `e.eraseBounds` under an LC specialization of `ctx` factors through Infer's
    principal monotype. Completeness is not claimed for decoration-sensitive
    TypeOf on raw BL-carrying terms. -/
...
theorem genScheme_generalizes {rigid : List Nat} {env : Env} {τ₁ : Ty} {R : Subst} {M : PolyTy}
    {Xs : List Nat}
    (hτ₁ : τ₁.IsLC) (hR : ∀ p ∈ R, p.2.IsLC) (hMwf : M.WF)
    (hXnodup : Xs.Nodup) (hXlen : Xs.length = M.paramCount)
    (hXMbody : ∀ x ∈ Xs, x ∉ M.body.freeVars)
    (htyr : Ty.openVars Xs M.body = R.onTy τ₁)
    (hXM'' : ∀ x ∈ Xs, x ∉ (R.onPolyTy (genScheme rigid env τ₁)).body.freeVars) :
    (R.onPolyTy (genScheme rigid env τ₁)).Generalizes M :=
  closeOver_generalizes (g := genVars rigid env τ₁) hτ₁ hR hMwf hXnodup hXlen hXMbody htyr hXM''

/-- `PolyTy.genGroup Ginf τ₁` (transported by `R`) generalizes any `M` it factors.
    The `g := Ty.genFilter Ginf τ₁` instance of `closeOver_generalizes` — the
    `letRec` per-binding analogue of `genScheme_generalizes`. -/
theorem genGroup_generalizes {Ginf : List Nat} {τ₁ : Ty} {R : Subst} {M : PolyTy}
    {Xs : List Nat}
    (hτ₁ : τ₁.IsLC) (hR : ∀ p ∈ R, p.2.IsLC) (hMwf : M.WF)
    (hXnodup : Xs.Nodup) (hXlen : Xs.length = M.paramCount)
    (hXMbody : ∀ x ∈ Xs, x ∉ M.body.freeVars)
...
/-- A `zip`-renaming `Ws ↦ vs` (the `Ws` distinct, all fresh for the `vs`) sends
    the key `fvar w` (with `(w, v)` in the zip) to its partner `v`. The
    block-residual computation behind `exists_recgroup_residual`. -/
private theorem Subst.onTy_zip_fvar_get : ∀ {Ws : List Nat} {vs : List Ty} {w : Nat} {v : Ty},
...
/-- The `renameG`-flavoured group generalisation: the inferred per-binding scheme
    `R.onPolyTy (genGroup Ginf τinf)` is at least as general as the declarative
...
/-- **The `letRec` group residual.** Construct an LC residual `R₀` (fixing `K`,
    ...
    monotype-var block `[Φ, Φ+n)` to the chosen declarative opened monotypes `vs`.
    A proxy-block `[Φ,Φ+n) ↦ [W,W+n)` (fresh `W`) shields the block from `S₀`,
    then a `[W,W+n) ↦ vs` block realises the targets. -/
...
/-- **The `letRec` body retyping.** The body, declaratively typed under the group
    schemes `Ms` (pool `G`, monotypes `τs`), retypes under the algorithm's
    `genGroupSchemes` (transported by the group residual `R₁`): each inferred
    scheme `R₁.onPolyTy (genGroup Ginf τs_inf[j])` generalizes the declarative
    `Ms[j] = genGroup G τs[j]` (via `genGroup_generalizes_renameG`), so
    `weaken_schemes` retypes the body. Shared by the `complete'` and producing
    `completeAt` `letRec` cases. -/
theorem letRec_body_retype
    {Φ : Nat} {ctx : Ctx} {S₁ R₁ S₀ : Subst} {bindings : List Expr}
    {body : Expr} {τs : List Ty} {Ms : List PolyTy} {G Xs : List Nat} {τ₀ : Ty} {K : List Nat}
    (hlen : bindings.length = τs.length) (hlen2 : τs.length = Ms.length)
    (hlc : ∀ τ ∈ τs, τ.IsLC) (hG : G.Nodup)
    (hgen : ∀ p ∈ τs.zip Ms, p.2 = PolyTy.genGroup G p.1)
    (hXlen : Xs.length = G.length) (hXnodup : Xs.Nodup)
    (hXG : ∀ g ∈ G, g ∉ Xs)
    (hXτs : ∀ x ∈ Xs, x ∉ Ty.freeVarsList τs)
    (hXenv : ∀ x ∈ Xs, x ∉ (S₀.onCtx ctx).env.freeVars)
    (hXrigid : ∀ x ∈ Xs, x ∉ bindings.flatMap Expr.tyFreeVars)
    (hS₁lc : ∀ p ∈ S₁, p.2.IsLC) (hR₁lc : ∀ p ∈ R₁, p.2.IsLC)
    (hctxS₁ : R₁.onCtx (S₁.onCtx ctx) = S₀.onCtx ctx)
    (hKrigid : ∀ y ∈ bindings.flatMap Expr.tyFreeVars, y ∈ K)
    ...
    TypeOfHM (R₁.onCtx ⟨genGroupSchemes (bindings.flatMap Expr.tyFreeVars) (S₁.onCtx ctx).env
        ...
        (S₁.onCtx ctx).ctors⟩)
      body τ₀ := by
  set rigid := bindings.flatMap Expr.tyFreeVars with hrigid_def
  set envS₁ := (S₁.onCtx ctx).env with henvS₁_def
  set τs_inf := (freshVars Φ bindings.length).map (fun i => S₁.onTy (Ty.fvar i)) with hτsinf_def
  have hτsinf_len : τs_inf.length = τs.length := by
    rw [hτsinf_def, List.length_map, freshVars_length, hlen]
  have hτsinf_lc : ∀ t ∈ τs_inf, t.IsLC := by
    intro t ht; rw [hτsinf_def] at ht; obtain ⟨i, _, rfl⟩ := List.mem_map.mp ht
    exact Subst.onTy_lc hS₁lc ContainsBvarsUpTo.fvar
  -- the algorithm's group schemes pointwise generalize the declarative `Ms`
  have hforall : List.Forall₂ PolyTy.Generalizes
      (R₁.onEnv (genGroupSchemes rigid envS₁ τs_inf)) Ms := by
    apply List.forall₂_of_getElem
    · simp only [Subst.onEnv, genGroupSchemes, List.length_map, hτsinf_len, hlen2]
    · intro i h1 h2
      simp only [Subst.onEnv, genGroupSchemes, List.getElem_map]
      have hi_τs : i < τs.length := by
        simp only [Subst.onEnv, genGroupSchemes, List.length_map, hτsinf_len] at h1; exact h1
      have hi_Ms : i < Ms.length := by rw [← hlen2]; exact hi_τs
      have hi_inf : i < τs_inf.length := by rw [hτsinf_len]; exact hi_τs
      have hzipmem : (τs[i]'hi_τs, Ms[i]'hi_Ms) ∈ τs.zip Ms := by
        have hz : (τs.zip Ms)[i]'(by rw [List.length_zip]; omega) = (τs[i], Ms[i]) :=
          List.getElem_zip
        rw [← hz]; exact List.getElem_mem _
      have hMsi : Ms[i]'hi_Ms = PolyTy.genGroup G (τs[i]'hi_τs) := hgen _ hzipmem
      have hci : R₁.onTy (τs_inf[i]'hi_inf) = Ty.renameG G Xs (τs[i]'hi_τs) := by
        have h := congrArg (fun l => l[i]?) hconn
        simp only [List.getElem?_map] at h
        rw [List.getElem?_eq_getElem hi_inf, List.getElem?_eq_getElem hi_τs] at h
        simpa using h
      rw [hMsi]
      refine genGroup_generalizes_renameG (hτsinf_lc _ (List.getElem_mem _))
        (hlc _ (List.getElem_mem _)) hR₁lc hG hXlen hXnodup hXG
        (fun x hx hc => hXτs x hx (Ty.mem_freeVarsList_of_mem (List.getElem_mem _) hc)) hci ?_
      -- hXinf : the structural rigid/env-bound argument
      intro x hx hmem
      change x ∈ (R₁.onTy (Ty.closeOver (Ty.genFilter (genGroupVars rigid envS₁ τs_inf)
        (τs_inf[i]'hi_inf)) (τs_inf[i]'hi_inf))).freeVars at hmem
      obtain ⟨v, hv, hxv⟩ := Ty.mem_freeVars_onTy_iff.mp hmem
      have hvτinf : v ∈ (τs_inf[i]'hi_inf).freeVars := Ty.freeVars_closeOver_subset hv
      have hvGinf : v ∉ genGroupVars rigid envS₁ τs_inf := by
        intro hgi
        exact Ty.not_mem_closeOver_freeVars
          (by simp only [Ty.genFilter, List.mem_filter, decide_eq_true_eq]; exact ⟨hgi, hvτinf⟩) hv
      have hv_flist : v ∈ Ty.freeVarsList τs_inf :=
        Ty.mem_freeVarsList_of_mem (List.getElem_mem hi_inf) hvτinf
      ...
      · obtain ⟨pt, hpt, hvpt⟩ := Env.mem_freeVars_iff.mp henv
        have hx_onTy : x ∈ (R₁.onTy pt.body).freeVars := Ty.mem_freeVars_onTy_iff.mpr ⟨v, hvpt, hxv⟩
  ...
  have hctxeq : R₁.onCtx ⟨genGroupSchemes rigid envS₁ τs_inf ++ envS₁, (S₁.onCtx ctx).ctors⟩
      = ⟨R₁.onEnv (genGroupSchemes rigid envS₁ τs_inf) ++ (S₀.onCtx ctx).env, (S₀.onCtx ctx).ctors⟩ := by
...
/-- **The named-branch `customTy` unification dodge.** Given a residual `R`
    (LC, fixing `K`, below-`Φ`-friendly) under which `scrutTy` types as
    `customTy ctor.tyName tyArgs`, the algorithm's per-branch unification of
...
/-! ### Path R residual completeness (`complete'`)

Declarative hypothesis is erase-normal TypeOf on `e.eraseBounds`.
Bodies fenced for proof agents; do not revive weak-Pins structural recovery.
-/

/-- Path R: principality of a given Infer derivation against residual TypeOfHM. -/
...
def Expr.eraseVarTyArgs : Expr → Expr
  ...
  | .letRec anns bindings body =>
      .letRec anns (RecGroup.eraseVarTyArgs bindings) body.eraseVarTyArgs
...
theorem Expr.eraseVarTyArgs_substTyFvars {e : Expr} {S : Subst} :
  ...
  | letRec anns bindings body ihbs ihbo =>
...
theorem Expr.eraseVarTyArgs_openTyVarsAux {Xs : List Nat} : ∀ (d : Nat) (e : Expr),
  ...
  | d, .letRec anns bindings body => by
      simp only [Expr.openTyVarsAux, Expr.eraseVarTyArgs,
        RecGroup.eraseVarTyArgs_openTyVarsAux d anns bindings, Expr.eraseVarTyArgs_openTyVarsAux d body]
...
theorem RecGroup.eraseVarTyArgs_openTyVarsAux {Xs : List Nat} :
    ∀ (d : Nat) (anns : List (Option PolyTy)) (bs : List Expr),
      RecGroup.eraseVarTyArgs (RecGroup.openTyVarsAux d Xs anns bs)
        = RecGroup.openTyVarsAux d Xs anns (RecGroup.eraseVarTyArgs bs)
  | _, _, [] => rfl
  | d, [], e :: rest => by
  ...
  | d, a :: as, e :: rest => by
      simp only [RecGroup.openTyVarsAux, RecGroup.eraseVarTyArgs,
        Expr.eraseVarTyArgs_openTyVarsAux (d + RecAnn.params a) e,
        RecGroup.eraseVarTyArgs_openTyVarsAux d as rest]
end
...
/-- The **annotated** `letIn` principality core (rigidity-aware). Choose a skolem
    ...
    `K ∪ Ys` (so `S₁` avoids `Ys`); the residual `R₁` (which fixes `K ∪ Ys`) is a
    unifier of `τ₁` and `σ.openVars Ys`, so `complete_K` yields the MGU `Schk`
    avoiding `K ∪ Ys` (hesc1). hesc2 is direct: the factor `V` of `R₁` through
    `Schk` fixes each `Y`, so a leak would land in `(S₀.onCtx ctx).env`, which
    avoids `Ys` by freshness. -/
...
/-- Every expression satisfies the principality property `CompleteAt`, assembled
    from the per-form case lemmas by strong induction on `e.size` — the annotated
...
/-- Path R: principality from residual TypeOf on `e.eraseBounds`. -/
        ...
        | some _ =>
          ...
          | some ⟨(Φ₂, S₂, branchesOut), hbranches, hav₂⟩ =>
            some ⟨(Φ₂, S₁ ++ S₂, .match_ scrutOut branchesOut, S₂.onTy (.fvar Φ₁)),
                  ...
                  · exact hav₂ p h⟩
  | .letRec anns bindings body =>
      -- Fused mixed recursion: decidably check each ANNOTATED scheme is WF
      -- (`bvarsBelow`), build the positional `RecSpec.init Φ anns` (fresh monotype
      -- var per unannotated member, declared scheme per annotated one), thread the
...
/-- Thread check-and-elaborate through a FUSED recursion group, casing on the head
    `RecSpec`: `mono τ` members infer-and-unify against `S₁.onTy τ` (Damas–Milner),
    `poly σ` members are skolem-checked scheme-relatively (Henglein), each threading
    the remaining specs via `RecSpec.onSubst`. Merges the historical
    `inferRecGroupCore`/`inferRecGroupAnnCore`. -/
def inferRecGroupCore (K : List Nat) (Φ : Nat) (ctx : Ctx) (bindings : List Expr) (specs : List RecSpec) :
    Option { r : Nat × Subst × List Expr //
      InferRecGroup Φ ctx bindings specs r.1 r.2.1 r.2.2 ∧ (∀ p ∈ r.2.1, p.1 ∉ K) } :=
  ...
  | e :: rest, .mono τ :: specs' =>
      ...
      | some ⟨(Φ₁, S₁, eOut, τ'), he, hav₁⟩ =>
        ...
        | some ⟨S₂, huni, hav₂⟩ =>
          ...
          | none => none
          | some ⟨(Φ₂, S₃, restOut), hrest, hav₃⟩ =>
            some ⟨(Φ₂, S₁ ++ S₂ ++ S₃, eOut :: restOut), .consMono he huni hrest, by
              intro p hp; rcases List.mem_append.mp hp with h | h
              · rcases List.mem_append.mp h with h | h
                · exact hav₁ p h
                · exact hav₂ p h
...
def infer (Φ : Nat) (ctx : Ctx) (e : Expr) : Option (Nat × Subst × Expr × Ty) :=
...
/-- `infer` soundness: a returned `(Φ', S, eOut, τ)` is a genuine `Infer` derivation
    (immediate — `inferCore` carries it). -/
theorem infer_sound {Φ : Nat} {ctx : Ctx} {e : Expr} {Φ' : Nat} {S : Subst} {eOut : Expr} {τ : Ty}
    (h : infer Φ ctx e = some (Φ', S, eOut, τ)) : Infer Φ ctx e Φ' S eOut τ := by
  rw [infer] at h
...
theorem infer_isPrincipal {Φ : Nat} {ctx : Ctx} {e : Expr} {Φ' : Nat} {S : Subst} {eOut : Expr} {τ : Ty}
    (h : infer Φ ctx e = some (Φ', S, eOut, τ)) (hwf : CtxWF ctx) (hbelow : CtxBelow Φ ctx)
...
theorem UnifyRel.gap_avoid {lo hi : Nat} : {a b : Ty} → {S : Subst} → UnifyRel a b S →
  ...
  | _, _, _, .blList h, ha, hb => by
    exact UnifyRel.gap_avoid h
      (fun v hv => ha v (by simpa only [Ty.freeVars] using hv))
      (fun v hv => hb v (by
        simp only [Ty.freeVars] at *
        exact TyList.mem_freeVars_of_mem List.mem_cons_self hv))
  | _, _, _, .listBl h, ha, hb => by
    exact UnifyRel.gap_avoid h
      (fun v hv => ha v (by
        simp only [Ty.freeVars] at *
...
theorem Infer.gap_avoid {lo hi : Nat} {Φ ctx e Φ' S eOut τ} (h : Infer Φ ctx e Φ' S eOut τ) :
  ...
  | primBinOpCharLt _ _ _ _ => intro _ _ _; refine ⟨?_, by simp, by simp⟩; intro v hv; simp [Ty.freeVars, TyList.freeVars] at hv
  | lambda hseed hbody =>
    intro hhi hctx htfv
    cases hseed with
    | none =>
...
theorem InferBranches.gap_avoid {lo hi : Nat} {Φ ctx scrutTy ρ brs Φ' S brsOut}
    (h : InferBranches Φ ctx scrutTy ρ brs Φ' S brsOut) :
    hi ≤ Φ → (∀ M ∈ ctx.env, ∀ v ∈ M.body.freeVars, v < lo ∨ hi ≤ v) →
    (∀ v ∈ scrutTy.freeVars, v < lo ∨ hi ≤ v) → (∀ v ∈ ρ.freeVars, v < lo ∨ hi ≤ v) →
...
theorem Infer.letInAnn_block_fresh {Φ : Nat} {ctx : Ctx} {σ : PolyTy} {rhs body : Expr}
    ...
    (h : Infer Φ ctx (.letIn (some σ) rhs body) Φ' S eOut τ)
    (hbelow : CtxBelow Φ ctx)
  ...
  cases h with
  | @letInAnn _ N _ _ _ _ Φ₁ Φ₂ S₁ Schk S₂ _ _ τ₁ τ₂ hσwf hΦN hrhs huni hesc1 hesc2 hbody =>
    simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append] at htfv
...
def InferCoreComplete (e : Expr) : Prop :=
  ∀ {Φ : Nat} {ctx : Ctx} {Φ' : Nat} {S : Subst} {eOut : Expr} {τ : Ty} (K : List Nat),
    CtxWF ctx → CtxBelow Φ ctx → (∀ k ∈ K, k < Φ) → (∀ y ∈ e.tyFreeVars, y ∈ K) →
...
theorem inferCore_complete_lambda {ann : Option Ty} {body : Expr} (ih : InferCoreComplete body) :
    InferCoreComplete (.lambda ann body) := by
  ...
  | lambda hseed hbody =>
    ...
    case none =>
      ...
      rw [inferCore]
      split
      · rename_i heq; rw [heq] at hsome; simp at hsome
      · rfl
    case some hcl =>
      expose_names
      simp only [Expr.tyFreeVars, Option.elim_some, List.mem_append] at hKe
      have hwf' : CtxWF { ctx with env := PolyTy.mkTrivial paramTy :: ctx.env } := by
        intro M hM; rcases List.mem_cons.mp hM with rfl | hM
        · exact hcl
        · exact hwf M hM
      have hbelow' : CtxBelow Φ { ctx with env := PolyTy.mkTrivial paramTy :: ctx.env } := by
        intro M hM; rcases List.mem_cons.mp hM with rfl | hM
        · exact Ty.BelowFvars.of_freeVars_lt (fun v hv => hKΦ v (hKe v (.inl hv)))
        · exact hbelow M hM
      have hsome := ih K hwf' hbelow' hKΦ (fun y hy => hKe y (.inr hy)) hSK hbody
      rw [inferCore, dif_pos ((Ty.bvarsBelow_iff paramTy).mpr hcl)]
      split
      · rename_i heq; rw [heq] at hsome; simp at hsome
      · rfl
...
theorem exists_app_unifier {A τa τ₀ argTy : Ty} {Φ₂ : Nat} {R₂ : Subst} {K : List Nat}
  ...
  refine ⟨U, Unifies.of_eq (by rw [hUniL, hUniR]), ?_, ?_⟩
  · rw [hUdef]
    intro p hp
    rcases List.mem_append.mp hp with hp' | hp'
    · rcases List.mem_append.mp hp' with hp'' | hp''
...
theorem inferCore_complete_app {f arg : Expr}
  ...
  exact sorry -- PhaseC-C2: InferCoreComplete needs complete' over erase-normal sourceSound
...
def InferBranchesCoreComplete (branches : List (MatchPattern × Expr)) : Prop :=
  ∀ {Φ : Nat} {ctx : Ctx} {scrutTy : Ty} {ρ : Ty} {Φ' : Nat} {S : Subst}
    {brsOut : List (MatchPattern × Expr)} (K : List Nat),
    ...
    (∀ p ∈ S, p.1 ∉ K) →
    InferBranches Φ ctx scrutTy ρ branches Φ' S brsOut →
    (inferBranchesCore K Φ ctx scrutTy ρ branches).isSome
...
theorem infer_complete {Φ : Nat} {ctx : Ctx} {e : Expr} {Φ' : Nat} {S : Subst} {eOut : Expr} {τ : Ty}
    (h : Infer Φ ctx e Φ' S eOut τ) (hwf : CtxWF ctx) (hbelow : CtxBelow Φ ctx)
    (hclosed : e.tyFreeVars = []) :
...
theorem infer_iff {Φ : Nat} {ctx : Ctx} {e : Expr}
    (hwf : CtxWF ctx) (hbelow : CtxBelow Φ ctx) (hclosed : e.tyFreeVars = []) :
    (infer Φ ctx e).isSome ↔ ∃ Φ' S eOut τ, Infer Φ ctx e Φ' S eOut τ := by
  constructor
  · intro h
    obtain ⟨⟨Φ', S, eOut, τ⟩, he⟩ := Option.isSome_iff_exists.mp h
    exact ⟨Φ', S, eOut, τ, infer_sound he⟩
  · rintro ⟨Φ', S, eOut, τ, hInfer⟩
    exact infer_complete hInfer hwf hbelow hclosed

/-- **The executable typechecker decides residual typeability.** `infer` succeeds
    iff `e` has an erase-normal declarative type under some LC specialization of
    `ctx` — the full-circle bridge from the executable to residual `TypeOfHM`. -/
...
def principalType (ctors : CtorEnv) (e : Expr) : Option Ty :=
  (inferCore e.tyFreeVars e.freshFloor ⟨[], ctors⟩ e).map (·.val.2.2.2)

/-- Monotype soundness (erase-normal): a computed principal type is a genuine
    residual declarative type (ctx ctors projected through `CtorEnv.eraseBounds`).
...
theorem principalType_sound {ctors : CtorEnv} {e : Expr} {τ : Ty}
  ...
  · simp only [Option.map_some, Option.some.injEq] at h
    subst h
    -- empty env: (S.onCtx empty).eraseBounds has empty env + erased ctors
    simpa [Subst.onCtx, Subst.onEnv, Ctx.eraseBounds, Env.eraseBounds] using
      Infer.sourceSound hInfer CtxWF.empty (fun M hM => by simp at hM)
...
theorem principalType_iff {ctors : CtorEnv} {e : Expr} :
  ...
  · intro h
    ...
    exact ⟨Ty.eraseBounds τ, principalType_sound hτ⟩
  · rintro ⟨τ, hτ⟩
    exact sorry -- PathR: completeAt from erase-normal TypeOfHM on e.eraseBounds
...
theorem typecheck_closed {ctors : CtorEnv} {e : Expr} {σ : PolyTy}
  ...
  rcases hi : inferCore e.tyFreeVars e.freshFloor ⟨[], ctors⟩ e with _ | ⟨⟨Φ', S, eOut, τ⟩, hInfer, hSK⟩ <;>
    rw [hi] at h
  · simp at h
  · simp only [Option.map_some, Option.some.injEq] at h
    subst h
...
/-- Whole-program soundness (Path R residual): successful `typecheck` generalizes
    a pure-HM type of the erase-projected program. -/
theorem typecheck_sound {ctors : CtorEnv} {e : Expr} {σ : PolyTy}
    (h : typecheck ctors e = some σ) :
    ∃ τ, TypeOfHM ⟨[], ctors.eraseBounds⟩ e.eraseBounds (Ty.eraseBounds τ) ∧
      σ = genScheme [] [] τ := by
  rw [typecheck] at h
...
/-- Whole-program principality (Path R residual typing of the principal monotype). -/
...
/-- Whole-program decidability (Path R): `typecheck` succeeds iff erase-projection
    is declaratively typeable. -/
