...
private def realUniversal (withMatch : Bool := false) : Except String Bool := do
  ...
  let used := RecursiveHMUniversal.atScopedNode node cert countInstance [arg]
  ...
  pure (used.actual.pretty == expected.pretty &&
...
private def cases : List (String × Bool) := [
  ("real annotated forall RHS universally specializes the original node and full caller intervals", match realUniversal with
    | .ok ok => ok | .error _ => false),
  ("real annotated lexical List-match universally specializes every actual branch derivation", match realUniversal true with
    | .ok ok => ok | .error _ => false)]
