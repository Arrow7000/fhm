...
def Ty.bvarsBelow (n : Nat) : Ty → Bool
  | .prim _          => true
  | .pair a b        => Ty.bvarsBelow n a && Ty.bvarsBelow n b
...
theorem Ty.bvarsBelow_iff {n : Nat} (t : Ty) :
  ...
  | pair a b iha ihb =>
    ...
    exact ⟨fun ⟨ha, hb⟩ => .pair ha hb, fun h => by cases h with | pair ha hb => exact ⟨ha, hb⟩⟩
  | arrow a b iha ihb =>
    simp only [Ty.bvarsBelow, Bool.and_eq_true, iha, ihb]
...
theorem Ty.noFreeVars_of_forall_not_mem {t : Ty} (h : ∀ z, z ∉ t.freeVars) :
  ...
  | pair a b iha ihb =>
    refine .pair (iha fun z hz => ?_) (ihb fun z hz => ?_)
...
theorem Infer.belowFvars {Φ ctx e Φ' S τ} (h : Infer Φ ctx e Φ' S τ) :
  ...
  | pair ha hb =>
    ...
    obtain ⟨ha_τ, ha_s⟩ := Infer.belowFvars ha hctx (fun y hy => htfv y (.inl hy))
    have hctx1 := Subst.onCtx_below ha_s (Infer.frontier_le ha) hctx
    obtain ⟨hb_τ, hb_s⟩ := Infer.belowFvars hb hctx1
      (fun y hy => lt_of_lt_of_le (htfv y (.inr hy)) (Infer.frontier_le ha))
  ...
  | fst he huni =>
    ...
    obtain ⟨he_τ, he_s⟩ := Infer.belowFvars he hctx htfv
  ...
  | match_ hscrut hne huni hbr =>
    ...
    expose_names
    simp only [Expr.tyFreeVars, List.mem_append] at htfv
    obtain ⟨hscrut_τ, hS₁⟩ := Infer.belowFvars hscrut hctx (fun y hy => htfv y (.inl hy))
...
theorem Infer.complete_pair {a b : Expr}
  ...
  cases hty with
  | pair hta htb =>
    obtain ⟨Φ₁, S₁, τa, R₁, hinfa, haga, htya, hR₁, hR₁fix, hS₁K⟩ :=
      iha K hwf hbelow hS₀ hKΦ (fun y hy => hKe y (.inl hy)) hKfix hta
    have hS₁ : ∀ p ∈ S₁, p.2.IsLC := (Infer.lc hinfa hwf).2
    have hle : Φ ≤ Φ₁ := Infer.frontier_le hinfa
    have hbelowτa : Ty.BelowFvars Φ₁ τa :=
      (Infer.belowFvars hinfa hbelow (fun y hy => hKΦ y (hKe y (.inl hy)))).1
    have hbelowS₁ : ∀ p ∈ S₁, Ty.BelowFvars Φ₁ p.2 :=
      (Infer.belowFvars hinfa hbelow (fun y hy => hKΦ y (hKe y (.inl hy)))).2
    ...
    obtain ⟨Φ₂, S₂, τb, R₂, hinfb, hagb, htyb, hR₂, hR₂fix, hS₂K⟩ :=
      ihb K hwf₁ hbelow₁ hR₁ (fun k hk => lt_of_lt_of_le (hKΦ k hk) hle)
        (fun y hy => hKe y (.inr hy)) hR₁fix htb'
    refine ⟨Φ₂, S₁ ++ S₂, .pair (S₂.onTy τa) τb, R₂, .pair hinfa hinfb, ?_, ?_, hR₂, hR₂fix, ?_⟩
...
theorem UnifyRel.greatest_K {K : List Nat} :
  ...
  | _, _, _, @UnifyRel.pair a b c d S₁ S₂ h₁ h₂, S', hlc, hS', hK => by
    simp only [Unifies, Subst.onTy_pair, Ty.pair.injEq] at hS'
    obtain ⟨hac, hbd⟩ := hS'
    obtain ⟨R₁, hR₁, hR₁lc, hR₁K⟩ := UnifyRel.greatest_K h₁ S' hlc hac hK
    have hR₁bd : Unifies R₁ (S₁.onTy b) (S₁.onTy d) := by
    ...
    · intro a b U hsz ha hb hUlc hU hUK
      ...
      | fvar n =>
        ...
        | pair b₁ b₂ =>
          ...
          · by_cases hnK : n ∈ K
            · exfalso
              have h1 : U.onTy (Ty.fvar n) = U.onTy (Ty.pair b₁ b₂) := hU
              rw [hUK n hnK, Subst.onTy_pair] at h1; exact absurd h1 (by simp)
            · exact ⟨[(n, .pair b₁ b₂)], .fvarL (by simp) hocc,
                by intro p' hp'; rw [List.mem_singleton] at hp'; subst hp'; exact hnK⟩
        | arrow b₁ b₂ =>
          by_cases hocc : n ∈ (Ty.arrow b₁ b₂).freeVars
      ...
      | pair a₁ a₂ =>
        ...
        | fvar m =>
          ...
          · by_cases hmK : m ∈ K
            · exfalso
              have h1 : U.onTy (Ty.pair a₁ a₂) = U.onTy (Ty.fvar m) := hU
              rw [hUK m hmK, Subst.onTy_pair] at h1; exact absurd h1 (by simp)
            · exact ⟨[(m, .pair a₁ a₂)], .fvarR (by simp) hocc,
                by intro p' hp'; rw [List.mem_singleton] at hp'; subst hp'; exact hmK⟩
        | prim q => simp [Unifies] at hU
        | arrow b₁ b₂ => simp [Unifies] at hU
        ...
        | pair b₁ b₂ =>
          cases hb with
          | pair hb₁ hb₂ =>
          have hpsz : (U.onTy (.pair a₁ a₂)).size = 1 + (U.onTy a₁).size + (U.onTy a₂).size := by
            simp [Subst.onTy_pair, Ty.size]
          simp only [Unifies, Subst.onTy_pair, Ty.pair.injEq] at hU
...
theorem TypeOfHM.weaken_scheme {ctors : CtorEnv} {env_post env : Env} {M M' : PolyTy}
  ...
  have H : ∀ {ctx : Ctx} {e₀ : Expr} {τ₀ : Ty}, TypeOfHM ctx e₀ τ₀ →
    ...
    | pair hfst hsnd ihfst ihsnd => intro ep heq; exact .pair (ihfst ep heq) (ihsnd ep heq)
    ...
    | @letIn ann ctx boundExpr bodyCtx body bodyTy Msch L hwf hann hcofin heqctx hbody ihcofin ihbody =>
      ...
      simpa only [heqctx, List.cons_append] using hbc
    | fst he ihe => intro ep heq; exact .fst (ihe ep heq)
    | snd he ihe => intro ep heq; exact .snd (ihe ep heq)
    ...
    | @match_ ctx scrut tyName tyArgs branches resultTy hscrut hne hbrs ihscrut ihbrs =>
      intro ep heq
      refine TypeOfHM.match_ (ihscrut ep heq) hne ?_
      intro branch hmem
      obtain ⟨pat, body⟩ := branch
      obtain ⟨ctorr, instContents, hlook, htyName, hpc, hcontents, hinstC, hbodyT, ihbody⟩ :=
        ihbrs (pat, body) hmem
...
/-- **Skolem unifier existence.** When the declarative residual `R` sends `τ₁` to
    ...
    skolems, `τ₁`'s vars all `< Φ₁` so disjoint from `Ys`, and `B`'s free vars all
    in `Ys`), there is a **`Ys`-fixing** unifier `U` of `τ₁` and `B`. It is built
    by conjugating `R` with a fresh block-swap `Ys ↔ Ws`, so it agrees with `R`
    off `Ys` (in particular on everything below `Φ₁`) while keeping `Ys` rigid. -/
theorem exists_skolem_unifier {τ₁ B : Ty} {R : Subst} {Ys : List Nat} {Φ₁ pc : Nat}
    (hYs : Ys = freshVars Φ₁ pc)
    (hR : ∀ p ∈ R, p.2.IsLC)
    (hτ₁below : ∀ z ∈ τ₁.freeVars, z < Φ₁)
    (hBYs : ∀ z ∈ B.freeVars, z ∈ Ys)
    (hRτ₁ : R.onTy τ₁ = B) :
    ∃ U : Subst, (∀ p ∈ U, p.2.IsLC) ∧
      (∀ Y ∈ Ys, U.onTy (.fvar Y) = .fvar Y) ∧
      Unifies U τ₁ B ∧
      (∀ t : Ty, (∀ z ∈ t.freeVars, z < Φ₁) → U.onTy t = R.onTy t) := by
  obtain ⟨W, hWge, hWfresh⟩ := exists_fresh_block
    (R.map Prod.fst ++ R.flatMap (fun p => p.2.freeVars)) Φ₁ pc
  have hWdom : ∀ p ∈ R, p.1 < W := fun p hp =>
    hWfresh p.1 (List.mem_append_left _ (List.mem_map.mpr ⟨p, hp, rfl⟩))
  have hWrng : ∀ p ∈ R, ∀ v ∈ p.2.freeVars, v < W := fun p hp v hv =>
    hWfresh v (List.mem_append_right _ (List.mem_flatMap.mpr ⟨p, hp, hv⟩))
  -- Domain bounds for the two block lists.
  have hBLkey : ∀ p ∈ blockList Φ₁ W pc, Φ₁ ≤ p.1 := by
    intro p hp; simp only [blockList, List.mem_map] at hp
    obtain ⟨i, _, rfl⟩ := hp; omega
  have hBBkey : ∀ p ∈ blockListBack Φ₁ W pc, W ≤ p.1 := by
    intro p hp; simp only [blockListBack, List.mem_map] at hp
    obtain ⟨i, _, rfl⟩ := hp; omega
  -- The factoring helper: `U` agrees with `R` on `Φ₁`-bounded types.
  have hfact : ∀ t : Ty, (∀ z ∈ t.freeVars, z < Φ₁) →
      (blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc).onTy t = R.onTy t := by
    intro t htlt
    rw [Subst.onTy_append, Subst.onTy_append]
    have hBLt : (blockList Φ₁ W pc).onTy t = t :=
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
        have := hBLkey p hp; have := htlt p.1 hc; omega)
    rw [hBLt]
    have hRtW : Ty.BelowFvars W (R.onTy t) :=
      Subst.onTy_belowFvars (fun p hp => Ty.BelowFvars.of_freeVars_lt
        (fun v hv => hWrng p hp v hv)) (Ty.BelowFvars.of_freeVars_lt (fun v hv => by
          have := htlt v hv; omega))
    exact Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
      have := hBBkey p hp; have := hRtW.mem_lt p.1 hc; omega)
  refine ⟨blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc, ?_, ?_, ?_, hfact⟩
  · -- LC
    intro p hp
    rcases List.mem_append.mp hp with hp' | hp'
    · rcases List.mem_append.mp hp' with hp'' | hp''
      · exact blockList_lc Φ₁ W pc p hp''
      · exact hR p hp''
    · exact blockListBack_lc Φ₁ W pc p hp'
  · -- fixing the skolems `Ys`
    intro Y hY
    rw [hYs] at hY
    have hYge := freshVars_ge Y hY
    have hYlt := freshVars_lt Y hY
    rw [Subst.onTy_append, Subst.onTy_append]
    rw [show (blockList Φ₁ W pc).onTy (Ty.fvar Y) = Ty.fvar (Y - Φ₁ + W) by
      rw [blockList, rangeMapList_onTy_fvar Φ₁ W pc (Or.inl hWge) Y, if_pos ⟨hYge, hYlt⟩]]
    rw [show R.onTy (Ty.fvar (Y - Φ₁ + W)) = Ty.fvar (Y - Φ₁ + W) from
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
        simp only [Ty.freeVars, List.mem_singleton] at hc
        have := hWdom p hp; omega)]
    rw [blockListBack, rangeMapList_onTy_fvar W Φ₁ pc (Or.inr hWge) (Y - Φ₁ + W),
        if_pos ⟨by omega, by omega⟩]
    congr 1; omega
  · -- `Unifies`: `U.onTy τ₁ = B = U.onTy B`
    have hUτ₁ : (blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc).onTy τ₁ = B := by
      rw [hfact τ₁ hτ₁below, hRτ₁]
    have hUB : (blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc).onTy B = B := by
      refine Subst.onTy_eq_self_of_fixes (fun v hv => ?_)
      -- every free var of `B` is a skolem, hence fixed
      have hvY : v ∈ Ys := hBYs v hv
      rw [hYs] at hvY
      have hvge := freshVars_ge v hvY
      have hvlt := freshVars_lt v hvY
      rw [Subst.onTy_append, Subst.onTy_append]
      rw [show (blockList Φ₁ W pc).onTy (Ty.fvar v) = Ty.fvar (v - Φ₁ + W) by
        rw [blockList, rangeMapList_onTy_fvar Φ₁ W pc (Or.inl hWge) v, if_pos ⟨hvge, hvlt⟩]]
      rw [show R.onTy (Ty.fvar (v - Φ₁ + W)) = Ty.fvar (v - Φ₁ + W) from
        Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
          simp only [Ty.freeVars, List.mem_singleton] at hc
          have := hWdom p hp; omega)]
      rw [blockListBack, rangeMapList_onTy_fvar W Φ₁ pc (Or.inr hWge) (v - Φ₁ + W),
          if_pos ⟨by omega, by omega⟩]
      congr 1; omega
    show (blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc).onTy τ₁
      = (blockList Φ₁ W pc ++ R ++ blockListBack Φ₁ W pc).onTy B
    rw [hUτ₁, hUB]

/-! ### Left-leaning unification (`OUnify`) and the orientation kernel

`OUnify` is `UnifyRel` with the var-var bias of the *algorithm*: its `fvarR`
constructor (bind the right variable) is restricted to a **non-variable** left
side. This is exactly what `unifyCore`/`UnifyRel.complete_aux` produce, and it is
what makes the skolem-orientation fact true: a right-hand bare variable that some
unifier keeps rigid is never bound. -/

mutual

/-- Left-leaning unification: `UnifyRel` whose `fvarR` requires a non-variable
    left side (var-var is always resolved by binding the left var, via `fvarL`). -/
inductive OUnify : Ty → Ty → Subst → Prop
  | prim {p} : OUnify (.prim p) (.prim p) []
  | fvarRefl {n} : OUnify (.fvar n) (.fvar n) []
  | fvarL {n τ} : τ ≠ .fvar n → n ∉ τ.freeVars → OUnify (.fvar n) τ [(n, τ)]
  | fvarR {n τ} : (∀ m, τ ≠ .fvar m) → n ∉ τ.freeVars → OUnify τ (.fvar n) [(n, τ)]
  | arrow {a b c d S₁ S₂} :
    OUnify a c S₁ → OUnify (S₁.onTy b) (S₁.onTy d) S₂ →
    OUnify (.arrow a b) (.arrow c d) (S₁ ++ S₂)
  | pair {a b c d S₁ S₂} :
    OUnify a c S₁ → OUnify (S₁.onTy b) (S₁.onTy d) S₂ →
    OUnify (.pair a b) (.pair c d) (S₁ ++ S₂)
  | customTy {nm tys₁ tys₂ S} :
    OUnifyList tys₁ tys₂ S → OUnify (.customTy nm tys₁) (.customTy nm tys₂) S

inductive OUnifyList : List Ty → List Ty → Subst → Prop
  | nil : OUnifyList [] [] []
  | cons {t₁ t₂ ts₁ ts₂ S₁ S₂} :
    OUnify t₁ t₂ S₁ → OUnifyList (ts₁.map S₁.onTy) (ts₂.map S₁.onTy) S₂ →
    OUnifyList (t₁ :: ts₁) (t₂ :: ts₂) (S₁ ++ S₂)

end

/-! Every left-leaning derivation is a genuine `UnifyRel` derivation. -/
mutual
theorem OUnify.toUnifyRel : {a b : Ty} → {S : Subst} → OUnify a b S → UnifyRel a b S
  | _, _, _, .prim => .prim
  | _, _, _, .fvarRefl => .fvarRefl
  | _, _, _, .fvarL hne hocc => .fvarL hne hocc
  | _, _, _, .fvarR hnv hocc => .fvarR (hnv _) hocc
  | _, _, _, .arrow h₁ h₂ => .arrow h₁.toUnifyRel h₂.toUnifyRel
  | _, _, _, .pair h₁ h₂ => .pair h₁.toUnifyRel h₂.toUnifyRel
  | _, _, _, .customTy hl => .customTy hl.toUnifyRelList
theorem OUnifyList.toUnifyRelList : {as bs : List Ty} → {S : Subst} →
    OUnifyList as bs S → UnifyRelList as bs S
  | _, _, _, .nil => .nil
  | _, _, _, .cons h₁ ht => .cons h₁.toUnifyRel ht.toUnifyRelList
end

/-- Completeness for the left-leaning relation: a unifier of two LC types yields
    an `OUnify` derivation. Mirrors `UnifyRel.complete_aux` exactly (the var-var
    case is resolved by `fvarL`, so every `fvarR` it builds has a non-variable
    left). -/
theorem OUnify.complete_aux : ∀ (N : Nat),
    (∀ {a b : Ty} {U : Subst}, 2 * (U.onTy a).size < N → a.IsLC → b.IsLC →
        Unifies U a b → ∃ S, OUnify a b S) ∧
    (∀ {as bs : List Ty} {U : Subst}, 2 * TyList.size (as.map U.onTy) + 1 < N →
        (∀ t ∈ as, t.IsLC) → (∀ t ∈ bs, t.IsLC) → as.length = bs.length →
        as.map U.onTy = bs.map U.onTy → ∃ S, OUnifyList as bs S) := by
  intro N
  induction N with
  | zero => exact ⟨fun h => absurd h (by omega), fun h => absurd h (by omega)⟩
  | succ N ih =>
    ...
    · intro a b U hsz ha hb hU
      ...
      | prim p =>
        ...
        | prim q =>
          simp only [Unifies, Subst.onTy_prim, Ty.prim.injEq] at hU
          subst hU; exact ⟨[], .prim⟩
        | fvar m => exact ⟨[(m, .prim p)], .fvarR (by intro k; simp) (by simp [Ty.freeVars])⟩
        | pair b₁ b₂ => simp [Unifies] at hU
        | arrow b₁ b₂ => simp [Unifies] at hU
        | customTy nm bs => simp [Unifies] at hU
      | fvar n =>
      ...
      | pair a₁ a₂ =>
        ...
        | fvar m =>
          ...
          · have hlt := Ty.size_onTy_fvar_lt (S := U) hocc (by simp)
            simp only [Unifies] at hU; rw [hU] at hlt; omega
          · exact ⟨[(m, .pair a₁ a₂)], .fvarR (by intro k; simp) hocc⟩
        | prim q => simp [Unifies] at hU
        | arrow b₁ b₂ => simp [Unifies] at hU
        | customTy nm bs => simp [Unifies] at hU
        | pair b₁ b₂ =>
          ...
          obtain ⟨S₁, h₁⟩ := ihU (a := a₁) (b := b₁) (U := U)
            (by rw [hpsz] at hsz; omega) ha₁ hb₁ hU.1
          obtain ⟨R, hR⟩ := UnifyRel.greatest h₁.toUnifyRel U hU.1
          have hS₁lc := UnifyRel.lc h₁.toUnifyRel ha₁ hb₁
          obtain ⟨S₂, h₂⟩ := ihU (a := S₁.onTy a₂) (b := S₁.onTy b₂) (U := R)
            (by rw [← hR a₂]; rw [hpsz] at hsz; omega)
            (Subst.onTy_lc hS₁lc ha₂) (Subst.onTy_lc hS₁lc hb₂)
            (by show R.onTy (S₁.onTy a₂) = R.onTy (S₁.onTy b₂); rw [← hR a₂, ← hR b₂]; exact hU.2)
      ...
      | arrow a₁ a₂ =>
        ...
        | fvar m =>
          ...
          · have hlt := Ty.size_onTy_fvar_lt (S := U) hocc (by simp)
            simp only [Unifies] at hU; rw [hU] at hlt; omega
          · exact ⟨[(m, .arrow a₁ a₂)], .fvarR (by intro k; simp) hocc⟩
        | prim q => simp [Unifies] at hU
        | pair b₁ b₂ => simp [Unifies] at hU
        | customTy nm bs => simp [Unifies] at hU
        | arrow b₁ b₂ =>
      ...
      | customTy nm tys₁ =>
        ...
        | fvar m =>
          ...
          · have hlt := Ty.size_onTy_fvar_lt (S := U) hocc (by simp)
            simp only [Unifies] at hU; rw [hU] at hlt; omega
          · exact ⟨[(m, .customTy nm tys₁)], .fvarR (by intro k; simp) hocc⟩
        | prim q => simp [Unifies] at hU
        | pair b₁ b₂ => simp [Unifies] at hU
        | arrow b₁ b₂ => simp [Unifies] at hU
        | customTy nm' tys₂ =>
      ...
      | cons t₁ ts₁ =>
        ...
        | cons t₂ ts₂ =>
          ...
          obtain ⟨S₁, h₁⟩ := ihU (a := t₁) (b := t₂) (U := U)
            (by rw [htsz] at hsz; omega) (has t₁ List.mem_cons_self)
            (hbs t₂ List.mem_cons_self) hmap.1
          obtain ⟨R, hR⟩ := UnifyRel.greatest h₁.toUnifyRel U hmap.1
          have hS₁lc := UnifyRel.lc h₁.toUnifyRel (has t₁ List.mem_cons_self) (hbs t₂ List.mem_cons_self)
          have key : ∀ l : List Ty, l.map (R.onTy ∘ S₁.onTy) = l.map U.onTy := by
            intro l; apply List.map_congr_left; intro t _; exact (hR t).symm
          have hkey1 : (ts₁.map S₁.onTy).map R.onTy = ts₁.map U.onTy := by
            rw [List.map_map]; exact key ts₁
...
/-- A left-leaning derivation exists for any unifiable pair of LC types. -/
theorem OUnify.complete {a b : Ty} {U : Subst}
    (ha : a.IsLC) (hb : b.IsLC) (hU : Unifies U a b) : ∃ S, OUnify a b S :=
  (OUnify.complete_aux (2 * (U.onTy a).size + 1)).1 (by omega) ha hb hU

/-! **The orientation kernel.** If `b`'s free vars are all skolems `Ys` and some
    unifier of `a` and `b` keeps every `Y ∈ Ys` rigid, then the left-leaning MGU
    `S` binds none of the `Ys`. Intuition: a skolem on the right is only ever
    bound by `fvarL`/`fvarR`, and both are impossible against a non-variable
    image or a rigid-kept variable. -/
mutual
theorem OUnify.skolem_escape {Ys : List Nat} :
    {a b : Ty} → {S : Subst} → OUnify a b S →
    (∀ z ∈ b.freeVars, z ∈ Ys) →
    (∃ U : Subst, Unifies U a b ∧ ∀ Y ∈ Ys, U.onTy (Ty.fvar Y) = Ty.fvar Y) →
    ∀ Y ∈ Ys, Y ∉ S.map Prod.fst
  | _, _, _, .prim, _, _ => by simp
  | _, _, _, .fvarRefl, _, _ => by simp
  | _, _, _, @OUnify.fvarL n τ hne hocc, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    intro Y hY hmem
    simp only [List.map_cons, List.map_nil, List.mem_cons, List.not_mem_nil, or_false] at hmem
    subst hmem
    have h2 : U.onTy (Ty.fvar Y) = Ty.fvar Y := hUfix Y hY
    have h1 : U.onTy (Ty.fvar Y) = U.onTy τ := hUuni
    rw [h2] at h1
    cases τ with
    | fvar m =>
      have hm : m ∈ Ys := hbY m (by simp [Ty.freeVars])
      rw [hUfix m hm] at h1
      exact hne h1.symm
    | prim p => simp at h1
    | bvar i => simp at h1
    | arrow x y => simp at h1
    | pair x y => simp at h1
    | customTy nm ts => simp at h1
  | _, _, _, @OUnify.fvarR n τ hnv hocc, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    have hn : n ∈ Ys := hbY n (by simp [Ty.freeVars])
    exfalso
    have h1 : U.onTy τ = U.onTy (Ty.fvar n) := hUuni
    rw [hUfix n hn] at h1
    cases τ with
    | fvar m => exact hnv m rfl
    | prim p => simp at h1
    | bvar i => simp at h1
    | arrow x y => simp at h1
    | pair x y => simp at h1
    | customTy nm ts => simp at h1
  | _, _, _, @OUnify.arrow a b c d S₁ S₂ h₁ h₂, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    simp only [Unifies, Subst.onTy_arrow, Ty.arrow.injEq] at hUuni
    obtain ⟨hac, hbd⟩ := hUuni
    have hcY : ∀ z ∈ c.freeVars, z ∈ Ys := fun z hz =>
      hbY z (by simp only [Ty.freeVars, List.mem_dedup, List.mem_append]; exact Or.inl hz)
    have hdY : ∀ z ∈ d.freeVars, z ∈ Ys := fun z hz =>
      hbY z (by simp only [Ty.freeVars, List.mem_dedup, List.mem_append]; exact Or.inr hz)
    have escape_S₁ : ∀ Y ∈ Ys, Y ∉ S₁.map Prod.fst :=
      OUnify.skolem_escape h₁ hcY ⟨U, hac, hUfix⟩
    obtain ⟨R₁, hR₁⟩ := (h₁.toUnifyRel).greatest U hac
    have hS₁fix : ∀ Y ∈ Ys, S₁.onTy (Ty.fvar Y) = Ty.fvar Y := fun Y hY =>
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
        simp only [Ty.freeVars, List.mem_singleton] at hc
        exact escape_S₁ Y hY (hc ▸ List.mem_map.mpr ⟨p, hp, rfl⟩))
    have hdY' : ∀ z ∈ (S₁.onTy d).freeVars, z ∈ Ys := by
      intro z hz
      obtain ⟨v, hv, hzv⟩ := Ty.mem_freeVars_onTy_iff.mp hz
      have hvY : v ∈ Ys := hdY v hv
      rw [hS₁fix v hvY] at hzv
      simp only [Ty.freeVars, List.mem_singleton] at hzv
      subst hzv; exact hvY
    have hR₁fix : ∀ Y ∈ Ys, R₁.onTy (Ty.fvar Y) = Ty.fvar Y := by
      intro Y hY
      calc R₁.onTy (Ty.fvar Y) = R₁.onTy (S₁.onTy (Ty.fvar Y)) := by rw [hS₁fix Y hY]
        _ = U.onTy (Ty.fvar Y) := (hR₁ (Ty.fvar Y)).symm
        _ = Ty.fvar Y := hUfix Y hY
    have hR₁uni : Unifies R₁ (S₁.onTy b) (S₁.onTy d) := by
      show R₁.onTy (S₁.onTy b) = R₁.onTy (S₁.onTy d)
      rw [← hR₁ b, ← hR₁ d]; exact hbd
    have escape_S₂ : ∀ Y ∈ Ys, Y ∉ S₂.map Prod.fst :=
      OUnify.skolem_escape h₂ hdY' ⟨R₁, hR₁uni, hR₁fix⟩
    intro Y hY hmem
    rw [List.map_append, List.mem_append] at hmem
    rcases hmem with hmem | hmem
    · exact escape_S₁ Y hY hmem
    · exact escape_S₂ Y hY hmem
  | _, _, _, @OUnify.pair a b c d S₁ S₂ h₁ h₂, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    simp only [Unifies, Subst.onTy_pair, Ty.pair.injEq] at hUuni
    obtain ⟨hac, hbd⟩ := hUuni
    have hcY : ∀ z ∈ c.freeVars, z ∈ Ys := fun z hz =>
      hbY z (by simp only [Ty.freeVars, List.mem_dedup, List.mem_append]; exact Or.inl hz)
    have hdY : ∀ z ∈ d.freeVars, z ∈ Ys := fun z hz =>
    ...
    · exact escape_S₁ Y hY hmem
    · exact escape_S₂ Y hY hmem
  | _, _, _, .customTy hl, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    simp only [Unifies, Subst.onTy_customTy, Ty.customTy.injEq, true_and] at hUuni
    exact OUnifyList.skolem_escape hl
      (fun t ht z hz => hbY z (by simp only [Ty.freeVars]; exact TyList.mem_freeVars_of_mem ht hz))
      ⟨U, hUuni, hUfix⟩
theorem OUnifyList.skolem_escape {Ys : List Nat} :
    {as bs : List Ty} → {S : Subst} → OUnifyList as bs S →
    (∀ t ∈ bs, ∀ z ∈ t.freeVars, z ∈ Ys) →
    (∃ U : Subst, as.map U.onTy = bs.map U.onTy ∧ ∀ Y ∈ Ys, U.onTy (Ty.fvar Y) = Ty.fvar Y) →
    ∀ Y ∈ Ys, Y ∉ S.map Prod.fst
  | _, _, _, .nil, _, _ => by simp
  | _, _, _, @OUnifyList.cons t₁ t₂ ts₁ ts₂ S₁ S₂ h₁ ht, hbY, hU => by
    obtain ⟨U, hUuni, hUfix⟩ := hU
    simp only [List.map_cons, List.cons.injEq] at hUuni
    obtain ⟨ht1t2, htail⟩ := hUuni
    have ht2Y : ∀ z ∈ t₂.freeVars, z ∈ Ys := hbY t₂ List.mem_cons_self
    have escape_S₁ : ∀ Y ∈ Ys, Y ∉ S₁.map Prod.fst :=
      OUnify.skolem_escape h₁ ht2Y ⟨U, ht1t2, hUfix⟩
    obtain ⟨R₁, hR₁⟩ := (h₁.toUnifyRel).greatest U ht1t2
    have hS₁fix : ∀ Y ∈ Ys, S₁.onTy (Ty.fvar Y) = Ty.fvar Y := fun Y hY =>
      Ty.substFvars_eq_self_of_no_key (fun p hp hc => by
        simp only [Ty.freeVars, List.mem_singleton] at hc
        exact escape_S₁ Y hY (hc ▸ List.mem_map.mpr ⟨p, hp, rfl⟩))
    have hR₁fix : ∀ Y ∈ Ys, R₁.onTy (Ty.fvar Y) = Ty.fvar Y := by
      ...
      calc R₁.onTy (Ty.fvar Y) = R₁.onTy (S₁.onTy (Ty.fvar Y)) := by rw [hS₁fix Y hY]
        _ = U.onTy (Ty.fvar Y) := (hR₁ (Ty.fvar Y)).symm
        _ = Ty.fvar Y := hUfix Y hY
    have hts2Y : ∀ t ∈ ts₂.map S₁.onTy, ∀ z ∈ t.freeVars, z ∈ Ys := by
      intro t ht z hz
      obtain ⟨t0, ht0, rfl⟩ := List.mem_map.mp ht
      obtain ⟨v, hv, hzv⟩ := Ty.mem_freeVars_onTy_iff.mp hz
      have hvY : v ∈ Ys := hbY t0 (List.mem_cons_of_mem _ ht0) v hv
      rw [hS₁fix v hvY] at hzv
      simp only [Ty.freeVars, List.mem_singleton] at hzv
      subst hzv; exact hvY
    have hR₁uni : (ts₁.map S₁.onTy).map R₁.onTy = (ts₂.map S₁.onTy).map R₁.onTy := by
      have key : ∀ l : List Ty, l.map (R₁.onTy ∘ S₁.onTy) = l.map U.onTy := fun l =>
        List.map_congr_left (fun t _ => (hR₁ t).symm)
      rw [List.map_map, List.map_map, key, key]; exact htail
    have escape_S₂ : ∀ Y ∈ Ys, Y ∉ S₂.map Prod.fst :=
      OUnifyList.skolem_escape ht hts2Y ⟨R₁, hR₁uni, hR₁fix⟩
    intro Y hY hmem
    rw [List.map_append, List.mem_append] at hmem
    rcases hmem with hmem | hmem
    · exact escape_S₁ Y hY hmem
...
theorem Infer.complete_pair_unify_aux {Φ : Nat} {ctx : Ctx} {S₀ : Subst} {e : Expr}
  ...
  -- STEP 1: recurse on `e`.
  obtain ⟨Φ₁, S₁, τe, R₁, hinfe, hage, htye, hR₁, hR₁K, hS₁K⟩ :=
    ihe K hwf hbelow hS₀ hKΦ hKe hKfix he
  have hS₁ : ∀ p ∈ S₁, p.2.IsLC := (Infer.lc hinfe hwf).2
  have hτeLC : τe.IsLC := (Infer.lc hinfe hwf).1
  have hle1 : Φ ≤ Φ₁ := Infer.frontier_le hinfe
  have hbe := Infer.belowFvars hinfe hbelow (fun y hy => hKΦ y (hKe y hy))
  ...
  -- Factor `U` through the `K`-avoiding derivation `S₂`.
  have hUlc : ∀ p ∈ U, p.2.IsLC := by
    rw [hUdef]
    intro p hp
    rcases List.mem_append.mp hp with hp' | hp'
  ...
  have hUK : ∀ k ∈ K, U.onTy (Ty.fvar k) = Ty.fvar k := fun k hk =>
    (hUvar k (lt_of_lt_of_le (hKΦ k hk) hle1)).trans (hR₁K k hk)
  obtain ⟨S₂, h₂, hS₂K⟩ := UnifyRel.complete_K hτeLC
    (ContainsBvarsUpTo.pair ContainsBvarsUpTo.fvar ContainsBvarsUpTo.fvar) hUlc hUni hUK
  obtain ⟨R₂, hR₂, hR₂lc, hR₂K⟩ := UnifyRel.greatest_K h₂ U hUlc hUni hUK
  -- Assemble.
  refine ⟨Φ₁, S₁, S₂, τe, R₂, hinfe, h₂, ?_, ?_, ?_, hR₂lc, hR₂K, ?_⟩
  · intro v hv
    have ht1 : Ty.BelowFvars Φ₁ (S₁.onTy (Ty.fvar v)) :=
...
theorem Infer.complete_fst {e : Expr} (ihe : Infer.CompleteAt e) :
  ...
  cases hty with
  | fst he =>
    obtain ⟨Φ₁, S₁, S₂, τe, R, hinfe, huni, hag, hα, _, hRlc, hRK, hSK⟩ :=
      Infer.complete_pair_unify_aux ihe hwf hbelow hS₀ hKΦ hKe hKfix he
    exact ⟨Φ₁ + 2, S₁ ++ S₂, S₂.onTy (.fvar Φ₁), R, .fst hinfe huni, hag, hα.symm, hRlc, hRK, hSK⟩
...
theorem Infer.complete_snd {e : Expr} (ihe : Infer.CompleteAt e) :
  ...
  cases hty with
  | snd he =>
    obtain ⟨Φ₁, S₁, S₂, τe, R, hinfe, huni, hag, _, hβ, hRlc, hRK, hSK⟩ :=
      Infer.complete_pair_unify_aux ihe hwf hbelow hS₀ hKΦ hKe hKfix he
    exact ⟨Φ₁ + 2, S₁ ++ S₂, S₂.onTy (.fvar (Φ₁ + 1)), R, .snd hinfe huni, hag, hβ.symm, hRlc, hRK, hSK⟩
...
theorem InferBranches.complete {branches : List (MatchPattern × Expr)} :
    ∀ {Φ : Nat} {ctx : Ctx} {tyName : TyName} {ta : List Ty} {ρ : Ty} {R : Subst} (K : List Nat),
  ...
  | nil =>
    intro Φ ctx tyName ta ρ R K _ihbr _hwf _hbelow _hta _hbta _hρ _hbρ hR _hKΦ _hKbr hKfix _hdecl
    exact ⟨Φ, [], R, InferBranches.nil, fun v _ => by rw [List.nil_append], hR, hKfix, by simp⟩
  | cons head rest ih =>
    intro Φ ctx tyName ta ρ R K ihbr hwf hbelow hta hbta hρ hbρ hR hKΦ hKbr hKfix hdecl
    ...
    | mk hlook htyName hpc hpc2 hforall hpb hbctx hbodyty =>
      ...
      obtain ⟨Φ_b, S_b, τ_b, R_b, hinfbody, hagbody, htybody, hR_b, hR_bK, hS_bK⟩ :=
        ihbr (pat, body) (List.mem_cons_self ..) K hbodyWF hbodyBelow hR hKΦ hKbody hKfix hbodyty2
      ...
      obtain ⟨Φ', S_r, R_r, hinfrest, hagrest, hR_r, hR_rK, hS_rK⟩ :=
        ih K (fun br hbr => ihbr br (List.mem_cons_of_mem _ hbr))
...
theorem Infer.complete_match_aux {scrut : Expr} {branches : List (MatchPattern × Expr)}
  ...
  obtain ⟨S₂, h₂, hS₂K⟩ := UnifyRel.complete_K hτsLC hcustomTy_lc hUlc hUni hUK
  ...
  obtain ⟨Φ₃, S₃, R₃, hinfbr, hagbr, hR₃lc, hR₃K, hS₃K⟩ :=
    InferBranches.complete K ihbranches (Subst.onCtx_wf hS₂ hwf₁)
      (Subst.onCtx_below hbS₂ (by omega) hbelow₁)
      hta_lc hta_below hρ_lc hρ_below hR₂lc
      (fun k hk => by have := hKΦ k hk; have := hle1; omega) hKbr hR₂K hbr'
  -- STEP 9: assemble.
  refine ⟨Φ₃, S₁ ++ S₂ ++ S₃, S₃.onTy (S₂.onTy (Ty.fvar (Φ₁ + tyArgs.length))), R₃,
    Infer.match_ hinfs hne h₂ hinfbr, ?_, ?_, hR₃lc, hR₃K, ?_⟩
  · intro v hv
...
theorem Infer.complete' {Φ ctx e Φ' S τ} (h : Infer Φ ctx e Φ' S τ)
  ...
  | pair Da Db =>
    ...
    | pair hta htb =>
      ...
      obtain ⟨R₁, haga, htya, hR₁, hR₁fix⟩ :=
        Infer.complete' Da hwf hbelow hS₀ K hKΦ (fun y hy => hKe y (.inl hy)) hKfix hta
      have hS₁ := (Infer.lc Da hwf).2
      have hle := Infer.frontier_le Da
      have hbelowτa := (Infer.belowFvars Da hbelow (fun y hy => hKΦ y (hKe y (.inl hy)))).1
      have hbelowS₁ := (Infer.belowFvars Da hbelow (fun y hy => hKΦ y (hKe y (.inl hy)))).2
      have hwf₁ := Subst.onCtx_wf hS₁ hwf
      ...
      obtain ⟨R₂, hagb, htyb, hR₂, hR₂fix⟩ :=
        Infer.complete' Db hwf₁ hbelow₁ hR₁ K (fun k hk => lt_of_lt_of_le (hKΦ k hk) hle)
          (fun y hy => hKe y (.inr hy)) hR₁fix htb'
      refine ⟨R₂, ?_, ?_, hR₂, hR₂fix⟩
      · exact Subst.AgreesBelow.trans_append hle haga hbelowS₁ hagb
      · rw [Subst.onTy_pair]
        refine congrArg₂ Ty.pair ?_ htyb
        rw [htya, ← Subst.onTy_append]
  ...
  | fst De Duni =>
    ...
    | fst he =>
      ...
      have hβLC : τβ.IsLC := by
        have := TypeOfHM.regular he; cases this with | pair _ h => exact h
      simp only [Expr.tyFreeVars] at hKe
      obtain ⟨R₁, hage, htye, hR₁, hR₁fix⟩ :=
        Infer.complete' De hwf hbelow hS₀ K hKΦ hKe hKfix he
      have hS₁ := (Infer.lc De hwf).2
      have hτeLC := (Infer.lc De hwf).1
      have hle1 := Infer.frontier_le De
      have hbe := Infer.belowFvars De hbelow (fun y hy => hKΦ y (hKe y hy))
      have hΦ₁τe : Φ₁ ∉ τe.freeVars := fun hm => by have := hbe.1.mem_lt _ hm; omega
      have hΦ₁1τe : Φ₁ + 1 ∉ τe.freeVars := fun hm => by have := hbe.1.mem_lt _ hm; omega
      obtain ⟨W, hWge, hWfresh⟩ := exists_fresh_block
        (R₁.map Prod.fst ++ R₁.flatMap (fun p => p.2.freeVars)
      ...
      have hUlc : ∀ p ∈ U, p.2.IsLC := by
        ...
        · rcases List.mem_cons.mp hp' with rfl | hp''
          ...
          · obtain rfl := List.mem_singleton.mp hp''
            exact hβLC
      have hUK : ∀ k ∈ K, U.onTy (Ty.fvar k) = Ty.fvar k :=
        fun k hk => (hUvar k (lt_of_lt_of_le (hKΦ k hk) hle1)).trans (hR₁fix k hk)
      obtain ⟨R₂, hR₂, hR₂lc, hR₂fix⟩ := UnifyRel.greatest_K h₂ U hUlc hUni hUK
      refine ⟨R₂, ?_, ?_, hR₂lc, hR₂fix⟩
      · intro v hv
        have ht1 : Ty.BelowFvars Φ₁ (S₁.onTy (Ty.fvar v)) :=
  ...
  | match_ Dscrut hne Duni Dbr =>
    ...
    have hUK : ∀ k ∈ K, U.onTy (Ty.fvar k) = Ty.fvar k := by
      intro k hk
      have hklt : k < Φ₁ := lt_of_lt_of_le (hKΦ k hk) hle1
      rw [hUonTy]
      have hA_id : Subst.onTy ((freshVars Φ₁ (taD.length + 1)).zip
          ((freshVars W₀ (taD.length + 1)).map (Ty.fvar ·))) (Ty.fvar k) = Ty.fvar k := by
        apply Ty.substFvars_eq_self_of_no_key
        intro p hp hc
        simp only [Ty.freeVars, List.mem_singleton] at hc
        have hp1 : p.1 ∈ freshVars Φ₁ (taD.length + 1) := (List.of_mem_zip hp).1
      ...
      have := freshVars_ge p.1 hp1
      omega
    obtain ⟨R₂, hR₂_eq, hR₂lc, hR₂fix⟩ := UnifyRel.greatest_K h₂ U hUlc hUni hUK
    have hS₂ : ∀ p ∈ S₂, p.2.IsLC := UnifyRel.lc h₂ hτsLC hcustomTy_lc
    ...
    obtain ⟨R₃, hagbr, hR₃lc, hR₃fix⟩ :=
      InferBranches.complete' Dbr (Subst.onCtx_wf hS₂ hwf₁)
        (Subst.onCtx_below hbS₂ (by omega) hbelow₁)
        hta_lc hta_below hρ_lc hρ_below hR₂lc K (fun k hk => by have := hKΦ k hk; omega)
        hKbr hR₂fix hbr'
    -- STEP 9: assemble.
    refine ⟨R₃, ?_, ?_, hR₃lc, hR₃fix⟩
    · intro v hv
...
theorem InferBranches.complete' {Φ ctx tn ta ρ branches Φ' S}
  ...
  | cons hlook htyName hpc hpc2 Dbody Duni Drest =>
      ...
      obtain ⟨R_b, hagbody, htybody, hR_b, hR_bfix⟩ :=
        Infer.complete' Dbody hbodyWF hbodyBelow hR₀ K hKΦ hKbody hKfix hbodyty2
      have hbodyLC := Infer.lc Dbody hbodyWF
      have hτb_lc := hbodyLC.1
      have hS_b := hbodyLC.2
      have hbb := Infer.belowFvars Dbody hbodyBelow (fun y hy => hKΦ y (hKbody y hy))
      ...
      obtain ⟨R_r, hagrest, hR_r, hR_rfix⟩ :=
        InferBranches.complete' Drest hwf' hbelow' hta' hbta' hρ' hbρ' hR_u K
...
/-- **The second escape, abstractly.** Given the rhs's MGU `Schk` against the
    skolem opening `σ.openVars Ys` (`Ys = freshVars Φ₁ σ.paramCount`) with the
    first escape `hesc1` (`Schk` binds no skolem), and *two* declarative residuals
    `R₁`, `R₁'` realising the typings at two **disjoint** openings `Ys`, `Ys'`
    (both agreeing with `S₀` below `Φ`), no skolem `Y ∈ Ys` leaks into the
    threaded body environment `Schk.onCtx (S₁.onCtx ctx)`. -/
theorem skolem_no_env_leak
    {Φ Φ₁ : Nat} {ctx : Ctx} {S₀ S₁ Schk R₁ R₁' : Subst} {τ₁ : Ty}
    {σ : PolyTy} {Ys Ys' : List Nat}
    (hσnfv : NoFreeVars σ.body)
    (hle : Φ ≤ Φ₁)
    (hbelow : CtxBelow Φ ctx)
    (hτ₁below : Ty.BelowFvars Φ₁ τ₁)
    (hbelowS₁ : ∀ p ∈ S₁, Ty.BelowFvars Φ₁ p.2)
    (hYs : Ys = freshVars Φ₁ σ.paramCount)
    (huni : UnifyRel τ₁ (σ.openVars Ys) Schk)
  ...
  -- The skolem-fixing unifier `U` and its factoring `V` through `Schk`.
  obtain ⟨U, hUlc, hUfix, hUuni, hUfact⟩ :=
    exists_skolem_unifier (Ys := Ys) (Φ₁ := Φ₁) (pc := σ.paramCount)
      hYs hR₁lc (fun z hz => hτ₁below.mem_lt z hz)
      (fun z hz => Ty.freeVars_openVars_closed hσnfv hz) htya.symm
  obtain ⟨V, hV⟩ := huni.greatest U hUuni
  have hbelow₁ : CtxBelow Φ₁ (S₁.onCtx ctx) := Subst.onCtx_below hbelowS₁ hle hbelow
  -- Both residuals send the threaded context to the same `S₀`-context.
  ...
  intro Y hY hmem
  have hYge : Φ₁ ≤ Y := freshVars_ge Y (hYs ▸ hY)
  -- `V` keeps the skolem `Y` rigid (it is kept rigid by `Schk` and then by `U`).
  ...
  have hVfixY : V.onTy (Ty.fvar Y) = Ty.fvar Y := by
    ...
    rw [hSchkY] at h1
    rw [← h1]; exact hUfix Y hY
  -- Locate the leaking (`Schk`-substituted) env scheme `Schk.onPolyTy pt0` and,
  -- inside it, the leaking variable `v` of the *pre-substitution* scheme `pt0`.
  ...
  -- `Schk` must move `v` (otherwise the skolem `Y` would equal `v < Φ₁`).
  ...
  -- A variable `Schk` binds is among `τ₁`'s/`B`'s free vars; `B`'s are skolems ≥ Φ₁.
  have hvfree : v ∈ τ₁.freeVars := by
    rcases UnifyRel.dom_mem huni p hp with h | h
    · rwa [hpv] at h
    · rw [hpv] at h
      have hvYs : v ∈ Ys := Ty.freeVars_openVars_closed hσnfv h
      have := freshVars_ge v (hYs ▸ hvYs); omega
  -- Push the skolem through `V`: `Y ∈ (R₁.onTy (.fvar v)).freeVars`.
  have hYR₁v : Y ∈ (R₁.onTy (Ty.fvar v)).freeVars := by
    have hVimg : V.onTy (Schk.onTy (Ty.fvar v)) = R₁.onTy (Ty.fvar v) := by
      rw [← hV (Ty.fvar v)]
      exact hUfact (Ty.fvar v) (fun z hz => by
        simp only [Ty.freeVars, List.mem_singleton] at hz; omega)
    rw [← hVimg]
    exact Ty.mem_freeVars_onTy_iff.mpr ⟨Y, hYv, by rw [hVfixY]; simp [Ty.freeVars]⟩
  -- The two residuals agree on the env variable `v`.
  ...
  rw [hR₁v] at hYR₁v
  -- At the second opening, the skolem `Y` is forced into `Ys'` — contradiction.
...
theorem Infer.completeAt (e : Expr) : Infer.CompleteAt e := by
  ...
  | succ n ih =>
    ...
    | pair a b =>
      exact Infer.complete_pair (ih a (by simp only [Expr.size] at he; omega))
        (ih b (by simp only [Expr.size] at he; omega))
    ...
    | fst e => exact Infer.complete_fst (ih e (by simp only [Expr.size] at he; omega))
    | snd e => exact Infer.complete_snd (ih e (by simp only [Expr.size] at he; omega))
...
/-- `unifyCore` computes a **left-leaning** unifier: the carried `UnifyRel` is in
    fact an `OUnify` derivation (its var-var arm binds the left variable and its
    `fvar`-right arm has a non-variable left). Proved by functional induction on
    `unifyCore`, simultaneously with the list version. -/
theorem unifyCore_oUnify {a b : Ty} (r : {S : Subst // UnifyRel a b S})
    (h : unifyCore a b = some r) : OUnify a b r.1 := by
  induction a, b using unifyCore.induct with
  | motive2 as bs =>
    exact ∀ (rl : {S : Subst // UnifyRelList as bs S}),
      unifyListCore as bs = some rl → OUnifyList as bs rl.1
  | case1 => -- (prim q, prim q)
    simp only [unifyCore, ↓reduceDIte, Option.some.injEq] at h
    rw [← h]; exact OUnify.prim
  | case2 => -- (prim p, prim q), p ≠ q
    simp only [unifyCore, ↓reduceDIte, reduceCtorEq, *] at h
  | case3 => -- (fvar m, fvar m)
    simp only [unifyCore, ↓reduceDIte, Option.some.injEq] at h
    rw [← h]; exact OUnify.fvarRefl
  | case4 => -- (fvar n, fvar m), n ≠ m
    rename_i n m hnm
    simp only [unifyCore, hnm, ↓reduceDIte, Option.some.injEq] at h
    rw [← h]
    exact OUnify.fvarL (by simp only [ne_eq, Ty.fvar.injEq]; omega)
      (by simp only [Ty.freeVars, List.mem_singleton]; omega)
  | case5 => -- arrow, first sub none
    simp only [unifyCore, reduceCtorEq, *] at h
  | case6 => -- arrow, first some, second none
    simp only [unifyCore, reduceCtorEq, *] at h
  | case7 => -- arrow, both some
    rename_i a₁ a₂ c₁ c₂ S₁ hS₁ he1 S₂ hS₂ he2 ih2 ih1
    simp only [unifyCore, he1, he2, Option.some.injEq] at h
    rw [← h]
    exact OUnify.arrow (ih2 ⟨S₁, hS₁⟩ he1) (ih1 ⟨S₂, hS₂⟩ he2)
  | case8 => -- pair, first sub none
    simp only [unifyCore, reduceCtorEq, *] at h
  | case9 => -- pair, first some, second none
    simp only [unifyCore, reduceCtorEq, *] at h
  | case10 => -- pair, both some
    rename_i a₁ a₂ c₁ c₂ S₁ hS₁ he1 S₂ hS₂ he2 ih2 ih1
    simp only [unifyCore, he1, he2, Option.some.injEq] at h
    rw [← h]
    exact OUnify.pair (ih2 ⟨S₁, hS₁⟩ he1) (ih1 ⟨S₂, hS₂⟩ he2)
  | case11 => -- customTy (same name), list none
    simp only [unifyCore, ↓reduceDIte, reduceCtorEq, *] at h
  | case12 => -- customTy (same name), list some
    rename_i ts₁ n₂ ts₂ S hS he ihL
    simp only [unifyCore, ↓reduceDIte, he, Option.some.injEq] at h
    rw [← h]
    exact OUnify.customTy (ihL ⟨S, hS⟩ he)
  | case13 => -- customTy, names differ
    simp only [unifyCore, ↓reduceDIte, reduceCtorEq, *] at h
  | case14 => -- (fvar n, b), n ∈ b.freeVars
    simp only [unifyCore, ↓reduceDIte, reduceCtorEq, *] at h
  | case15 => -- (fvar n, b), n ∉ b.freeVars
    rename_i n b hnv hocc
    simp only [unifyCore, hocc, ↓reduceDIte, Option.some.injEq] at h
    rw [← h]
    exact OUnify.fvarL (fun heq => hnv _ heq) hocc
  | case16 => -- (a, fvar n), n ∈ a.freeVars
    simp only [unifyCore, ↓reduceDIte, reduceCtorEq, *] at h
  | case17 => -- (a, fvar n), n ∉ a.freeVars
    rename_i a n hnvL _ hocc
    simp only [unifyCore, hocc, ↓reduceDIte, Option.some.injEq] at h
    rw [← h]
    exact OUnify.fvarR hnvL hocc
  | case18 => -- catch-all (no arm applies)
    simp only [unifyCore, reduceCtorEq, *] at h
  | case19 => -- list nil nil
    intro rl hl
    simp only [unifyListCore, Option.some.injEq] at hl
    rw [← hl]; exact OUnifyList.nil
  | case20 => -- list cons, first none
    intro rl hl
    simp only [unifyListCore, reduceCtorEq, *] at hl
  | case21 => -- list cons, first some, rest none
    intro rl hl
    simp only [unifyListCore, reduceCtorEq, *] at hl
  | case22 => -- list cons, both some
    rename_i t₁ ts₁ t₂ ts₂ S₁ hS₁ he1 S₂ hS₂ he2 ih2 ih1
    intro rl hl
    simp only [unifyListCore, he1, he2, Option.some.injEq] at hl
    rw [← hl]
    exact OUnifyList.cons (ih2 ⟨S₁, hS₁⟩ he1) (ih1 ⟨S₂, hS₂⟩ he2)
  | case23 => -- list catch-all
    intro rl hl
    simp only [unifyListCore, reduceCtorEq, *] at hl
...
theorem inferCore_complete_pair {a b : Expr}
  ...
  | @pair _ _ _ _ Φ₁ Φ₂ S₁ S₂ τa τb ha hb =>
    ...
    have hSK₂ : ∀ p ∈ S₂, p.1 ∉ K := fun p hp => hSK p (List.mem_append_right _ hp)
    have hKea : ∀ y ∈ a.tyFreeVars, y ∈ K := fun y hy => hKe y (.inl hy)
    have hKeb : ∀ y ∈ b.tyFreeVars, y ∈ K := fun y hy => hKe y (.inr hy)
    -- recurse on `a`, get the function's first output `(Φ₁', S₁', τa')`
    have hsa := iha K hwf hbelow hKΦ hKea hSK₁ ha
    obtain ⟨⟨⟨Φ₁', S₁', τa'⟩, ha', hav₁⟩, hea⟩ := Option.isSome_iff_exists.mp hsa
    -- factor the derivation's `S₁` through the function's principal `ha'`
    have hS₁ := (Infer.lc ha hwf).2
    obtain ⟨R₁, hag1, _, hR₁, hR₁K⟩ := Infer.complete' ha' hwf hbelow hS₁ K hKΦ hKea
      (fun k hk => Subst.fixes_fvar_of_avoids hSK₁ hk) (Infer.sound ha hwf K hKΦ hKea hSK₁)
    have hS₁' := (Infer.lc ha' hwf).2
    have hbf := Infer.belowFvars ha' hbelow (fun y hy => hKΦ y (hKea y hy))
    have hle1 := Infer.frontier_le ha'
    have hwf₁ := Subst.onCtx_wf hS₁' hwf
    have hbelow₁ := Subst.onCtx_below hbf.2 hle1 hbelow
    have hctxeq : R₁.onCtx (S₁'.onCtx ctx) = S₁.onCtx ctx := by
      rw [← Subst.onCtx_append]; exact Subst.onCtx_congr (fun v hv => (hag1 v hv).symm) hbelow
    -- transfer `b`'s typing to the function's intermediate context, re-derive via `Infer.completeAt`
    have hwfS₁ := Subst.onCtx_wf hS₁ hwf
    have htyb' : TypeOfHM ((R₁ ++ S₂).onCtx (S₁'.onCtx ctx)) b τb := by
      rw [Subst.onCtx_append, hctxeq]
      exact Infer.sound hb hwfS₁ K (fun k hk => lt_of_lt_of_le (hKΦ k hk) (Infer.frontier_le ha)) hKeb hSK₂
    have hR₁S₂lc : ∀ p ∈ R₁ ++ S₂, p.2.IsLC := by
      intro p hp; rcases List.mem_append.mp hp with hp | hp
      · exact hR₁ p hp
      · exact (Infer.lc hb hwfS₁).2 p hp
    obtain ⟨_, _, _, _, hinfb, _, _, _, _, hSbK⟩ := Infer.completeAt b K hwf₁ hbelow₁ hR₁S₂lc
      (fun k hk => lt_of_lt_of_le (hKΦ k hk) hle1) hKeb
      (fun k hk => by rw [Subst.onTy_append, hR₁K k hk, Subst.fixes_fvar_of_avoids hSK₂ hk]) htyb'
    have hsb := ihb K hwf₁ hbelow₁ (fun k hk => lt_of_lt_of_le (hKΦ k hk) hle1) hKeb hSbK hinfb
    obtain ⟨⟨⟨Φ₂', S₂', τb'⟩, hb', hav₂⟩, heb⟩ := Option.isSome_iff_exists.mp hsb
...
theorem inferCore_complete_letIn {ann : Option PolyTy} {rhs body : Expr}
  ...
  | @letInAnn _ _ σd _ _ Φ1d Φ2d S1d Schkd S2d τ1d τ2 hrhs hσwf hσnfv huni hesc1 hesc2 hbody =>
    cases happ with
    | letIn hMwf hann hcofin heqctx hbodyD =>
      expose_names
      obtain ⟨hMσ, hσnfv'⟩ := hann σd rfl
      subst heqctx
      rw [hMσ] at hcofin hbodyD
      -- inferCore on rhs succeeds; reorient its typing to the skolem opening
      obtain ⟨⟨⟨Φ₁', S₁', τ₁'⟩, hrhs'⟩, herhs⟩ :=
        Option.isSome_iff_exists.mp (iha hwf hbelow hrhs)
      have hrhs_Ys : TypeOfHM ((S1d ++ Schkd ++ S2d).onCtx ctx) rhs
          (σd.openVars (freshVars Φ₁' σd.paramCount)) :=
        typeOfHM_at_block hσnfv' (freshVars_length _ _) hcofin
      obtain ⟨R₁, haga, htya, hR₁⟩ := Infer.complete' hrhs' hwf hbelow hSlc hrhs_Ys
      have hτ₁lc := (Infer.lc hrhs' hwf).1
      have hS₁lc := (Infer.lc hrhs' hwf).2
      have hle := Infer.frontier_le hrhs'
      have hbelowτ₁ := (Infer.belowFvars hrhs' hbelow).1
      have hbelowS₁ := (Infer.belowFvars hrhs' hbelow).2
      have hBlc : (σd.openVars (freshVars Φ₁' σd.paramCount)).IsLC :=
        PolyTy.openVars_isLC hσwf (by simp)
      -- the `Ys`-fixing unifier; `unifyCore` succeeds, carrying a left-leaning `Schk`
      obtain ⟨U, hUlc, hUfix, hUuni, hUfact⟩ :=
        exists_skolem_unifier (Ys := freshVars Φ₁' σd.paramCount) (Φ₁ := Φ₁')
          (pc := σd.paramCount) rfl hR₁ (fun z hz => hbelowτ₁.mem_lt z hz)
          (fun z hz => Ty.freeVars_openVars_closed hσnfv' hz) htya.symm
      have huniSome : (unifyCore τ₁' (σd.openVars (freshVars Φ₁' σd.paramCount))).isSome :=
        (unifyCore_complete_aux (2 * (U.onTy τ₁').size + 1)).1 (by omega) hτ₁lc hBlc hUuni
      obtain ⟨⟨Schk, hSchk⟩, heuni⟩ := Option.isSome_iff_exists.mp huniSome
      have hO : OUnify τ₁' (σd.openVars (freshVars Φ₁' σd.paramCount)) Schk :=
        unifyCore_oUnify ⟨Schk, hSchk⟩ heuni
      have hescA : ∀ y ∈ freshVars Φ₁' σd.paramCount, y ∉ Schk.map Prod.fst :=
        OUnify.skolem_escape hO (fun z hz => Ty.freeVars_openVars_closed hσnfv' hz)
          ⟨U, hUuni, hUfix⟩
      -- second opening for the env-escape
      have hrhs_Ys' : TypeOfHM ((S1d ++ Schkd ++ S2d).onCtx ctx) rhs
          (σd.openVars (freshVars (Φ₁' + σd.paramCount) σd.paramCount)) :=
        typeOfHM_at_block hσnfv' (freshVars_length _ _) hcofin
      obtain ⟨R₁', haga', htya', hR₁'⟩ := Infer.complete' hrhs' hwf hbelow hSlc hrhs_Ys'
      have hYdisj : ∀ Y ∈ freshVars Φ₁' σd.paramCount,
          Y ∉ freshVars (Φ₁' + σd.paramCount) σd.paramCount := by
        intro Y hY hc
        have h1 := freshVars_lt Y hY
        have h2 := freshVars_ge Y hc
        omega
      have hescB : ∀ Y ∈ freshVars Φ₁' σd.paramCount,
          Y ∉ (Schk.onCtx (S₁'.onCtx ctx)).env.freeVars :=
        skolem_no_env_leak hσnfv' hle hbelow hbelowτ₁ hbelowS₁ rfl hSchk hescA hR₁ htya haga
          htya' haga' hYdisj
      -- thread the body context and recurse with `ihb`
      obtain ⟨V, hV, hVlc⟩ := UnifyRel.greatest_lc hSchk U hUlc hUuni
      have hagSchkV : Subst.AgreesBelow Φ (S1d ++ Schkd ++ S2d) (S₁' ++ Schk ++ V) := by
        intro v hv
        have hbv : Ty.BelowFvars Φ₁' (S₁'.onTy (Ty.fvar v)) :=
          Subst.onTy_belowFvars hbelowS₁ (Ty.BelowFvars.fvar (by omega))
        calc (S1d ++ Schkd ++ S2d).onTy (Ty.fvar v)
            = (S₁' ++ R₁).onTy (Ty.fvar v) := haga v hv
          _ = R₁.onTy (S₁'.onTy (Ty.fvar v)) := by rw [Subst.onTy_append]
          _ = U.onTy (S₁'.onTy (Ty.fvar v)) :=
              (hUfact _ (fun z hz => hbv.mem_lt z hz)).symm
          _ = V.onTy (Schk.onTy (S₁'.onTy (Ty.fvar v))) := hV _
          _ = (S₁' ++ Schk ++ V).onTy (Ty.fvar v) := by
                rw [Subst.onTy_append, Subst.onTy_append]
      have hwf₁ := Subst.onCtx_wf hS₁lc hwf
      have hwfSchk := Subst.onCtx_wf (UnifyRel.lc hSchk hτ₁lc hBlc) hwf₁
      have hwfB : CtxWF { (Schk.onCtx (S₁'.onCtx ctx)) with
          env := σd :: (Schk.onCtx (S₁'.onCtx ctx)).env } := by
        intro N hN; rcases List.mem_cons.mp hN with rfl | hN
        · exact hσwf
        · exact hwfSchk N hN
      have hσopen : Ty.BelowFvars (Φ₁' + σd.paramCount)
          (σd.openVars (freshVars Φ₁' σd.paramCount)) :=
        Ty.openVars_belowFvars (Ty.BelowFvars.of_noFreeVars hσnfv')
          (fun x hx => by have := freshVars_lt x hx; omega)
      have hSchkbelow : ∀ p ∈ Schk, Ty.BelowFvars (Φ₁' + σd.paramCount) p.2 :=
        UnifyRel.belowFvars hSchk (hbelowτ₁.mono (by omega)) hσopen
      have hbelowSchk : CtxBelow (Φ₁' + σd.paramCount) (Schk.onCtx (S₁'.onCtx ctx)) :=
        Subst.onCtx_below hSchkbelow (by omega) (Subst.onCtx_below hbelowS₁ hle hbelow)
      have hbelowB : CtxBelow (Φ₁' + σd.paramCount) { (Schk.onCtx (S₁'.onCtx ctx)) with
          env := σd :: (Schk.onCtx (S₁'.onCtx ctx)).env } := by
        intro N hN; rcases List.mem_cons.mp hN with rfl | hN
        · exact Ty.BelowFvars.of_noFreeVars hσnfv'
        · exact hbelowSchk N hN
      have hmain : V.onCtx (Schk.onCtx (S₁'.onCtx ctx))
          = (S1d ++ Schkd ++ S2d).onCtx ctx := by
        have hc : (S₁' ++ Schk ++ V).onCtx ctx = (S1d ++ Schkd ++ S2d).onCtx ctx :=
          Subst.onCtx_congr (fun v hv => (hagSchkV v hv).symm) hbelow
        rwa [Subst.onCtx_append, Subst.onCtx_append] at hc
      have hbodyV : TypeOfHM (V.onCtx { (Schk.onCtx (S₁'.onCtx ctx)) with
          env := σd :: (Schk.onCtx (S₁'.onCtx ctx)).env }) body τ := by
        show TypeOfHM ⟨V.onPolyTy σd :: V.onEnv (Schk.onCtx (S₁'.onCtx ctx)).env,
            (Schk.onCtx (S₁'.onCtx ctx)).ctors⟩ body τ
        rw [Subst.onPolyTy_eq_self_of_closed hσnfv',
            show V.onEnv (Schk.onCtx (S₁'.onCtx ctx)).env
                = ((S1d ++ Schkd ++ S2d).onCtx ctx).env from congrArg Ctx.env hmain]
        exact hbodyD
      obtain ⟨_, _, _, _, hinfbody, _, _, _⟩ := Infer.complete hwfB hbelowB hVlc hbodyV
      obtain ⟨⟨⟨Φ₂', S₂', τ₂'⟩, hbody'⟩, hebody⟩ :=
        Option.isSome_iff_exists.mp (ihb hwfB hbelowB hinfbody)
      -- drive the executable `inferCore` through its annotated-`let` arm
      rw [inferCore]
      simp only [herhs]
      rw [dif_pos (PolyTy.wf_iff_bvarsBelow.mpr hσwf),
          dif_pos (Ty.noFreeVars_iff_freeVars_nil.mp hσnfv')]
      simp only [heuni]
      rw [dif_pos hescA, dif_pos hescB]
...
theorem inferCore_complete : ∀ e, InferCoreComplete e := by
  ...
  | lambda ann body ih => exact inferCore_complete_lambda ih
  | app f inp ihf ihi => exact inferCore_complete_app ihf ihi
  | letIn ann be body ihbe ihbody => exact inferCore_complete_letIn ihbe ihbody
