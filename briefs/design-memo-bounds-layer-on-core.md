...
| **T2** | Match/coverage diagnostic spans often file-top | **Done** — binder-tagged errors → def span; body-level match → match/body hull (`bodyDiagSpan`). |
...
| **T11** | **Bounds elaborator × Infer Core coupling** | **Unparked → redesign (2026-08-04).** Root packaging fix: keep BL on Core through Infer; bounds is a second elab that rewrites list intervals; then bounds-aware coverage. **Authoritative plan:** `briefs/design-memo-bounds-preserving-elaboration.md`. Historical diagnosis only: `next-agent-brief-bounds-elaborator-architecture.md` / `design-memo-bounds-elaborator.md` (both superseded for product path). Do **not** extend erase/ofLower/Matches dual walk. |
...
| **R4** | Pair-match demand peel | **Done** — `branchBctx` / `nonListArmEnv` peel Pair field βs; `scratch/bl-r4-pair-peel.fhm` |
...
- [x] T2 body-level match spans + R4 pair-match peel  
- [x] Local E2E smoke `scripts/bl-e2e-smoke.sh` (CI hook still open)  
- [ ] E2E gate in GitHub Actions CI
