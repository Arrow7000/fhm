...
| **T11** | **Bounds elaborator × Infer Core coupling** | **Parked (architecture).** Dual Anns/Matches walks + weak use of Infer types → endless case patches when authoring freely. Dual-stack design is fine; executable elaborator needs unification. Full write-up + derisking + estimates: **`briefs/next-agent-brief-bounds-elaborator-architecture.md`**. Do **not** start without user priority + P0 plan + e2e freeze. Prefer main BL product spine until blocked. |
