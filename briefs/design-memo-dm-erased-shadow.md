...
> **CORRECTION (2026-09-09, post-session review):** D4 as originally written overstated what was agreed. The ratified content is: `.bl` stays in `Ty` (Path R carrying), elaboration's output carries a type at every node, BL is a downstream synth/check pass. The **mechanism was never decided**: both the `.found : Ty → Expr → Expr` wrapper in the same `Expr` (the owner's `.found v2` proposal; owner lean: "rather not have it unless it buys a lot") and a parallel `ElabExpr` artifact (agent recommendation) remain live options with the tradeoffs tabled in-session. The "Against `.found`" clause below was the agent's unilateral resolution of the owner's "unless needed" conditional, not an owner decision. Any implementation work must start by getting the owner's explicit mechanism choice.
